...
APP_ERROR Alexnet::Init(const InitParam &initParam) {
    ...
    if (ret != APP_ERR_OK) {
        LogError << "AlexnetPostProcess init failed, ret=" << ret << ".";
        return ret;
    }
...
APP_ERROR Alexnet::ReadImage(const std::string &imgPath, cv::Mat &imageMat) {
    imageMat = cv::imread(imgPath, cv::IMREAD_COLOR);
    LogInfo << "data" << imageMat.size();
    return APP_ERR_OK;
}

void Alexnet::ResizeImage(const cv::Mat &srcImageMat, cv::Mat &dstImageMat) {
    static constexpr uint32_t resizeHeight = 227;
    static constexpr uint32_t resizeWidth = 227;
    cv::resize(srcImageMat, dstImageMat, cv::Size(resizeWidth, resizeHeight));
}

APP_ERROR Alexnet::CVMatToTensorBase(const cv::Mat &imageMat, MxBase::TensorBase &tensorBase) {
...
APP_ERROR Alexnet::Inference(const std::vector<MxBase::TensorBase> &inputs,
...
APP_ERROR Alexnet::PostProcess(const std::vector<MxBase::TensorBase> &inputs,
...
APP_ERROR Alexnet::SaveResult(const std::string &imgPath,
...
APP_ERROR Alexnet::Process(const std::string &imgPath) {
    cv::Mat imageMat;
