...
def set_default():
    ...
    config.file_name = os.path.join(config.train_url, "yolov4")
    if config.lr_scheduler == 'cosine_annealing' and config.max_epoch > config.t_max:
        config.t_max = config.max_epoch

    config.lr_epochs = list(map(int, config.lr_epochs.split(',')))
    config.data_root = os.path.join(config.data_url, 'train2017')
    config.annFile = os.path.join(config.data_url, 'annotations/instances_train2017.json')

    config.data_val_root = os.path.join(config.data_url, 'val2017')
    config.ann_val_file = os.path.join(config.data_url, 'annotations/instances_val2017.json')
...
def export_save():
    ...
    export(network_export, input_data, file_name=config.file_name, file_format=config.file_format)
...
def run_train():
    ...
    for i, data in enumerate(data_loader):
        ...
        loss = network(images, batch_y_true_0, batch_y_true_1,
                       batch_y_true_2, batch_gt_box0, batch_gt_box1,
