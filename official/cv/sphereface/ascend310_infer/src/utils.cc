...
std::vector<std::string> GetAllFiles(std::string_view dirName) {
  ...
  while ((filename = readdir(dir)) != nullptr) {
    ...
    std::cout << dName << filename->d_type;
...
mindspore::MSTensor ReadFileToTensor(const std::string &file) {
  ...
  size_t s = ifs.tellg();
  mindspore::MSTensor buffer(file, mindspore::DataType::kNumberTypeFloat32, {static_cast<int64_t>(s)}, nullptr, s);
  ifs.seekg(0, std::ios::beg);
  ifs.read(reinterpret_cast<char *>(buffer.MutableData()), s);
  ifs.close();
