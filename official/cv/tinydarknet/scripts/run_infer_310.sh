...
function infer()
{
    ...
    mkdir time_Result
    if [ "$DVPP" == "DVPP" ];then
      echo "TinyDarknet CURRENTLY ONLY SUPPORT CPU MODE"
      exit 1
    elif [ "$DVPP" == "CPU"  ]; then
      ../ascend310_infer/out/main --mindir_path=$model --dataset_path=$data_path --cpu_dvpp=$DVPP --device_id=$device_id --image_height=224 --image_width=224 &> infer.log
    else
...
function cal_acc()
{
    python ../postprocess.py --result_path=./result_Files --label_file=$label_file &> acc.log &
}
