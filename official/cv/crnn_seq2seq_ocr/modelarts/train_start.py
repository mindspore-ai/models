...
from src.attention_ocr import AttentionOCR, AttentionOCRWithLossCell, TrainingWrapper, AttentionOCRInfer
...
def train():
    ...
    network_export = AttentionOCRInfer(config.eval_batch_size,
                                       int(config.img_width / 4),
                                       config.encoder_hidden_size,
                                       config.decoder_hidden_size,
                                       config.decoder_output_size,
                                       config.max_length,
                                       config.dropout_p)
    checkpoint_path = os.path.join(save_ckpt_path, "crnn_seq2seq_ocr-{}_{}.ckpt".format(config.keep_checkpoint_max,
                                                                                        config.steps_per_epoch))
    ckpt = load_checkpoint(checkpoint_path)
    load_param_into_net(network_export, ckpt)
    ...
    export(network_export, *inputs, file_name=os.path.join(save_ckpt_path, config.file_name), file_format="AIR")
