...
class YoloTinyHead(nn.Cell):
    def __init__(self, in_channels, out_chls, out_channels):
        super(YoloTinyHead, self).__init__()
        self.conv1 = basic_conv(in_channels, out_chls, 3)
        self.conv2 = nn.Conv2d(in_channels=out_chls, out_channels=out_channels, kernel_size=1, has_bias=True)
...
class YOLOv4_Tiny(nn.Cell):
    """
     YOLOv4_Tiny Network.

     Note:
         backbone = DarkNet53Tiny
     ...
     Examples:
         YOLOv4_Tiny(feature_shape=[1,3,416,416],
                backbone_shape=[64, 128, 256, 512, 1024]
                backbone=CspDarkNet53(),
                out_channel=255)
     """

    def __init__(self, backbone, out_channel, num_anchors, num_classes):
        super(YOLOv4_Tiny, self).__init__()
        self.out_channel = out_channel
        self.backbone = backbone
        self.conv_for_p5 = basic_conv(512, 256, 1)
        self.yolo_tiny_head1 = YoloTinyHead(256, 512, num_anchors * (5 + num_classes))
        self.yolo_tiny_head2 = YoloTinyHead(384, 256, num_anchors * (5 + num_classes))
    ...
    def construct(self, x):
        """
        input_shape of x is (batch_size, 3, h, w)
        feature_map1 is (batch_size, backbone_shape[3], h/32, w/32)
        feature_map2 is (batch_size, backbone_shape[4], h/16, w/16)
        """

        # input=(1,3,416,416)
        # feature_map1=(8, 512, 13, 13)
        # feature_map2=(8, 256, 26, 26)
        feature_map1, feature_map2 = self.backbone(x)
        # 8, 256, 13, 13
        conv_out = self.conv_for_p5(feature_map1)
        # 8, 255, 13, 13
        out0 = self.yolo_tiny_head1(conv_out)
        # 8, 128, 26, 26
        ups_out = self.upsample(conv_out)
        # 8, 128, 26, 26    8, 256, 26, 26
        cat_out = self.concat((ups_out, feature_map2))
        out1 = self.yolo_tiny_head2(cat_out)
...
class YoloLossBlock(nn.Cell):
    ...
    def construct(self, prediction, pred_xy, pred_wh, y_true, gt_box, input_shape):
        ...
        ciou = self.ciou(pred_boxes_me, true_boxes_me)
        ciou_loss = object_mask_me * box_loss_scale_me * (1 - ciou)
        ciou_loss_me = self.reduce_sum(ciou_loss, ())
        loss = ciou_loss_me * 6 + confidence_loss + class_loss
        batch_size = P.Shape()(prediction)[0]
        return loss / batch_size


class YOLOV4TinyCspDarkNet53(nn.Cell):
    """
    ...
    def __init__(self):
        super(YOLOV4TinyCspDarkNet53, self).__init__()
        self.config = default_config
        self.test_img_shape = Tensor(tuple(self.config.test_img_shape), ms.float32)

        # YOLOv4 network
        self.feature_map = YOLOv4_Tiny(backbone=CspDarkNet53Tiny(),
        ...
        self.detect_2 = DetectionBlock('s')
    ...
    def construct(self, x, input_shape=None):
        ...
        output1 = self.detect_1(big_object_output, input_shape)
        output2 = self.detect_2(small_object_output, input_shape)
...
class YoloWithLossCell(nn.Cell):
    ...
    def construct(self, x, y_true_0, y_true_1, gt_0, gt_1, input_shape):
...
class Giou(nn.Cell):
    ...
    def construct(self, box_p, box_gt):
        ...
        giou = iou - res_mid1
        giou = C.clip_by_value(giou, Tensor(-1.0, mstype.float32), Tensor(1.0, mstype.float32))
        return giou
...
class Ciou(nn.Cell):
    def __init__(self):
        ...
        self.mean = P.ReduceMean()
        self.div = P.RealDiv()
        self.eps = Tensor(1e-6, mstype.float32)
        self.t = Tensor(float('inf'), mstype.float32)
    ...
    def construct(self, box_p, box_gt):
        ...
        """
        b1_xy = (box_p[..., 2:4] + box_p[..., 0:2]) / 2
        b1_wh = box_p[..., 2:4] - box_p[..., 0:2]
        b1_wh_half = b1_wh / 2.
        b1_mins = b1_xy - b1_wh_half
        b1_maxes = b1_xy + b1_wh_half
        b2_xy = (box_gt[..., 2:4] + box_gt[..., 0:2]) / 2
        b2_wh = box_gt[..., 2:4] - box_gt[..., 0:2]
        b2_wh_half = b2_wh / 2.
        b2_mins = b2_xy - b2_wh_half
        b2_maxes = b2_xy + b2_wh_half
        intersect_mins = self.max(b1_mins, b2_mins)
        intersect_maxes = self.min(b1_maxes, b2_maxes)
        intersect_wh = self.max(intersect_maxes - intersect_mins, self.zeroslike(intersect_maxes))
        intersect_area = intersect_wh[..., 0:1] * intersect_wh[..., 1:2]
        b1_area = b1_wh[..., 0:1] * b1_wh[..., 1:2]
        b2_area = b2_wh[..., 0:1] * b2_wh[..., 1:2]
        union_area = C.clip_by_value(b1_area + b2_area - intersect_area, self.eps, self.t)
        iou = intersect_area / union_area
        center_distance = self.sum(self.pow((b1_xy - b2_xy), 2), -1)
        enclose_mins = self.min(b1_mins, b2_mins)
        enclose_maxes = self.max(b1_maxes, b2_maxes)
        enclose_wh = self.max(enclose_maxes - enclose_mins, self.zeroslike(intersect_maxes))
        enclose_diagonal = self.sum(self.pow(enclose_wh, 2), -1)
        ciou = iou - 1.0 * (center_distance) / C.clip_by_value(enclose_diagonal, self.eps, self.t)
        v = (4 / (math.pi ** 2)) * self.pow(
            (self.atan(b1_wh[..., 0:1] / C.clip_by_value(b1_wh[..., 1:2], self.eps, self.t))
             - self.atan(b2_wh[..., 0:1] / C.clip_by_value(b2_wh[..., 1:2], self.eps, self.t))), 2)
        alpha = v / C.clip_by_value(1.0 - iou + v, self.eps, self.t)
        ciou = ciou - alpha * v
        return ciou
