...
def run():
    ...
    with open("./simclr.pipeline", 'rb') as f:
        pipelineStr = f.read()
    ...
    for file_name in file_list:
        ...
        if not (file_name.lower().endswith(".png") or file_name.lower().endswith(".bmp")):
            continue

        with open(file_path, 'rb') as f:
            data_input.data = f.read()

        stream_name = b'im_simclr'
        in_plugin_id = 0
        ...
        if load_dict.get('MxpiClass') is None:
            with open(res_dir_name + "/" + file_name[:-suffix_len] + '.txt',
                      'w') as f_write:
                f_write.write("")
            continue
        res_vec = load_dict.get('MxpiClass')

        with open(res_dir_name + "/" + file_name[:-suffix_len] + '_1.txt',
                  'w') as f_write:
