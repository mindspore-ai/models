...
if [[ $# -lt 5 || $# -gt 6 ]]; then
    echo "Usage: bash run_infer_310.sh [AIR_PATH] [DATA_PATH] [ANNO_PATH] [IMAGESHAPE_PATH] [IMAGEID_PATH] [DEVICE_ID]
...
image_shape_path=$(get_real_path $4)
image_id_path=$(get_real_path $5)
...
echo "image id path: "$image_id_path
...
function air_to_om()
{
    atc --input_format=NCHW --framework=1 --model=$model --output=yolov3_darknet53_quant --soc_version=Ascend310 &> atc.log
}
...
function infer()
{
    ...
    ../ascend310_infer/src/out/main --om_path=./yolov3_darknet53_quant.om --dataset_path=$data_path --acljson_path=../ascend310_infer/src/acl.json --device_id=$device_id &> infer.log
