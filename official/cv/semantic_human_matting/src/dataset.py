...
class HumanMattingData:
    ...
    def __getitem__(self, index):
        # read files
        image, trimap, alpha = read_files(os.path.join(self.data_root, self.train_mode),
...
def create_dataset(cfg, usage='train', repeat_num=1):
    ...
    if cfg['group_size'] == 1:
        dataset = ds.GeneratorDataset(dataset_generator, ["image", "trimap_ch", "trimap", "alpha"],
                                      num_parallel_workers=cfg['nThreads'], shuffle=False,
        ...
        dataset = ds.GeneratorDataset(dataset_generator, ["image", "trimap_ch", "trimap", "alpha"],
                                      num_parallel_workers=cfg['nThreads'], sampler=sampler,
                                      python_multiprocessing=True)
