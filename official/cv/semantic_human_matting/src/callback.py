...
class TrainCallBack(Callback):
    ...
    def step_end(self, run_context):
        ...
        if self._per_print_times != 0 and cb_params.cur_step_num % self._per_print_times == 0:
            print("train epoch: %s step: %s, loss: %s, speed: %s" % (
                cb_params.cur_epoch_num + self.cur_epoch, cur_step_in_epoch, loss,
                time.time() - self.time_step_begin), flush=True)
    ...
    def epoch_end(self, run_context):
        ...
        if cur_epoch % self.eval_per_epoch == 0:
            ...
            log = "{} {} [{}/{}]\tstage: {}\tspeed: {:.5f}s\tlr: {:.5f}\tloss: {:.5f}\t" \
                .format(current_time, 'train', cur_epoch + self.cur_epoch, self.cfg['nEpochs'],
                        self.cfg['train_phase'], time.time() - self.start_time, float(self.cfg['lr']), loss)
            print(log)
            ...
            if self.cfg['train_phase'] != 'pre_train_t_net':
                ...
                current_time = datetime.strftime(datetime.now(), '%Y-%m-%d %H:%M:%S')
                log = "{} {} [{}/{}]\tstage: {}\tspeed: {:.5f}s\tlr: {:.5f}\tloss: {:.5f}\tsad: {:.5f}\t" \
                    .format(current_time, 'eval', cur_epoch + self.cur_epoch, self.cfg['nEpochs'],
                            self.cfg['train_phase'], time.time() - t0, float(self.cfg['lr']),
                            ret["sad"][1], np.float(ret["sad"][0]))
                print(log)
                self.trainlog.save_log(log, mode='latest')
    ...
    def end(self, run_context):
        ...
        if self.stage == 'end_to_end':
            ...
            print('metrics sad: num: {}\tsum_sad: {}\tave_sad: {}'.format(
    ...
    def step_end(self, run_context):
        ...
        if isinstance(loss, float) and (np.isnan(loss) or np.isinf(loss)):
            raise ValueError("epoch: {} step: {}. Invalid loss, terminating training.".format(
                cb_params.cur_epoch_num + self.cur_epoch, cur_step_in_epoch))
        if self._per_print_times != 0 and cb_params.cur_step_num % self._per_print_times == 0:
            print("epoch: %s step: %s, loss is %s" % (cb_params.cur_epoch_num + self.cur_epoch,
