...
def get_offset(h_in, w_in, h_out, w_out, align_corners=False):
    ...
    weight_lrtb = (Tensor(weight_lt.reshape(1, 1, h_out, w_out).astype(np.float32)),
...
class InvertedResidual(nn.Cell):
    ...
    def __init__(self, inp, oup, stride, expand_ratio):
        ...
        self.conv = nn.SequentialCell([
            # pw
            nn.Conv2d(inp, inp * expand_ratio, 1, 1, pad_mode='same', padding=0, has_bias=False, weight_init='Uniform'),
            nn.BatchNorm2d(inp * expand_ratio),
            nn.ReLU6(),
            # dw
            nn.Conv2d(inp * expand_ratio, inp * expand_ratio, 3, stride, pad_mode='pad', padding=1,
                      group=inp * expand_ratio, has_bias=False, weight_init='Uniform'),
            nn.BatchNorm2d(inp * expand_ratio),
            nn.ReLU6(),
            # pw-linear
            nn.Conv2d(inp * expand_ratio, oup, 1, 1, pad_mode='same', padding=0, has_bias=False, weight_init='Uniform'),
            nn.BatchNorm2d(oup),
        ])
...
class mobilenet_v2(nn.Cell):
    ...
    def __init__(self, nInputChannels=3):
        ...
        self.head_conv = nn.SequentialCell([
            nn.Conv2d(in_channels=nInputChannels, out_channels=32, kernel_size=3, stride=1,
                      pad_mode='pad', padding=1, has_bias=False, weight_init='Uniform'),
            nn.BatchNorm2d(32),
            nn.ReLU()])
        ...
        # 1/4
        self.block_2 = nn.SequentialCell([
...
class T_mv2_unet(nn.Cell):
    ...
    def __init__(self, classes=3):
        ...
        # decoder
        # ---------------------
        self.s5_up_conv = nn.SequentialCell([Upsample(scale_factor=2, align_corners=False),
                                             nn.Conv2d(320, 96, 3, 1, padding=1, pad_mode='pad', has_bias=True,
                                                       weight_init='Uniform', bias_init='Uniform'),
                                             nn.BatchNorm2d(96),
                                             nn.ReLU()])
        self.s4_fusion = nn.SequentialCell([nn.Conv2d(96, 96, 3, 1, padding=1, pad_mode='pad', has_bias=True,
                                                      weight_init='Uniform', bias_init='Uniform'),
                                            nn.BatchNorm2d(96)])

        self.s4_up_conv = nn.SequentialCell([Upsample(scale_factor=2, align_corners=False),
                                             nn.Conv2d(96, 32, 3, 1, padding=1, pad_mode='pad', has_bias=True,
                                                       weight_init='Uniform', bias_init='Uniform'),
                                             nn.BatchNorm2d(32),
                                             nn.ReLU()])
        self.s3_fusion = nn.SequentialCell([nn.Conv2d(32, 32, 3, 1, padding=1, pad_mode='pad', has_bias=True,
                                                      weight_init='Uniform', bias_init='Uniform'),
                                            nn.BatchNorm2d(32)])

        self.s3_up_conv = nn.SequentialCell([Upsample(scale_factor=2, align_corners=False),
                                             nn.Conv2d(32, 24, 3, 1, padding=1, pad_mode='pad', has_bias=True,
                                                       weight_init='Uniform', bias_init='Uniform'),
                                             nn.BatchNorm2d(24),
                                             nn.ReLU()])
        self.s2_fusion = nn.SequentialCell([nn.Conv2d(24, 24, 3, 1, padding=1, pad_mode='pad', has_bias=True,
                                                      weight_init='Uniform', bias_init='Uniform'),
                                            nn.BatchNorm2d(24)])

        self.s2_up_conv = nn.SequentialCell([Upsample(scale_factor=2, align_corners=False),
                                             nn.Conv2d(24, 16, 3, 1, padding=1, pad_mode='pad', has_bias=True,
                                                       weight_init='Uniform', bias_init='Uniform'),
                                             nn.BatchNorm2d(16),
                                             nn.ReLU()])
        self.s1_fusion = nn.SequentialCell([nn.Conv2d(16, 16, 3, 1, padding=1, pad_mode='pad', has_bias=True,
                                                      weight_init='Uniform', bias_init='Uniform'),
                                            nn.BatchNorm2d(16)])

        self.last_conv = nn.Conv2d(16, classes, 3, 1, padding=1, pad_mode='pad', has_bias=True,
                                   weight_init='Uniform', bias_init='Uniform')
