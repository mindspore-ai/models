...
APP_ERROR semantic_human_matting::Init(const InitParam &initParam) {
...
APP_ERROR semantic_human_matting::DeInit() {
    model_->DeInit();
...
APP_ERROR semantic_human_matting::ReadTensorFromFile(const std::string &file, uint32_t *data, uint32_t size) {
...
APP_ERROR semantic_human_matting::ReadInputTensor(const std::string &fileName,
...
APP_ERROR semantic_human_matting::Inference(const std::vector<MxBase::TensorBase> &inputs,
...
APP_ERROR  semantic_human_matting::WriteResult(const std::string& imageFile, std::vector<MxBase::TensorBase> &outputs) {
    ...
    for (size_t i = 0; i < outputs.size(); ++i) {
        ...
        fileName.replace(imageFile.find('.'), imageFile.size() - imageFile.find('.'), '_' + std::to_string(i) + ".bin");
        std::string outFileName = infer_result_path + "/" + fileName;
...
APP_ERROR semantic_human_matting::Process(const std::string &inferPath, const std::string &fileName) {
    std::vector<MxBase::TensorBase> inputs = {};
