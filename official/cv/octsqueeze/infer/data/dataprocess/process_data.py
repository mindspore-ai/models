        ...
        if node and node.attribute != "leaf":
            ...
            if node.depth < 4:
                for _ in range(node.depth):
                    node_info += [node_cur.position[0], node_cur.position[1], node_cur.position[2],
                                  node_cur.octant, node_cur.depth, node_data2int(node_cur.parent.data)]
...
def feature_arrange(nodes, points_num):
    nodes_num = len(nodes)
    node_info = {'gt': np.empty(nodes_num), 'cur_node': np.empty([nodes_num, 6]),
                 'parent1': np.empty([nodes_num, 6]), 'parent2': np.empty([nodes_num, 6]),
                 'parent3': np.empty([nodes_num, 6]), 'points_num': points_num}
...
def write_data(data_dir, points_num, inp, gt):
    ...
    # points_num
    fpath = os.path.join(data_dir, 'points_num.txt')
    np.savetxt(fpath, [points_num], fmt='%i', delimiter=' ')

    # feature
    fpath = os.path.join(data_dir, 'input.txt')
    np.savetxt(fpath, inp, fmt='%s', delimiter=' ')

    # gt
    fpath = os.path.join(data_dir, 'gt.txt')
    np.savetxt(fpath, gt, fmt='%i', delimiter=' ')


def KITTI_feature(input_route, output_route, min_file, max_file):
    if not os.path.exists(output_route): os.makedirs(output_route)
    if args.mode == 'train':
        ...
        for precision in precision_oct:
            ...
            if not os.path.exists(output_route_precision): os.makedirs(output_route_precision)
            for i in range(min_file, max_file + 1, 1):
                data_route = input_route + str(i).zfill(6) + '.bin'
                pcd_example = bin_loader(data_route)
                points_num = pcd_example.shape[0]
                ...
                print("Precision: {} ;Feature extraction from {} is done"
                      .format(precision, data_dir + '.bin'))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--mode', '-m', type=str, default='inference',
                        choices=['train', 'inference'], help='train or inference')
    args = parser.parse_args()

    KITTI_feature(input_route=args.input_route, output_route=args.output_route,
                  min_file=args.min_file, max_file=args.max_file)
