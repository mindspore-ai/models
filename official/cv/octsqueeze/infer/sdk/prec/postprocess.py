...
def def_arguments():
    ...
    parser.add_argument('--pipeline', '-m', type=str, default='../data/config/octsqueeze.pipeline',
                        help='route of model')
...
def run_octsqueeze(dataset_path, precision_oct, if_eval=True):
    ...
    for frame_idx, frame in enumerate(frames):
        ...
        outputs = np.fromfile('results/{}/{:06d}_results.bin'.format(precision_oct, frame_n),
    ...
    if if_eval:
        print("Precision: {} - Totally time cost: {} - Average cost: {}"
              .format(precision_oct, time_sum, time_sum / frame_num))
    return bpip_CD


if __name__ == '__main__':
    args = def_arguments()
    if not os.path.exists(args.compression): os.makedirs(args.compression)
