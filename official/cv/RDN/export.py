...
parser = argparse.ArgumentParser(description='rdn export')
parser.add_argument("--batch_size", type=int, default=1, help="batch size")
parser.add_argument("--ckpt_path", type=str, required=True, help="path of checkpoint file")
parser.add_argument("--file_name", type=str, default="rdn", help="output file name.")
...
def run_export(args):
    ...
    net = RDN(arg)
    max_lr_size = MAX_HR_SIZE // arg.scale[arg.task_id]
    param_dict = load_checkpoint(args.ckpt_path)
    load_param_into_net(net, param_dict)
    net.set_train(False)
    print('load mindspore net and checkpoint successfully.')
    inputs = Tensor(np.ones([args.batch_size, 3, max_lr_size, max_lr_size], np.float32))
