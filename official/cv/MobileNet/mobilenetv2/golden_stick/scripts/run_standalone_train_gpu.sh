...
if [ $# != 3 ] && [ $# != 5 ]; then
    ...
    echo "    bash run_standalone_train_gpu.sh /path/to/train.py mobilenetv2_cifar10_config.yaml /path/to/dataset"
    echo "  Train from full precision checkpoint:"
    echo "    bash run_standalone_train_gpu.sh /path/to/train.py mobilenetv2_cifar10_config.yaml /path/to/dataset FP32 /path/to/fp32_ckpt"
    echo "  Train from pretrained checkpoint:"
    echo "    bash run_standalone_train_gpu.sh /path/to/train.py mobilenetv2_cifar10_config.yaml /path/to/dataset PRETRAINED /path/to/pretrained_ckpt"
    exit 1
fi
...
if [ "x$CKPT_TYPE" == "xFP32" ]; then
  python train.py --config_path=$CONFIG_FILE --run_distribute=False --platform="GPU" \
         --dataset_path=$DATASET_PATH --fp32_ckpt=$CKPT_FILE --output_path './output' &> log &
elif [ "x$CKPT_TYPE" == "xPRETRAINED" ]; then
  python train.py --config_path=$CONFIG_FILE --run_distribute=False --platform="GPU" \
         --dataset_path=$DATASET_PATH --pretrain_ckpt=$CKPT_FILE --output_path './output' &> log &
else
  python train.py --config_path=$CONFIG_FILE --run_distribute=False --platform="GPU" \
         --dataset_path=$DATASET_PATH --output_path './output' &> log &
fi
