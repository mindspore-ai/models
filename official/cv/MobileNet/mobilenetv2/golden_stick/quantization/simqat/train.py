...
from src.dataset import create_dataset_cifar10, extract_features
...
def train_mobilenetv2():
    ...
    if config.mode_name == "GRAPH":
        ms.set_context(mode=ms.GRAPH_MODE, device_target=config.platform,
                       save_graphs=False, enable_graph_kernel=True)
    else:
        ms.set_context(mode=ms.PYNATIVE_MODE, device_target=config.platform,
                       save_graphs=False, enable_graph_kernel=True)
    if config.run_distribute:
        ...
        ms.set_auto_parallel_context(parallel_mode=ms.ParallelMode.DATA_PARALLEL,
                                     gradients_mean=True)
    config.train_dataset_path = config.dataset_path
    config.eval_dataset_path = config.dataset_path
    ...
    print("\nconfig: {} \n".format(config))
    # define network
    backbone_net, head_net, network = define_net(config, config.is_training)
    algo = create_simqat()
    net = algo.apply(network)
    dataset = create_dataset_cifar10(dataset_path=config.train_dataset_path, do_train=True, config=config,
