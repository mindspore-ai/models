...
# ============================================================================
"""Run onnx evaluation for a model """
...
def retinanet_eval():
    ...
    session, input_names = create_session(config.onnx_path, config.device_target)
    ...
    for data in ds.create_dict_iterator(output_numpy=True):
        ...
        output = session.run(None, {input_names[0]: img_np})
        all_boxes, all_box_scores = output
        for batch_idx in range(img_np.shape[0]):
            pred_data.append({"boxes": all_boxes[batch_idx],
                              "box_scores": all_box_scores[batch_idx],
