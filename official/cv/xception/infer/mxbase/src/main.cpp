...
int main(int argc, char* argv[]) {
    ...
    initParam.modelPath = "../data/models/xception.om";
    auto inferXception = std::make_shared<Xception>();
    APP_ERROR ret = inferXception->Init(initParam);
    if (ret != APP_ERR_OK) {
        inferXception->DeInit();
        LogError << "XceptionClassify init failed, ret=" << ret << ".";
        return ret;
    }
    ...
    for (auto &imgFile : imgFilePaths) {
        ret = inferXception->Process(imgFile);
        if (ret != APP_ERR_OK) {
            LogError << "XceptionClassify process failed, ret=" << ret << ".";
            inferXception->DeInit();
            return ret;
        }
    }
    inferXception->DeInit();
    double fps = 1000.0 * imgFilePaths.size() / inferXception->GetInferCostMilliSec();
