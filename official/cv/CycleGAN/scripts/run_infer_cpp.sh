...
if [[ $# -lt 5 || $# -gt 6 ]]; then
    echo "Usage: bash run_infer_cpp.sh [MINDIR_PATH] [DATASET_PATH] [DATASET_MODE] [NEED_PREPROCESS] [DEVICE_TARGET] [DEVICE_ID]
    DEVICE_TARGET must choose from ['GPU', 'CPU', 'Ascend']
    DATASET_MODE must choose from ['AtoB', 'BtoA']
...
function infer()
{
    ...
    cpp_infer/out/main --device_type=$device_target --mindir_path=$model --input0_path=./preprocess_Result/00_data --device_id=$device_id --fusion_switch_path=./cpp_infer/fusion_switch.cfg &> infer.log
