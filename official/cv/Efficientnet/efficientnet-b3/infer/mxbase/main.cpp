...
int main(int argc, char* argv[]) {
    if (argc <= 1) {
        LogWarn << "Please input image path, such as './build/efficentnet_b3 ./image_dir'.";
        return APP_ERR_OK;
    }
    ...
    initParam.modelPath = "../data/model/efficient_b3.om";
    auto efficientnet = std::make_shared<EfficientnetClassify>();
    APP_ERROR ret = efficientnet->Init(initParam);
    if (ret != APP_ERR_OK) {
        LogError << "EfficientnetClassify init failed, ret=" << ret << ".";
        return ret;
    }
    ...
    for (auto & entry : fs::directory_iterator(imgDir)) {
        ...
        ret = efficientnet->Process(entry.path());
        if (ret != APP_ERR_OK) {
            LogError << "EfficientnetClassify process failed, ret=" << ret << ".";
            efficientnet->DeInit();
            return ret;
        }
    }
