...
#include <string>
#include <vector>
#include "Efficientnet_b3Classify.h"
...
APP_ERROR EfficientnetClassify::Init(const InitParam &initParam) {
...
APP_ERROR EfficientnetClassify::DeInit() {
    model_->DeInit();
    post_->DeInit();
...
APP_ERROR EfficientnetClassify::ReadImage(const std::string &imgPath, cv::Mat &imageMat) {
    imageMat = cv::imread(imgPath, cv::IMREAD_COLOR);
    return APP_ERR_OK;
}

APP_ERROR EfficientnetClassify::CenterCropImage(const cv::Mat &img, cv::Mat &cropImg) {
...
APP_ERROR EfficientnetClassify::Resize(const cv::Mat &srcImageMat, cv::Mat &dstImageMat) {
...
APP_ERROR EfficientnetClassify::CVMatToTensorBase(const cv::Mat &imageMat, MxBase::TensorBase &tensorBase) {
    const uint32_t dataSize = imageMat.cols * imageMat.rows * MxBase::YUV444_RGB_WIDTH_NU * 4;
...
APP_ERROR EfficientnetClassify::Inference(const std::vector<MxBase::TensorBase> &inputs,
...
APP_ERROR EfficientnetClassify::PostProcess(const std::vector<MxBase::TensorBase> &inputs,
...
APP_ERROR EfficientnetClassify::SaveInferResult(const std::string &imagePath,
...
APP_ERROR EfficientnetClassify::Process(const std::string &imgPath) {
    cv::Mat imageMat;
    ...
    if (ret != APP_ERR_OK) {
        LogError << "Inference process failed, ret: " << ret << ".";
        return ret;
    }
    ...
    if (ret != APP_ERR_OK) {
        LogError << "Post process failed, ret: " << ret << ".";
        return ret;
    }
