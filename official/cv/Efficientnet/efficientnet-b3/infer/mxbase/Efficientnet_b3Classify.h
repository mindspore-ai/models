...
#ifndef MXBASE_EFFICIENTNETCLASSIFY_H
#define MXBASE_EFFICIENTNETCLASSIFY_H
...
class EfficientnetClassify {
 public:
     APP_ERROR Init(const InitParam &initParam);
     APP_ERROR DeInit();
     APP_ERROR ReadImage(const std::string &imgPath, cv::Mat &imageMat);
     APP_ERROR Resize(const cv::Mat &srcImageMat, cv::Mat &dstImageMat);
     APP_ERROR CenterCropImage(const cv::Mat &img, cv::Mat &cropImg);
