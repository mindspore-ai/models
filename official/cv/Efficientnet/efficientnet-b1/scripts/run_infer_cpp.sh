...
if [[ $# -lt 3 || $# -gt 4 ]]; then
    echo "Usage: bash scripts/run_infer_cpp.sh [MINDIR_PATH] [DATA_PATH] [DEVICE_TYPE] [DEVICE_ID]
...
function infer()
{
    ...
    mkdir result_Files
    mkdir time_Result
    ./cpp_infer/src/main --device_type=$device_type --mindir_path=$model --dataset_path=$data_path --device_id=$device_id  &> infer.log
}
