...
int main(int argc, char **argv) {
  ...
  std::shared_ptr<TensorTransform> resize = std::make_shared<Resize>(std::vector<int>{280});
  std::shared_ptr<TensorTransform> centercrop = std::make_shared<CenterCrop>(std::vector<int>{260});
  std::shared_ptr<TensorTransform> normalize = std::make_shared<Normalize>(
      std::vector<float>{127.50, 127.50, 127.50}, std::vector<float>{127.50, 127.50, 127.50});
