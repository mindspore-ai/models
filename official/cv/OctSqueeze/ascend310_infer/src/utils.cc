...
void read_points_num(const string &data_dir, int32_t &var) {
    var = 0;
    string path = data_dir + "/points_num.bin";
...
void read_input(const string &data_dir, std::vector<float> &vec) {
    vec.clear();
    string path = data_dir + "/input.bin";
    ...
    if (fp.is_open()) {
      ...
      while (std::getline(fp, line)) {
          if (0) {
              float var = std::static_cast<float>(atof(line.c_str()));
              char mem[PATH_MAX] = {'\0'};
              snprintf(mem, PATH_MAX, "%.16f", var);
              std::cout << "line=" << line << ", var=" << mem << std::endl;
          }
          vec.push_back(var);
      }
      fp.close();
...
void read_gt(const string &data_dir, std::vector<uint32_t> &vec) {
    vec.clear();
    string path = data_dir + "/gt.bin";
    std::ifstream fp(path.c_str(), std::ios::binary);
...
std::string print_data_shape(const std::vector<int64> &shape) {
    std::string var;
    for (size_t i = 0; i < shape.size(); ++i) {
        var += to_string(shape[i]) + ", ";
    }
    return var;
}
...
float WriteResult(const std::string &data_dir, const std::vector<MSTensor> &outputs,
    ...
    for (size_t batch_idx = 0; batch_idx < batch_num; ++batch_idx) {
        ...
        std::vector<int64> shape = output.Shape();
