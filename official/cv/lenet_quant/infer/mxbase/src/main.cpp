...
//    }
    ...
    initParam.modelPath = "../data/model/lenet_quant.om";
    std::string dataPath = "../data/lenet_quant_mnist_310_infer_data/mnist_bs_1_bin/";
    std::string annoPath = "../data/lenet_quant_mnist_310_infer_data/mnist_bs_1_label.txt";

    auto model_lenet_quant = std::make_shared<lenet_quant>();
    APP_ERROR ret = model_lenet_quant->Init(initParam);
    if (ret != APP_ERR_OK) {
    ...
    for (int i=0; i < img_size; i++) {
        ret = model_lenet_quant->Process(dataPath + imagesPath[i], initParam, outputs);
        if (ret !=APP_ERR_OK) {
            LogError << "Lenet-quant process failed, ret=" << ret << ".";
            model_lenet_quant->DeInit();
            return ret;
        }
    }
    ...
    double total_time = model_lenet_quant->GetInferCostMilliSec() / 1000;
