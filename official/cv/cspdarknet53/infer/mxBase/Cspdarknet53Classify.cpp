...
#include <string>
#include <vector>
#include "Cspdarknet53Classify.h"
...
APP_ERROR CspdarknetClassify::Init(const InitParam &initParam) {
...
APP_ERROR CspdarknetClassify::DeInit() {
    dvppWrapper_->DeInit();
    model_->DeInit();
    post_->DeInit();
...
APP_ERROR CspdarknetClassify::ReadImage(const std::string &imgPath, cv::Mat &imageMat) {
    imageMat = cv::imread(imgPath, cv::IMREAD_COLOR);
    return APP_ERR_OK;
}

APP_ERROR CspdarknetClassify::CenterCropImage(const cv::Mat &img, cv::Mat &cropImg) {
...
APP_ERROR CspdarknetClassify::Resize(const cv::Mat &srcImageMat, cv::Mat &dstImageMat) {
...
APP_ERROR CspdarknetClassify::CVMatToTensorBase(const cv::Mat &imageMat, MxBase::TensorBase &tensorBase) {
...
APP_ERROR CspdarknetClassify::Inference(const std::vector<MxBase::TensorBase> &inputs,
...
APP_ERROR CspdarknetClassify::PostProcess(const std::vector<MxBase::TensorBase> &inputs,
...
APP_ERROR CspdarknetClassify::SaveInferResult(const std::string &imagePath,
...
APP_ERROR CspdarknetClassify::Process(const std::string &imgPath) {
    cv::Mat imageMat;
