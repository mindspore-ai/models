...
int main(int argc, char* argv[]) {
    if (argc <= 1) {
        LogWarn << "Please input image path, such as './build/cspdarknet53 ./image_dir'.";
        return APP_ERR_OK;
    }
    ...
    initParam.modelPath = "../data/models/cspdarknet53.om";
    auto cspdarknet = std::make_shared<CspdarknetClassify>();
    APP_ERROR ret = cspdarknet->Init(initParam);
    if (ret != APP_ERR_OK) {
        LogError << "CspdarknetClassify init failed, ret=" << ret << ".";
        return ret;
    }
    ...
    for (auto & entry : fs::directory_iterator(imgDir)) {
        ...
        ret = cspdarknet->Process(entry.path());
        if (ret != APP_ERR_OK) {
            LogError << "CspdarknetClassify process failed, ret=" << ret << ".";
            cspdarknet->DeInit();
            return ret;
        }
    }
