...
if __name__ == '__main__':
    ...
    # load weights
    load_param_into_net(netG,
                        load_checkpoint('{0}/netG_epoch_{1}.ckpt'.format(args_opt.experiment, args_opt.niter - 1)))

    # initialize noise
    fixed_noise = Tensor(np.random.normal(size=[1, nz, 1, 1]), dtype=mstype.float32)

    # frozen to air file
    export(netG, fixed_noise, file_name='{0}/WGAN'.format(args_opt.experiment), file_format="AIR")
