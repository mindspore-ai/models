...
if [[ $# -lt 5 || $# -gt 6 ]]; then
    echo "Usage: bash run_infer_cpp.sh [MINDIR_PATH] [CONFIG_PATH] [NEED_PREPROCESS] [NIMAGES] [DEVICE_TYPE] [DEVICE_ID]
...
function infer()
{
    ...
    ../cpp_infer/out/main --device_type=$device_type --mindir_path=$model --input0_path=./preprocess_Result --device_id=$device_id &> infer.log
