...
class CoarseNet(nn.Cell):
    def __init__(self, init_weights=True):
        ...
        self.conv2 = nn.Conv2d(in_channels=64, out_channels=128, kernel_size=5, padding=(1, 1, 2, 2), pad_mode="pad",
                               has_bias=True)
        ...
        self.deconv6 = nn.Conv2dTranspose(in_channels=512, out_channels=256, kernel_size=2, stride=2, pad_mode="pad",
                                          has_bias=True)
        self.conv7 = nn.Conv2d(in_channels=512, out_channels=128, kernel_size=3, pad_mode="same", has_bias=True)
        self.deconv8 = nn.Conv2dTranspose(in_channels=128, out_channels=64, kernel_size=2, stride=2, pad_mode="pad",
                                          has_bias=True)
        self.conv9 = nn.Conv2d(in_channels=192, out_channels=64, kernel_size=3, padding=(1, 0, 0, 0), pad_mode="pad",
                               has_bias=True)
    ...
    def _initialize_weights(self):
        for _, m in self.cells_and_names():
            if isinstance(m, (nn.Conv2d)):
                ...
                m.weight.set_data(
                    Tensor(np.random.normal(0, 0.001, m.weight.data.shape).astype("float32")))
                m.bias.set_data(
                    Tensor(np.random.normal(0, 0.001, m.bias.data.shape).astype("float32")))
