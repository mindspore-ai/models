...
using MxBase::Resnet50PostProcess;
...
APP_ERROR VIT::Init(const InitParam &initParam) {
...
void VIT::read_image(const std::string &imgPath, cv::Mat *imageMat) {
    *imageMat = cv::imread(imgPath, cv::IMREAD_COLOR);
}

void VIT::resize_image(const cv::Mat &srcImageMat, cv::Mat *dstImageMat) {
    static constexpr uint32_t resizeHeight = 264;
    static constexpr uint32_t resizeWidth = 264;
    cv::resize(srcImageMat, *dstImageMat, cv::Size(resizeWidth, resizeHeight));
}

void VIT::crop_image(const cv::Mat &srcImageMat, cv::Mat &dstImageMat) {
    int img_size = 224;
    int crop_x = 20;
    int crop_y = 20;
    static cv::Rect rectOfImg(crop_x, crop_y, img_size, img_size);
    dstImageMat = srcImageMat(rectOfImg).clone();
}

APP_ERROR VIT::CVMatToTensorBase(const cv::Mat &imageMat,
...
APP_ERROR VIT::Inference(const std::vector<MxBase::TensorBase> &inputs,
...
APP_ERROR VIT::PostProcess(const std::vector<MxBase::TensorBase> &inputs,
...
APP_ERROR VIT::SaveResult(const std::string &imgPath,
...
APP_ERROR VIT::Process(const std::string &imgPath) {
    cv::Mat imageMat;
    read_image(imgPath, &imageMat);
    cv::Mat resizeimg;
    resize_image(imageMat, &resizeimg);
    cv::Mat cropImage;
    crop_image(resizeimg, cropImage);
    std::vector<MxBase::TensorBase> inputs = {};
    std::vector<MxBase::TensorBase> outputs = {};
    TensorBase tensorBase;
    APP_ERROR ret = CVMatToTensorBase(cropImage, &tensorBase);
    if (ret != APP_ERR_OK) {
