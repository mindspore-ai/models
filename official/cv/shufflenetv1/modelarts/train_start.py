...
parser = argparse.ArgumentParser('mindspore shufflenetv1 training')
...
config.train_dataset_path = CACHE_DATA_URL
config.save_checkpoint = True
config.save_ckpt_path = CACHE_TRAIN_URL
config.epoch_size = args.epoch_size if args.epoch_size else config.epoch_size
config.resume = args.pre_trained if args.pre_trained else config.resume
...
def model_export():
    ...
    ckpt_file = get_last_ckpt(config.save_ckpt_path)
    if not ckpt_file:
        return
    ...
    input_arr = Tensor(numpy.ones([config.batch_size, 3, image_height, image_width]), mindspore.float32)
    export(net, input_arr, file_name=os.path.join(config.save_ckpt_path, config.file_name),
