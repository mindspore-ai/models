...
int main(int argc, char* argv[]) {
    if (argc <= 1) {
        LogWarn << "Please input image path, such as './shufflenetv1 image_dir'";
        return APP_ERR_OK;
    }
    ...
    auto shufflenetv1 = std::make_shared<Shufflenetv1>();
    ...
    for (auto &imgFile : imgFilePaths) {
        ret = shufflenetv1->Process(imgFile);
        if (ret !=APP_ERR_OK) {
            LogError << "Shufflenetv1 Classify process failed, ret=" << ret << ".";
            shufflenetv1->DeInit();
            return ret;
        }
    }
    ...
    double fps = 1000.0*imgFilePaths.size() / shufflenetv1->GetInferCostMilliSec();
