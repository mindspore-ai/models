...
#include <string>
#include <vector>
#include "Shufflenetv1.h"
#include "MxBase/DeviceManager/DeviceManager.h"
#include "MxBase/Log/Log.h"


APP_ERROR Shufflenetv1::Init(const InitParam &initParam) {
    ...
    if (ret != APP_ERR_OK) {
        LogError << "Shufflenetv1PostProcess init failed, ret=" << ret << ".";
        return ret;
    }
...
APP_ERROR Shufflenetv1::ReadImage(const std::string &imgPath, cv::Mat &imageMat) {
    imageMat = cv::imread(imgPath, cv::IMREAD_COLOR);
    LogInfo << "data" << imageMat.size();
    return APP_ERR_OK;
}

void Shufflenetv1::ResizeImage(const cv::Mat &srcImageMat, cv::Mat &dstImageMat) {
    static constexpr uint32_t resizeHeight = 268;
    static constexpr uint32_t resizeWidth = 268;
    cv::resize(srcImageMat, dstImageMat, cv::Size(resizeWidth, resizeHeight));
}

void Shufflenetv1::CropImage(const cv::Mat &srcImageMat, cv::Mat &dstImageMat) {
    static cv::Rect centerCrop((268 - 224)/2, (268 - 224)/2, 224, 224);
    dstImageMat = srcImageMat(centerCrop).clone();
}

APP_ERROR Shufflenetv1::CVMatToTensorBase(const cv::Mat &imageMat, MxBase::TensorBase &tensorBase) {
...
APP_ERROR Shufflenetv1::Inference(const std::vector<MxBase::TensorBase> &inputs,
...
APP_ERROR Shufflenetv1::PostProcess(const std::vector<MxBase::TensorBase> &inputs,
...
APP_ERROR Shufflenetv1::SaveResult(const std::string &imgPath,
...
APP_ERROR Shufflenetv1::Process(const std::string &imgPath) {
    cv::Mat imageMat;
