...
std::vector<MxBase::TensorBase> *tensors) {
    for (int i = 0; i < tensor_package_list.tensorpackagevec_size(); ++i) {
        for (int j = 0; j < tensor_package_list.tensorpackagevec(i).tensorvec_size(); j++) {
            ...
            memory_data.ptrData = reinterpret_cast<void *>(tensor_package_list.
...
/**
 * @brief Get information of origin input image and aligned image
...
static void NMS(cv::Mat *plain, float threshold) {
    ...
    *plain = plain_with_border_clone(cv::Rect(border_padding, border_padding, plain_center.cols - border_padding,
        plain_center.rows - border_padding)).clone();
}
 ...
 * @return APP_ERROR
 */
APP_ERROR MxpiOpenposePostProcess::ResizeHeatmaps(const std::vector<int> &vision_infos,
    ...
    /*
    std::cout << "origin_height= " << origin_height << std::endl;
    std::cout << "origin_width= " << origin_width << std::endl;
    std::cout << "inputHeight_= " << inputHeight_ << std::endl;
    std::cout << "inputWidth_= " << inputWidth_ << std::endl;
    */
 ...
 * @return APP_ERROR
 */
APP_ERROR MxpiOpenposePostProcess::ExtractKeypoints(const std::vector<cv::Mat> &keypoint_heatmap,
 ...
 */
std::vector<float> MxpiOpenposePostProcess::OneSkeletonScore(const cv::Mat &paf_x, const cv::Mat &paf_y,
...
std::vector<std::vector<PartPair> > *person_list) {
    for (int i = 0; i < K_NUM_BODY_PARTS + 1; i++) {
        // Choose candidate skeletons for each category, there are a total of kNumBodyPart + 1 categories of skeletons
 ...
 * @param image_decoder_visionListSptr - Source MxpiVisionList containing vision data including size of input image
...
const MxpiTensorPackageList src_mxpi_tensor_package, mxpiopenposeproto::MxpiPersonList *dst_mxpi_person_list) {
    // Get tensor
    std::vector<MxBase::TensorBase> tensors = {};
...
APP_ERROR MxpiOpenposePostProcess::Init(std::map<std::string, std::shared_ptr<void>> &config_param_map) {
    ...
    this->parentName_ = *parent_name_prop_sptr.get();
...
APP_ERROR MxpiOpenposePostProcess::Process(std::vector<MxpiBuffer*> &mxpi_buffer) {
    ...
    if (errorInfoPtr != nullptr) {
        ...
        SetMxpiErrorInfo(pluginName_, mxpi_error_info, buffer);
...
APP_ERROR MxpiOpenposePostProcess::SetMxpiErrorInfo(const std::string &plugin_name,
    const MxpiErrorInfo &mxpi_error_info, MxpiBuffer *buffer) {
