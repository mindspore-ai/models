...
# ============================================================================
"""Eval Retinaface_resnet50 on ONNXRUNTIME"""
...
class DetectionEngine:
    ...
    def detect(self, boxes, confs, resize, scale, image_path, priors):
        ...
        dets[:, 2:4] = (dets[:, 2:4].astype(np.int32) - dets[:, 0:2].astype(np.int32)).astype(np.float64) # int
        dets[:, 0:4] = dets[:, 0:4].astype(np.int32).astype(np.float64)                                   # int
        ...
        self.results[event_name][img_name[:-4]] = {'img_path': image_path,
                                                   'bboxes': dets[:, :5].astype(np.float64).tolist()}
    ...
    def _norm_pre_score(self):
        ...
        for event in self.results:
            for name in self.results[event].keys():
                bbox = np.array(self.results[event][name]['bboxes']).astype(np.float64)
                if bbox.shape[0] <= 0:
                    continue
    ...
    def _image_eval(self, predict, gt, keep, iou_thresh, section_num):
        ...
        image_pr = np.zeros((section_num, 2), dtype=np.float64)
    ...
    def get_eval_result(self):
        ...
        for _set in range(len(sets)):
            ...
            count_gt = 0
            pr_curve = np.zeros((section_num, 2), dtype=np.float64)
            for i, _ in enumerate(event_list):
                ...
                for j, _ in enumerate(image_list):
                    predict = np.array(event_predict_dict[str(image_list[j][0][0])]['bboxes']).astype(np.float64)
...
def val():
    ...
    for i, img_name in enumerate(test_dataset):
        ...
        if cfg['device'] == 'GPU':
            ...
            raise ValueError(
                f'Please Select CPU or GPU'
            )
        session = onnxruntime.InferenceSession(cfg['onnx_model'], providers=providers)
        inputs = {session.get_inputs()[0].name: img}
        boxes, confs, _ = session.run(None, inputs)
