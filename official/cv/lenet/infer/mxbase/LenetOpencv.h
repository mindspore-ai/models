...
class LenetOpencv {
 public:
        ...
        APP_ERROR WriteResult(std::vector<MxBase::TensorBase> &outputs, const std::string &imgPath);
        // get infer time
        double GetInferCostMilliSec() const {return inferCostTimeMilliSec;}

 private:
        APP_ERROR SaveResult(const std::string &imgPath, std::vector<std::vector<MxBase::ClassInfo>> &batchClsInfos);
        std::shared_ptr<MxBase::DvppWrapper> dvppWrapper_;
        std::shared_ptr<MxBase::ModelInferenceProcessor> model_;
        std::shared_ptr<MxBase::ClassPostProcessBase> post_;
