...
function air_to_om()
{
    atc --input_format=NCHW --framework=1 --model=$model --output=mobilenetv2_quant --soc_version=Ascend310 &> atc.log
}
...
function infer()
{
    ...
    mkdir result_Files
    mkdir time_Result
    ../ascend310_infer/src/out/main --om_path=./mobilenetv2_quant.om --dataset_path=$data_path --acljson_path=../ascend310_infer/src/acl.json --device_id=$device_id &> infer.log
}
