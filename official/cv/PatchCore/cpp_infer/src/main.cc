...
int main(int argc, char **argv) {
  ...
  for (size_t i = 0; i < size; ++i) {
    ...
    inputs.emplace_back(model_inputs[0].Name(), model_inputs[0].DataType(), model_inputs[0].Shape(), input.Data().get(),
                        input.DataSize());
    ...
    WriteResult(input_files[i], outputs, FLAGS_result_path);
  }
  double average = 0.0;
  int inferCount = 0;
