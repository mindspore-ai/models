...
if [[ $# -lt 5 || $# -gt 6 ]]; then 
    echo "Usage: bash run_infer_cpp.sh [MINDIR_PATH] [DATA_PATH] [ANN_FILE_PATH] [DATASET] [DEVICE_TYPE] [DEVICE_ID]
...
function infer()
{
    ...
    mkdir result_Files
    mkdir time_Result
    ../cpp_infer/out/main --device_type=$device_type --mindir_path=$model --dataset_path=$data_path --device_id=$device_id &> infer.log
