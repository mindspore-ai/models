...
int main(int argc, char* argv[]) {
    if (argc <= 1) {
        LogWarn << "Please input image path, such as './resnet101 image_dir'.";
        return APP_ERR_OK;
    }
    ...
    initParam.modelPath = "../convert/resnet101.om";
    auto resnext101 = std::make_shared<Resnet101ClassifyOpencv>();
    APP_ERROR ret = resnext101->Init(initParam);
    if (ret != APP_ERR_OK) {
        LogError << "Resnet101Classify init failed, ret=" << ret << ".";
        return ret;
    }
    ...
    for (auto &imgFile : imgFilePaths) {
        ret = resnext101->Process(imgFile);
        if (ret != APP_ERR_OK) {
            LogError << "Resnet101Classify process failed, ret=" << ret << ".";
            resnext101->DeInit();
            return ret;
        }
    }
    ...
    double fps = 1000.0 * imgFilePaths.size() / resnext101->GetInferCostMilliSec();
