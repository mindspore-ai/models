...
APP_ERROR se_ResNet50::Init(const InitParam &initParam) {
...
void se_ResNet50::ReadImage(std::string &imgPath, cv::Mat &imageMat) {
    imageMat = cv::imread(imgPath, cv::IMREAD_COLOR);
}

APP_ERROR se_ResNet50::ResizeImage(cv::Mat &srcImageMat, cv::Mat &dstImageMat) {
    static constexpr uint32_t resizeHeight = 292;
    static constexpr uint32_t resizeWidth = 292;

    cv::resize(srcImageMat, dstImageMat, cv::Size(resizeWidth, resizeHeight));
    return APP_ERR_OK;
}

APP_ERROR se_ResNet50::CVMatToTensorBase(cv::Mat &imageMat, MxBase::TensorBase &tensorBase) {
...
APP_ERROR se_ResNet50::CropImage(const cv::Mat &srcImageMat, cv::Mat &dstImageMat) {
    static cv::Rect rectOfImg(16, 16, 256, 256);
    dstImageMat = srcImageMat(rectOfImg).clone();
    return APP_ERR_OK;
}

APP_ERROR se_ResNet50::Inference(std::vector<MxBase::TensorBase> &inputs,
...
APP_ERROR se_ResNet50::PostProcess(const std::vector<MxBase::TensorBase> &inputs,
...
APP_ERROR se_ResNet50::SaveResult(const std::string &imgPath,
...
APP_ERROR se_ResNet50::Process(std::string &imgPath) {
    cv::Mat imageMat;
    ReadImage(imgPath, imageMat);
    cv::Mat resizeImage;
    APP_ERROR ret = ResizeImage(imageMat, resizeImage);
    if (ret != APP_ERR_OK) {
    ...
    cv::Mat cropImage;
    ret = CropImage(resizeImage, cropImage);
    if (ret != APP_ERR_OK) {
