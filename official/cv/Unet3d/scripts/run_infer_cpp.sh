...
if [[ $# -lt 3 || $# -gt 4 ]]; then
    echo "Usage: bash run_infer_cpp.sh [MINDIR_PATH] [NEED_PREPROCESS] [DEVICE_TYPE] [DEVICE_ID]
...
function infer()
{
    ...
    ../cpp_infer/out/main --device_type=$device_type --mindir_path=$model --input0_path=./preprocess_Result/00_data --device_id=$device_id &> infer.log
