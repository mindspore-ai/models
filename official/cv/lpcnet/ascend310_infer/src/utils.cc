...
std::vector<std::string> GetAllFiles(std::string_view dirName) {
    ...
    while ((filename = readdir(dir)) != nullptr) {
        ...
        if (dName == "." || dName == ".." || dName.substr(dName.size() - 4, 4) != ".f32"
    ...
    for (auto &f : res) {
        std::cout << "Audio file: " << f << std::endl;
    }
    return res;
}
...
int WriteResult(const std::string& imageFile, const std::vector<int16_t> &output) {
    std::string homePath = "./result_Files";
    const int INVALID_POINTER = -1;
    const int ERROR = -2;
    ...
    size_t size = fwrite(&output[0], sizeof(int16_t), output.size(), outputFile);
...
mindspore::MSTensor ReadFileToTensor(const std::string &file) {
  ...
  mindspore::MSTensor buffer(file, mindspore::DataType::kNumberTypeFloat32, {static_cast<int64_t>(size / 144),
          static_cast<int64_t>(36)}, nullptr, size);
...
void ReadFileToVector(const std::string &file, std::vector<float> &buffer) {
  if (file.empty()) {
    std::cout << "Pointer file is nullptr" << std::endl;
    return;
  }
