...
APP_ERROR resnext50Classify::DeInit() {
    model_->DeInit();
    post_->DeInit();
...
APP_ERROR resnext50Classify::ReadImage(const std::string &imgPath, cv::Mat &imageMat) {
    imageMat = cv::imread(imgPath, cv::IMREAD_COLOR);
    return APP_ERR_OK;
}

APP_ERROR resnext50Classify::Resize(const cv::Mat &srcImageMat, cv::Mat &dstImageMat) {
...
APP_ERROR resnext50Classify::Crop(const cv::Mat &srcMat, cv::Mat &dstMat) {
    static cv::Rect rectOfImg(16, 16, 224, 224);
    dstMat = srcMat(rectOfImg).clone();
    return APP_ERR_OK;
}

APP_ERROR resnext50Classify::CVMatToTensorBase(const cv::Mat &imageMat, MxBase::TensorBase &tensorBase) {
...
APP_ERROR resnext50Classify::Inference(const std::vector<MxBase::TensorBase> &inputs,
...
APP_ERROR resnext50Classify::PostProcess(const std::vector<MxBase::TensorBase> &inputs,
...
APP_ERROR resnext50Classify::Process(const std::string &imgPath) {
    cv::Mat imageMat;
...
APP_ERROR resnext50Classify::SaveResult(const std::string &imgPath, \
