...
class BRDNet_onnx(nn.Cell):
    """
    args:
        channel: 3 for color, 1 for gray
    """
    def __init__(self, channel):
        super(BRDNet_onnx, self).__init__()

        self.Conv2d_1 = nn.Conv2d(channel, 64, kernel_size=(3, 3), stride=(1, 1), has_bias=True)
        self.BRN_1 = nn.BatchNorm2d(64, eps=1e-3)
        self.layer1 = self.make_layer1(15)
        self.Conv2d_2 = nn.Conv2d(64, channel, kernel_size=(3, 3), stride=(1, 1), has_bias=True)
        self.Conv2d_3 = nn.Conv2d(channel, 64, kernel_size=(3, 3), stride=(1, 1), has_bias=True)
        self.BRN_2 = nn.BatchNorm2d(64, eps=1e-3)
        self.layer2 = self.make_layer2(7)
        self.Conv2d_4 = nn.Conv2d(64, 64, kernel_size=(3, 3), stride=(1, 1), has_bias=True)
        self.BRN_3 = nn.BatchNorm2d(64, eps=1e-3)
        self.layer3 = self.make_layer2(6)
        self.Conv2d_5 = nn.Conv2d(64, 64, kernel_size=(3, 3), stride=(1, 1), has_bias=True)
        self.BRN_4 = nn.BatchNorm2d(64, eps=1e-3)
        self.Conv2d_6 = nn.Conv2d(64, channel, kernel_size=(3, 3), stride=(1, 1), has_bias=True)
        self.Conv2d_7 = nn.Conv2d(channel*2, channel, kernel_size=(3, 3), stride=(1, 1), has_bias=True)
        self.relu = nn.ReLU()
        self.sub = ops.Sub()
    ...
    def make_layer1(self, nums):
        ...
        assert nums > 0
        for _ in range(nums):
            layers.append(nn.Conv2d(64, 64, kernel_size=(3, 3), stride=(1, 1), has_bias=True))
    ...
    def make_layer2(self, nums):
        ...
        assert nums > 0
        for _ in range(nums):
            layers.append(nn.Conv2d(64, 64, kernel_size=(3, 3), has_bias=True))
            layers.append(nn.ReLU())
        return nn.SequentialCell(layers)
