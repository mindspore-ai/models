...
class AutoEncoder(nn.Cell):
    def __init__(self, cfg):
        ...
        encoded_layers.extend([
            nn.Conv2d(cfg.input_channel, cfg.flc, 4, stride=2,
                      weight_init="xavier_uniform", has_bias=True, pad_mode='pad', padding=1),
            nn.LeakyReLU(alpha=0.2),
            nn.Conv2d(cfg.flc, cfg.flc, 4, stride=2,
                      weight_init="xavier_uniform", has_bias=True, pad_mode='pad', padding=1),
            nn.LeakyReLU(alpha=0.2)
            ])
        ...
        encoded_layers.extend([
            nn.Conv2d(cfg.flc, cfg.flc, 3, stride=1,
                      weight_init="xavier_uniform", has_bias=True, pad_mode='pad', padding=1),
            nn.LeakyReLU(alpha=0.2),
            nn.Conv2d(cfg.flc, cfg.flc*2, 4, stride=2,
                      weight_init="xavier_uniform", has_bias=True, pad_mode='pad', padding=1),
            nn.LeakyReLU(alpha=0.2),
            nn.Conv2d(cfg.flc*2, cfg.flc*2, 3, stride=1,
                      weight_init="xavier_uniform", has_bias=True, pad_mode='pad', padding=1),
            nn.LeakyReLU(alpha=0.2),
            nn.Conv2d(cfg.flc * 2, cfg.flc * 4, 4, stride=2,
                      weight_init="xavier_uniform", has_bias=True, pad_mode='pad', padding=1),
            nn.LeakyReLU(alpha=0.2),
            nn.Conv2d(cfg.flc * 4, cfg.flc * 2, 3, stride=1,
                      weight_init="xavier_uniform", has_bias=True, pad_mode='pad', padding=1),
            nn.LeakyReLU(alpha=0.2),
            nn.Conv2d(cfg.flc * 2, cfg.flc, 3, stride=1,
                      weight_init="xavier_uniform", has_bias=True, pad_mode='pad', padding=1),
            nn.LeakyReLU(alpha=0.2),
            nn.Conv2d(cfg.flc, cfg.z_dim, 8, stride=1, pad_mode='valid',
                      weight_init="xavier_uniform", has_bias=True)
        ])
        self.encoded = nn.SequentialCell(encoded_layers)
        decoded_layers = []
        decoded_layers.extend([
            nn.Conv2dTranspose(cfg.z_dim, cfg.flc, 8, stride=1,
                               pad_mode='valid', weight_init="xavier_uniform", has_bias=True),
            nn.LeakyReLU(alpha=0.2),
            nn.Conv2d(cfg.flc, cfg.flc*2, 3, stride=1,
                      weight_init="xavier_uniform", has_bias=True, pad_mode='pad', padding=1),
            nn.LeakyReLU(alpha=0.2),
            nn.Conv2d(cfg.flc*2, cfg.flc*4, 3, stride=1,
                      weight_init="xavier_uniform", has_bias=True, pad_mode='pad', padding=1),
            nn.LeakyReLU(alpha=0.2),
            nn.Conv2dTranspose(cfg.flc*4, cfg.flc * 2, 4, stride=2,
                               weight_init="xavier_uniform", has_bias=True),
            nn.LeakyReLU(alpha=0.2),
            nn.Conv2d(cfg.flc * 2, cfg.flc * 2, 3, stride=1,
                      weight_init="xavier_uniform", has_bias=True, pad_mode='pad', padding=1),
            nn.LeakyReLU(alpha=0.2),
            nn.Conv2dTranspose(cfg.flc*2, cfg.flc, 4, stride=2,
                               weight_init="xavier_uniform", has_bias=True),
            nn.LeakyReLU(alpha=0.2),
            nn.Conv2d(cfg.flc, cfg.flc, 3, stride=1,
                      weight_init="xavier_uniform", has_bias=True, pad_mode='pad', padding=1),
            nn.LeakyReLU(alpha=0.2),
            nn.Conv2dTranspose(cfg.flc, cfg.flc, 4, stride=2,
                               weight_init="xavier_uniform", has_bias=True),
            nn.LeakyReLU(alpha=0.2)
        ])
        if cfg.crop_size == 256:
        ...
        decoded_layers.extend([
            nn.Conv2dTranspose(cfg.flc, cfg.input_channel, 4, stride=2,
