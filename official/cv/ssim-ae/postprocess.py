...
def generate_single(test_img, decoded_img, img_name):
    ...
    rec_img = np.reshape((decoded_img * 255.).astype(np.uint8), test_img.shape)
    ...
    if cfg.image_level:
        ...
        if cfg.grayscale:
            ssim_residual_map = 1 - ssim(test_img, rec_img, win_size=11, data_range=1, gradient=False,
                                         full=True, gaussian_weights=True, sigma=7)[1]
            l1_residual_map = np.abs(test_img / 255. - rec_img / 255.)
        else:
            ssim_residual_map = ssim(test_img, rec_img, win_size=11, data_range=1, gradient=False, full=True,
                                     channel_axis=2, gaussian_weights=True, sigma=7)[1]
            ssim_residual_map = 1 - np.mean(ssim_residual_map, axis=2)
            l1_residual_map = np.mean(np.abs(test_img / 255. - rec_img / 255.), axis=2)
    ssim_residual_map *= cfg.depr_mask
    mask = np.zeros((cfg.mask_size, cfg.mask_size))
    if cfg.ssim_threshold < 0 or cfg.l1_threshold < 0:
        print("[WARNING] Please check the ssim_threshold:{} and l1_threshold:{}".format(
