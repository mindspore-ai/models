...
def train():
    ...
    optimizer = nn.AdamWeightDecay(params=auto_encoder.trainable_params(), learning_rate=cfg.lr,
                                   weight_decay=cfg.decay)
    ckpt_config = CheckpointConfig(save_checkpoint_steps=cfg.dataset_size,
                                   keep_checkpoint_max=1)
    ckpoint_cb = ModelCheckpoint(prefix='ssim_autocoder_' + cfg.dataset,
    ...
    model = Model(net_loss, optimizer=optimizer, loss_scale_manager=loss_scale_manager, amp_level="O3")
    if rank == 0:
