...
void InitGcnParam(InitParam* initParam) {
...
int main(int argc, char* argv[]) {
  ...
  InitParam initParam;
  InitGcnParam(&initParam);
  initParam.modelPath = argv[3];
  NODES = atoi(argv[4]);
  FEATURES = atoi(argv[5]);
  CLASS_NUM = atoi(argv[6]);
  auto gcnBase = std::make_shared<GcnNerBase>();
  APP_ERROR ret = gcnBase->Init(initParam);
  if (ret != APP_ERR_OK) {
    LogError << "Gcnbase init failed, ret=" << ret << ".";
    return ret;
  }
  ...
  for (uint32_t i = 0; i < files.size(); i++) {
    ...
    ret = gcnBase->Process(inferPath, files[i], eval);
    if (ret != APP_ERR_OK) {
