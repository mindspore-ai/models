...
APP_ERROR GcnNerBase::LoadLabels(const std::string &labelPath,
...
APP_ERROR GcnNerBase::Init(const InitParam &initParam) {
...
APP_ERROR GcnNerBase::ReadTensorFromFile(const std::string &file, float *data,
                                         uint32_t size) {
  if (data == NULL) {
...
APP_ERROR GcnNerBase::ReadTensorFromFile(const std::string &file, int *data,
                                         uint32_t size) {
  if (data == NULL) {
    LogError << "input data is invalid.";
    return APP_ERR_COMM_INVALID_POINTER;
...
APP_ERROR GcnNerBase::ReadInputTensor(const std::string &fileName,
                                      uint32_t index,
...
APP_ERROR GcnNerBase::Inference(const std::vector<MxBase::TensorBase> &inputs,
...
APP_ERROR GcnNerBase::PostProcess(std::vector<MxBase::TensorBase> *outputs,
...
APP_ERROR GcnNerBase::CountPredictResult(const std::string &labelFile,
...
void GcnNerBase::GetClunerLabel(
    const std::vector<uint32_t> &argmax,
...
APP_ERROR GcnNerBase::WriteResult(const std::string &fileName,
                                  const std::vector<uint32_t> &argmax) {
...
APP_ERROR GcnNerBase::Process(const std::string &inferPath,
                              const std::string &fileName, bool eval) {
  std::vector<MxBase::TensorBase> inputs = {};
  std::string inputAdjacencyFile = inferPath + "adjacency.txt";
  APP_ERROR ret = ReadInputTensor(inputAdjacencyFile, INPUT_ADJACENCY, &inputs,
                                  NODES * NODES);
  if (ret != APP_ERR_OK) {
  ...
  ret = ReadInputTensor(inputFeatureFile, INPUT_FEATURE, &inputs,
                        NODES * FEATURES);
  if (ret != APP_ERR_OK) {
