...
def TestBGCF(forward_net, num_user, num_item, input_dim, test_graph_dataset):
    ...
    if config.enable_modelarts:
        if config.device_target == "Ascend":
            with open(config.output_path + '/test_inputs.pkl', 'wb') as file:
...
class BGCFLoss(nn.Cell):
    ...
    def construct(self, all_user_embed, all_user_rep, all_pos_item_embed,
        ...
        pos_item_item_distance = self.cast(
            pos_item_item_distance, mstype.float32)

        neg_item_embed = self.cast(neg_item_embed, mstype.float16)
        neg_item_rep = self.concat2((neg_item_rep, neg_item_embed))

        item_rep = self.concat(
            (self.expand_dims(pos_item_rep, 1), neg_item_rep))

        pos_rating = self.reduce_sum(self.multiply(
            self.squeeze(user_rep), pos_item_rep), 1)
        pos_rating = self.expand_dims(pos_rating, 1)
        pos_rating = self.tile(pos_rating, (1, self.neg_item_num))
        pos_rating = self.reshape(
            pos_rating, (self.shape(pos_rating)[0] * self.neg_item_num, 1))
        ...
        neg_rating = self.reshape(
            neg_rating, (self.shape(neg_rating)[0] * self.neg_item_num, 1))
        ...
        reg_loss = self.l2_embed * \
            (self.l2_loss(user_rep) + self.l2_loss(item_rep))

        loss = bpr_loss + reg_loss + self.dist_reg * \
            (user_user_distance + pos_item_item_distance)
        return loss
