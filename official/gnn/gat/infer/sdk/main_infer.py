...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description="gat process")
    ...
    parser.add_argument("--pipeline", type=str, default="../utils/gat_cora.pipeline", help="SDK infer pipeline")
...
if args.dataset == "cora":
    ...
    Feature_dim = 1433
    Class_num = 7
    args.pipeline = "../data/config/gat_cora.pipeline"
elif args.dataset == "citeseer":
    Node_num = 3312
    Feature_dim = 3703
    Class_num = 6
    args.pipeline = "../data/config/gat_citeseer.pipeline"
...
def run():
    ...
    if not send_appsrc_data(0, args.data_feature, stream_name, stream_manager_api, feature_shape, np.float32):
        return

    if not send_appsrc_data(1, args.data_adj, stream_name, stream_manager_api, adj_shape, np.float64):
        return
