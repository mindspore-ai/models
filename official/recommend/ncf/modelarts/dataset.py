...
class NCFDataset:
    ...
    def __init__(self,
        ...
        self._is_row_vector_input = is_row_vector_input
    ...
    def _get_train_item(self, index):
        ...
        if self._is_row_vector_input:
            users = np.reshape(users, (1, self._batch_size))  # (1, _batch_size), int32
            items = np.reshape(items, (1, self._batch_size))  # (1, _batch_size), int32
        else:
    ...
    def _get_eval_item(self, index):
        ...
        if self._is_row_vector_input:
            users = np.reshape(users.flatten(), (1, self._batch_size))  # (1, self._batch_size), int32
            items = np.reshape(items.flatten(), (1, self._batch_size))  # (1, self._batch_size), int32
            duplicate_mask = np.reshape(duplicate_mask.flatten(), (1, self._batch_size))  # (1, self._batch_size), bool
        else:
...
def create_dataset(test_train=True, data_dir='./dataset/', dataset='ml-1m', train_epochs=14, batch_size=256,
                   eval_batch_size=160000, num_neg=4, rank_id=None, rank_size=None, row_vector=False):
    """
    Create NCF dataset.
    """
    ...
    if test_train:
        ...
        dataset = NCFDataset(train_pos_users, train_pos_items, num_users, num_items, batch_size, total_negatives,
                             index_bounds, sorted_train_pos_items, num_neg, is_training=True,
                             is_row_vector_input=row_vector)
