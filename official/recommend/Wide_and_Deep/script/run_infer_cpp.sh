...
if [[ $# -lt 5 || $# -gt 6 ]]; then
    echo "Usage: bash run_infer_cpp.sh [MINDIR_PATH] [DATASET_PATH] [DATA_TYPE] [NEED_PREPROCESS] [DEVICE_TYPE] [DEVICE_ID]
...
function preprocess_data()
{
    ...
    python ../preprocess.py --dataset_path=$dataset_path --dataset_type=$dataset_type --result_path=./preprocess_Result/
}
...
function infer()
{
    ...
    ../cpp_infer/out/main --device_type=$device_type --mindir_path=$model --input0_path=./preprocess_Result/00_feat_ids --input1_path=./preprocess_Result/01_feat_vals --input2_path=./preprocess_Result/02_labels --device_id=$device_id &> infer.log

}

function cal_acc()
{
    python ../postprocess.py --result_path=./result_Files --label_path=./preprocess_Result/02_labels &> acc.log
}
