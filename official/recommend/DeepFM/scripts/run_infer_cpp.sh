...
if [[ $# -lt 4 || $# -gt 5 ]]; then
    echo "Usage: bash run_infer_cpp.sh [MINDIR_PATH] [DATASET_PATH] [NEED_PREPROCESS] [DEVICE_TYPE] [DEVICE_ID]
...
function infer()
{
    ...
    ../cpp_infer/out/main --device_type=$device_type --mindir_path=$model --input0_path=./preprocess_Result/00_batch_ids --input1_path=./preprocess_Result/01_batch_wts --input2_path=./preprocess_Result/02_labels --device_id=$device_id &> infer.log
