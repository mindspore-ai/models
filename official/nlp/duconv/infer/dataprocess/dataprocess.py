...
class DataProcessor:
    ...
    def file_based_convert_examples_to_features(self, input_file, output_path):
        ...
        for example in tqdm(examples):
            ...
            context_id.append(np.array(record.context_ids, dtype=np.int64))
            context_pos_id.append(np.array(record.context_pos_ids, dtype=np.int64))
            context_segment_id.append(np.array(record.segment_ids, dtype=np.int64))
            labels_list.append(np.array([record.label_ids], dtype=np.int64))

            if 'kn' in self.task_name:
                kn_id.append(np.array(record.kn_ids, dtype=np.int64))
                kn_seq_length.append(np.array(record.kn_seq_length, dtype=np.int64))

        np.savetxt(output_path + '/context_id.txt', np.array(context_id), fmt='%i', delimiter=' ')
        np.savetxt(output_path + '/context_pos_id.txt', np.array(context_pos_id), fmt='%i', delimiter=' ')
        np.savetxt(output_path + '/context_segment_id.txt', np.array(context_segment_id), fmt='%i', delimiter=' ')
        np.savetxt(output_path + '/labels_list.txt', np.array(labels_list), fmt='%i', delimiter=' ')
        if 'kn' in self.task_name:
            np.savetxt(output_path + '/kn_id.txt', np.array(kn_id), fmt='%i', delimiter=' ')
            np.savetxt(output_path + '/kn_seq_length.txt', np.array(kn_seq_length), fmt='%i', delimiter=' ')
...
if __name__ == "__main__":
    ...
    processor.file_based_convert_examples_to_features(args_opt.input_file, args_opt.output_path)
