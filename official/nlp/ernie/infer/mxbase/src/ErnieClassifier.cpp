...
const uint32_t MAX_LENGTH = 256;
const uint32_t CLASS_NUM = 2;


APP_ERROR ErnieClassifier::Init(const InitParam &initParam) {
...
APP_ERROR ErnieClassifier::DeInit() {
    model_->DeInit();
...
APP_ERROR ErnieClassifier::ReadTensorFromFile(const std::string &file, uint32_t *data) {
    if (data == NULL) {
...
APP_ERROR ErnieClassifier::ReadInputTensor(const std::string &fileName, uint32_t index,
...
APP_ERROR ErnieClassifier::Inference(const std::vector<MxBase::TensorBase> &inputs,
...
APP_ERROR ErnieClassifier::PostProcess(std::vector<MxBase::TensorBase> *outputs, std::vector<uint32_t> *argmax) {
...
APP_ERROR ErnieClassifier::CountPredictResult(const std::string &labelFile, const std::vector<uint32_t> &argmax) {
    uint32_t data[1] = {0};
...
APP_ERROR ErnieClassifier::WriteResult(const std::string &fileName, const std::vector<uint32_t> &argmax) {
...
APP_ERROR ErnieClassifier::Process(const std::string &inferPath, const std::string &fileName, bool eval) {
    std::vector<MxBase::TensorBase> inputs = {};
