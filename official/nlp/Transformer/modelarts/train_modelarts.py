...
set_seed(1)
parser = argparse.ArgumentParser(description='transformer', formatter_class=argparse.ArgumentDefaultsHelpFormatter)
...
def save_ckpt_to_air(save_ckpt_path, path):
    ...
    parameter_dict = load_weights(path)
    ...
    export(tfm_model, source_ids, source_mask, file_name=save_ckpt_path+'transformer', file_format="AIR")
...
def modelarts_pre_process():
    ...
    args.data_url = os.path.join(args.data_url, 'ende-l128-mindrecord')
...
def run_transformer_train():
    ...
    model.train(args.epoch, dataset, callbacks=callbacks, dataset_sink_mode=False)
    print("============== End Training ==============")
    path = os.path.join(save_ckpt_path, 'transformer'+'-'+str(args.epoch)+'_'+str(dataset.get_dataset_size())+'.ckpt')
