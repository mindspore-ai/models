...
- 在`default_config_large.yaml`中设置选项，包括loss_scale、学习率和网络超参数。点击[这里](https://www.mindspore.cn/tutorials/zh-CN/r1.8/advanced/dataset.html)查看更多数据集信息。
...
| 脚本                        | [Transformer 脚本](https://gitee.com/mindspore/models/tree/r1.8/official/nlp/transformer) |
| 模型版本            | large                                                                                   |large|
...
- **Q: 为什么程序运行报错＂For 'Add', x.shape and y.shape need to broadcast.＂，或者其他算子Shape不匹配的问题？**

  **A**: 因为现有的参数是根据readme中提供的数据集配置的，用户更换数据集后，需要根据参数定义重新配置相关参数。
