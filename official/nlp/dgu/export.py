...
parser.add_argument("--file_name", type=str, default="Bert", help="bert output mindir name.")
...
if __name__ == "__main__":
    if args.task_name == 'udc':
        net = BertCLS(bert_net_udc_cfg, False, num_labels=args.number_labels)
        load_checkpoint(args.ckpt_file, net=net)
        net.set_train(False)
        input_ids = Tensor(np.zeros([args.batch_size, bert_net_udc_cfg.seq_length]), mstype.int32)
        input_mask = Tensor(np.zeros([args.batch_size, bert_net_udc_cfg.seq_length]), mstype.int32)
        token_type_id = Tensor(np.zeros([args.batch_size, bert_net_udc_cfg.seq_length]), mstype.int32)
        label_ids = Tensor(np.zeros([args.batch_size, bert_net_udc_cfg.seq_length]), mstype.int32)
    else:
