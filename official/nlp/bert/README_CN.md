...
- **Q: 为什么在bert boost模式下图形编译时间变长，内存变大？**

  **A**：因为boost模式通过自定义数据集的方式来将数据集中的不同句长数据进行分桶，以此加速网络训练过程，该操作会使能更多图优化来加速性能，因此编译时间会有所上升，同时，自定义数据时使用了额外的数据队列来存储分桶数据，因此内存会上涨，用户也可参考[自定义数据集](https://www.mindspore.cn/tutorials/zh-CN/master/advanced/dataset/custom.html)，根据自身需求来自定义数据。
