...
def train_single(platform: "Ascend"):
    ...
    pre_train_dataset = [os.path.join(config.data_path, config.pre_train_dataset, filename) for filename in
                         os.listdir(os.path.join(config.data_path, config.pre_train_dataset))]
    fine_tune_dataset = [os.path.join(config.data_path, config.fine_tune_dataset, filename) for filename in
                         os.listdir(os.path.join(config.data_path, config.fine_tune_dataset))]
    test_dataset = [os.path.join(config.data_path, config.test_dataset, filename) for filename in
                    os.listdir(os.path.join(config.data_path, config.test_dataset))]

    pre_train_dataset = load_dataset(data_files=pre_train_dataset,
                                     batch_size=config.batch_size,
                                     epoch_count=1,
                                     sink_mode=config.dataset_sink_mode,
                                     sink_step=config.dataset_sink_step) if config.pre_train_dataset else None
    fine_tune_dataset = load_dataset(data_files=fine_tune_dataset,
                                     batch_size=config.batch_size,
                                     epoch_count=1,
                                     sink_mode=config.dataset_sink_mode,
                                     sink_step=config.dataset_sink_step) if config.fine_tune_dataset else None
    test_dataset = load_dataset(data_files=test_dataset,
                                batch_size=config.batch_size,
                                epoch_count=1,
                                sink_mode=config.dataset_sink_mode,
...
if __name__ == '__main__':
    ...
    config.vocab = os.path.join(config.data_path, config.vocab)
    ...
    config.batch_size = 1
    ckpt_dir = os.path.join(config.output_path, config.checkpoint_path)
    model_dir = os.path.join(ckpt_dir, os.listdir(ckpt_dir)[0])
    files = os.listdir(model_dir)
    for file in files:
        if '.ckpt' not in file:
            files.remove(file)
    config.checkpoint_file_path = os.path.join(model_dir, files[-1])
    config.file_name = os.path.join(config.output[:-5], config.checkpoint_file_path)
    tfm_model = TransformerInferModel(config=config, use_one_hot_embeddings=False)
    tfm_model.init_parameters_data()
    export_params = tfm_model.trainable_params()
    export_weights = load_infer_weights(config)

    for export_param in export_params:
        export_value = export_param.data
        export_name = export_param.name
        if export_name not in export_weights:
            raise ValueError(f'{export_name} is not found in weights.')
        export_weights_name = export_name
        if isinstance(export_value, Tensor):
            if export_weights_name in export_weights:
                assert export_weights_name in export_weights
                export_param.set_data(Tensor(export_weights[export_weights_name], mstype.float32))
            else:
                raise ValueError(f'{export_weights_name} is not found in checkpoint')
        else:
            raise TypeError(f'Type of {export_weights_name} is not Tensor')
    ...
    export(tfm_model, source_ids, source_mask, file_name=config.file_name, file_format="AIR")
