...
APP_ERROR MassNerBase::Init(const InitParam &initParam) {
...
APP_ERROR MassNerBase::ReadTensorFromFile(const std::string &file, int32_t *data,
                                         uint32_t size) {
  if (data == NULL) {
...
APP_ERROR MassNerBase::ReadInputTensor(int32_t *data, const std::string &fileName,
                                      uint32_t index,
...
APP_ERROR MassNerBase::Inference(const std::vector<MxBase::TensorBase> &inputs,
...
APP_ERROR MassNerBase::PostProcess(std::vector<MxBase::TensorBase> *outputs,
  ...
  for (uint32_t i = 0; i < length; i++) {
    for (uint32_t j = 0; j < FEATURES + 1; j++) {
      int32_t value = *(reinterpret_cast<int32_t *>(data) + i * FEATURES + j);
...
APP_ERROR MassNerBase::WriteResult(const std::string &fileName,
  ...
  tfile << predict[0];
  for (uint32_t i = 1; i < FEATURES + 1; i++) {
    tfile << " " << predict[i];
  }
  tfile << std::endl;
...
APP_ERROR MassNerBase::Process(const std::string &inferPath,
                              const std::string &fileName) {
  std::string inputAdjacencyFile = inferPath + "source_ids.txt";
  std::string inputFeatureFile = inferPath + "mask_ids.txt";
  ...
  while (std::getline(fp1, line1), std::getline(fp2, line2)) {
    int32_t *data1 = new int32_t[FEATURES];
    int32_t *data2 = new int32_t[FEATURES];
    std::vector<MxBase::TensorBase> inputs = {};
    std::vector<MxBase::TensorBase> outputs = {};
    std::string number1;
    ...
    ret = ReadInputTensor(data1, inputAdjacencyFile, INPUT_ADJACENCY, &inputs,
                                  FEATURES);
    if (ret != APP_ERR_OK) {
