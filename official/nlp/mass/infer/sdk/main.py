...
def parse_args():
    ...
    parser.add_argument("--pipeline", type=str, default="../data/config/mass.pipeline", help="SDK infer pipeline")
...
if __name__ == '__main__':
    ...
    mask_ids_path = os.path.join(args.data_dir, "mask_ids.txt")
    source_ids = np.loadtxt(source_ids_path, dtype=np.int32).reshape(-1, 64)
    mask_ids = np.loadtxt(mask_ids_path, dtype=np.int32).reshape(-1, 64)
    stream_name = b'mass'
