...
from src import check_reconstruction, read_node_label, check_multi_label_classification
    ...
    if config['reconstruction']['check']:
        if args.dataset == 'WIKI':
            ...
            check_reconstruction(embeddings, dataset.get_graph(), idx2node,
                                 config['reconstruction']['k_query'])
    if config['classify']['check']:
        labels = read_node_label(label_path, node2idx)
        check_multi_label_classification(embeddings, labels, config['classify']['tr_frac'])
