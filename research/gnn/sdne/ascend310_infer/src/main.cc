...
int main(int argc, char **argv) {
  ...
  if (dataset_name == "WIKI") {
    auto k_query = std::vector({1, 10, 20, 100, 200, 1000, 2000, 6000, 8000, 10000});
    auto precisions = CheckReconstruction2(graph, reconstructions, k_query);
    for (unsigned int i = 0; i < k_query.size(); i++) {
      std::cout << "precision@" << k_query[i] << " : " << precisions[i] << std::endl;
    }
    std::cout << "MAP: " << precisions.back() << std::endl;
  } else {
    auto k_query = std::vector({10, 100});
    auto precisions = CheckReconstruction1(graph, embeddings, k_query);
    for (unsigned int i = 0; i < k_query.size(); i++) {
      std::cout << "precision@" << k_query[i] << " : " << precisions[i] << std::endl;
    }
  }
  WriteResult(embeddings);
