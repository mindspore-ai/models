...
class DgcnBase {
 public:
     ...
     APP_ERROR Process(const std::string &inferPath, const std::string &fileName, const std::string &dataname);
 ...
 protected:
     ...
     APP_ERROR ReadInputTensor(const std::string &fileName, uint32_t index, std::vector<MxBase::TensorBase> *inputs,
                                        uint32_t size, MxBase::TensorDataType type, const std::string &dataname);
