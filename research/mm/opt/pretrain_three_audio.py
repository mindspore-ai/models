...
def init_model(opts):
    ...
    return (local_rank, rank_id, device_id, bucket_dir, callback_size, strategy_ckpt_save_file, device_num,
            net_with_grads, net_with_loss, new_epoch, ds, optimizer)
...
def main(opts):
    ...
    (local_rank, rank_id, device_id, bucket_dir, callback_size, strategy_ckpt_save_file, device_num,
     net_with_grads, net_with_loss, new_epoch, ds, optimizer) = init_model(opts)
    ...
    if opts.load_ckpt:
        ...
        if rank_dirs:
            ...
            if ckpt_file is not None:
                # LOGGER.info(f"rank_%d: start loading {ckpt_file}." % (rank_id))
