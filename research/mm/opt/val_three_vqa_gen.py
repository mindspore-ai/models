...
from src.model_mindspore.vqa import UniterThreeForPretrainingForVQAgenfinetuneEval
from src.data import data_column, get_batch_data
from src.data.dataset import create_dataset_metaloader
...
def main(opts):
    ...
    ds, metaloader = create_dataset_metaloader(opts, device_num=device_num, rank=rank, column_name=data_column,
                                               token_size=opts.train_batch_size, full_batch=opts.full_batch,
                                               is_train=True, batch_size=opts.val_batch_size)
    dataset_size = ds.get_dataset_size()
    print("=====dataset size: ", dataset_size, flush=True)

    net_without_loss = UniterThreeForPretrainingForVQAgenfinetuneEval(opts.model_config, img_dim=opts.img_dim,
