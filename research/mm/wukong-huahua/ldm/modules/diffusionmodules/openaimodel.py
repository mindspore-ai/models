...
class ResBlock(nn.Cell):
    ...
    def __init__(
        ...
        if self.out_channels == channels:
            ...
            self.skip_connection = conv_nd(
                dims, channels, self.out_channels, 1, has_bias=True, pad_mode='pad').to_float(self.dtype)
...
class UNetModel(nn.Cell):
    ...
    def __init__(
        ...
        if use_spatial_transformer:
            assert context_dim is not None, 'You forgot to include the dimension of cross-attention conditioning'

        if context_dim is not None:
            assert use_spatial_transformer, 'You forgot to use the spatial transformer for cross-attention conditioning'
