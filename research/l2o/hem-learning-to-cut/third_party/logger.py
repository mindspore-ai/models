...
import mindspore
import numpy as np
from third_party.tabulate import tabulate
...
class MyEncoder(json.JSONEncoder):
    def default_(self, o):
        ...
        if isinstance(o, type):
            return_dict = {'$class': o.__module__ + "." + o.__name__}
        ...
        # else:
        #     return_dict = json.JSONEncoder.default(self, o)
        return return_dict
...
class Logger():
    def __init__(self):
        ...
        self._text_outputs = []
        self.tabular_outputs = []

        self._text_fds = {}
        self.tabular_fds = {}
        self.tabular_header_written = set()
    ...
    def add_output(self, file_name, arr, fds, mode='a'):
    ...
    def add_text_output(self, file_name):
        self.add_output(file_name, self._text_outputs, self._text_fds,
                        mode='a')
    ...
    def add_tabular_output(self, file_name, relative_to_snapshot_dir=False):
        ...
        self.add_output(file_name, self.tabular_outputs, self.tabular_fds,
                        mode='w')

    def remove_tabular_output(self, file_name, relative_to_snapshot_dir=False):
        if relative_to_snapshot_dir:
            file_name = osp.join(self._snapshot_dir, file_name)
        if self.tabular_fds[file_name] in self.tabular_header_written:
            self.tabular_header_written.remove(self.tabular_fds[file_name])
        self._remove_output(
            file_name, self.tabular_outputs, self.tabular_fds)

    def set_snapshot_dir(self, dir_name):
        self._snapshot_dir = dir_name
        self.tb_logger = SummaryWriter(self._snapshot_dir)
    ...
    def dump_tabular(self, *args, **kwargs):
        ...
        if self._tabular:
            ...
            for tabular_fd in list(self.tabular_fds.values()):
                writer = csv.DictWriter(tabular_fd,
                                        fieldnames=list(tabular_dict.keys()))
                if wh or (
                        wh is None and tabular_fd not in self.tabular_header_written):
                    writer.writeheader()
                    self.tabular_header_written.add(tabular_fd)
    ...
    def save_itr_params(self, itr, params):
        if self._snapshot_dir:
            if self._snapshot_mode == 'all':
                ...
                mindspore.save(params, file_name)
    ...
    def save_npy(self, itr, data):
        file_name = osp.join(self._snapshot_dir, 'itr_%d.npy' % itr)
    ...
    def save_replay_pool(self, dataset):
        """
        dataset from simple pool is a dict
        """
