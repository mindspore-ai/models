...
#   - or a list of elements not to be displayed
#     if the table has column headers.
...
def _latex_line_begin_tabular(colwidths, colaligns):
    ...
    return "\\begin{tabular}{" + tabular_columns_fmt + r"}\n\hline"
...
def _isint(string):
    ...
    return isinstance(string, int) or \
        isinstance(string, _binary_type, _text_type) and \
...
def _type(string, has_invisible=True):
    ...
    if has_invisible and \
            isinstance(string, _text_type, _binary_type):
        string = _strip_invisible(string)
    return_type = None
    if string is None:
        return_type = _none_type
    elif hasattr(string, "isoformat"):  # datetime.datetime, date, and time
        return_type = _text_type
    elif _isint(string):
        return_type = int
    elif _isnumber(string):
        return_type = float
    elif isinstance(string, _binary_type):
        return_type = _binary_type
    else:
        return_type = _text_type
    return return_type
...
def _strip_invisible(s):
    ...
    if isinstance(s, _text_type):
        return_s = re.sub(_invisible_codes, "", s)
    else:  # a bytestring
        return_s = re.sub(_invisible_codes_bytes, "", s)
    return return_s
...
def _visible_width(s):
    ...
    if isinstance(s, _text_type, _binary_type):
        return_wid = len(_strip_invisible(s))
    else:
        return_wid = len(_text_type(s))
    return return_wid
...
def _format(val, valtype, floatfmt, missingval=""):
    ...
    if valtype in [int, _text_type]:
        return_s = "{0}".format(val)
    elif valtype is _binary_type:
        return_s = _text_type(val, "ascii")
    elif valtype is float:
        return_s = format(float(val), floatfmt)
    else:
        return_s = "{0}".format(val)
    return return_s
...
def _align_header(header, alignment, width):
    return_h = None
    if alignment == "left":
        return_h = _padright(width, header)
    elif alignment == "center":
        return_h = _padboth(width, header)
    elif not alignment:
        return_h = "{0}".format(header)
    else:
        return_h = _padleft(width, header)
    return return_h
...
def _build_row(padded_cells, colwidths, colaligns, rowfmt):
    ...
    if hasattr(rowfmt, "__call__"):
        return_fmt = rowfmt(padded_cells, colwidths, colaligns)
    else:
        return_fmt = _build_simple_row(padded_cells, rowfmt)
    return return_fmt
...
def _build_line(colwidths, colaligns, linefmt):
    ...
    if hasattr(linefmt, "__call__"):
        return_fmt = linefmt(colwidths, colaligns)
    else:
        begin, fill, sep, end = linefmt
        cells = [fill * w for w in colwidths]
        return_fmt = _build_simple_row(cells, (begin, sep, end))
