...
if __name__ == '__main__':
    ...
    parser.add_argument('--class_num', type=int, default=config.class_num, help='The number of class')
    parser.add_argument('--batch_size', type=int, default=config.batch_size, help='The batch size')
    parser.add_argument('--loss_scale', type=int, default=config.loss_scale, help='The loss scale of model')
    parser.add_argument('--epoch_size', type=int, default=config.epoch_size, help='The epoch size')
    ...
    loss = CrossEntropySmooth(smooth_factor=config.label_smooth_factor, num_classes=args_opt.class_num)
    print("loss init")
    ...
    if config.opt == 'sgd':
        optimizer = SGD(net.trainable_params(), learning_rate=lr, momentum=config.momentum,
                        weight_decay=config.weight_decay, loss_scale=args_opt.loss_scale)
    elif config.opt == 'rmsprop':
        optimizer = RMSProp(net.trainable_params(), learning_rate=lr, decay=0.9, weight_decay=config.weight_decay,
                            momentum=config.momentum, epsilon=config.opt_eps, loss_scale=args_opt.loss_scale)
    print("lr init")
    # define model
    ...
    if config.save_checkpoint and (device_num == 1 or device_id == 0):
        ...
        if args_opt.run_modelarts:
            ckpt_cb = ModelCheckpoint(f"Mnasnet-rank{device_id}", directory=local_train_url, config=config_ck)
        else:
    ...
    for f in os.listdir(local_train_url):
        ...
        if f.endswith('.ckpt'):
            ...
            export(net, input_array, file_name=local_train_url+'/mnasnet', file_format='AIR')
    if args_opt.run_modelarts and config.save_checkpoint and (device_num == 1 or device_id == 0):
        mox.file.copy_parallel(local_train_url, args_opt.train_url)
