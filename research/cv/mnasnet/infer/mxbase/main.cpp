...
int main(int argc, char* argv[]) {
    if (argc <= 1) {
        LogWarn << "Please input image path, such as './mnasnet image_dir'.";
        return APP_ERR_OK;
    }
    ...
    initParam.modelPath = "../data/models/mnasnet.om";
    auto mnasnet = std::make_shared<mnasnetClassifyOpencv>();
    APP_ERROR ret = mnasnet->Init(initParam);
    if (ret != APP_ERR_OK) {
        LogError << "mnasnetClassify init failed, ret=" << ret << ".";
        return ret;
    }
    ...
    for (auto &imgFile : imgFilePaths) {
        ret = mnasnet->Process(imgFile);
        if (ret != APP_ERR_OK) {
            LogError << "mnasnetClassify process failed, ret=" << ret << ".";
            mnasnet->DeInit();
            return ret;
        }
    }
    ...
    double fps = 1000.0 * imgFilePaths.size() / mnasnet->GetInferCostMilliSec();
