...
class faceattribute {
 public:
     ...
     APP_ERROR PostProcess(std::vector<MxBase::TensorBase> &tensors, int classNum[]);
     APP_ERROR Process(const std::string &imgPath, int ground_class[]);
     // get infer time
     double GetInferCostMilliSec() const {return inferCostTimeMilliSec;}
     cv::Mat BGRToRGB(cv::Mat img);

 private:
        APP_ERROR Compare(int ground_class[], int class_predict[]);
