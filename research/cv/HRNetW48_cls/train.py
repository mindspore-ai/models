...
def main():
    ...
    lr = get_linear_lr(config.lr, config.total_epoch, step_size,
                       config.lr_init, config.lr_end, config.warmup_epochs)
    ...
    if device_id == 0 and config.save_ckpt:
        ckpt_config = CheckpointConfig(save_checkpoint_steps=step_size*config.save_checkpoint_epochs,
                                       keep_checkpoint_max=config.keep_checkpoint_max)
        ckpt_cb = ModelCheckpoint(prefix=f"hrnetw48-{config.dataset}",
                                  directory=config.train_path,
                                  config=ckpt_config)
        cb.append(ckpt_cb)
    if config.eval_data_path:
        eval_cb = EvalCallBack(model, eval_data, config.eval_interval)
        cb.append(eval_cb)
    # begin train
