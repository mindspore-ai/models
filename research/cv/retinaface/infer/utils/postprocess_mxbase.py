...
def mxbase_eval(args_opt):
    testset_folder = args_opt.data_path
    bin_folder = args_opt.bin_path
    ...
    for i, img_name in enumerate(test_dataset):
        ...
        # read bin
        _bin_path = os.path.join(bin_folder, img_name)
        bin_path = _bin_path.replace("jpg", "bin")
        print("bin_path:", bin_path)

        f = open(bin_path, "rb")
        strb = f.read()
        data = np.frombuffer(strb, dtype='<f4')
        boxes = data[0: 964656].reshape((1, 241164, 4))
        confs = data[964656:].reshape((1, 241164, 2))

        if val_origin_size:
            resize = 1

        scale = np.array([1056, 5568, 1056, 5568], dtype=img.dtype)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Retinaface Mxbase eval')
    # Datasets
    parser.add_argument('--data_path', default='../data/input/val_images/', type=str,
                        help='data path')
    parser.add_argument('--bin_path', default='../mxbase/mxbase_out/', type=str,
                        help='infer_result path')
    parser.add_argument('--result_url', default='../mxbase/mxbase_out/', type=str,
                        help='result path')
    parser.add_argument('--ground_truth', default='../data/input/ground_truth/', type=str,
