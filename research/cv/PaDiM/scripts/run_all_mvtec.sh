...
then
    ...
    echo "bash run_all_mvtec.sh DATA_PATH PRETRAINED_PATH SAVE_PATH DEVICE_ID"
    echo "For example: bash run_all_mvtec.sh /path/dataset /path/pretrained_path /path/save_path 0"
...
for value in "${arr[@]}"
do
  bash run_train.sh  $DATA_PATH $CKPT_APTH $SAVE_PATH $value $4
  bash run_eval.sh  $DATA_PATH $CKPT_APTH $SAVE_PATH $value $4
done
