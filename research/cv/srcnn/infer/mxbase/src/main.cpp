...
int main(int argc, char* argv[]) {
    ...
    auto srcnn = std::make_shared<Srcnn>();
    APP_ERROR ret = srcnn->Init(initParam);
    if (ret != APP_ERR_OK) {
        LogError << "Srcnn init failed, ret=" << ret << ".";
        return ret;
    }
    ...
    for (uint32_t i = 0; i < files.size(); i++) {
        ...
        ret = srcnn->Process(input_data_path, files[i]);
        if (ret != APP_ERR_OK) {
            LogError << "Srcnn process failed, ret=" << ret << ".";
            srcnn->DeInit();
            return ret;
        }
    }
