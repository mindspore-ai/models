...
if [[ $# -lt 4 || $# -gt 5 ]]; then
    echo "Usage: bash run_infer_cpp.sh [MINDIR_PATH] [DATA_PATH] [ANNO_PATH] [DEVICE_TYPE] [DEVICE_ID]
...
function infer()
{
    ...
    mkdir result_Files
    mkdir time_Result
    ../cpp_infer/out/main --device_type=$device_type --mindir_path=$model --dataset_path=$data_path --cpu_dvpp='CPU' --device_id=$device_id --image_height=320 --image_width=320 &> infer.log
}

function cal_acc()
{
    python ../postprocess.py --result_path=./result_Files --img_path=$data_path --anno_path=$anno --drop &> acc.log &
}
