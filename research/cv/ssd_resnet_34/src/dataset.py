...
from .box_utils import ssd_bboxes_encode
...
def get_image_id_from_filename(filename, id_iter):
    """Get imageID from file name if file name is int, else return id_iter."""
...
def preprocess_fn(img_id, input_image, box, is_training):
    """Preprocess function for dataset."""
    cv2.setNumThreads(2)

    def _infer_data(image, input_shape):
        """Preprocess images for the inference."""
    ...
    def _data_aug(image, box, is_training, image_size=(300, 300)):
        ...
        image_cropped, box_cropped = random_sample_crop(image, box_float)
        ih, iw, _ = image_cropped.shape
        ...
        flip = _rand() < .5
        if flip:
            image_resized = cv2.flip(image_resized, 1, dst=None)

        # When the channels of image is 1
        if len(image.shape) == 2:
            image_resized = np.expand_dims(image_resized, axis=-1)
            image_resized = np.concatenate([image_resized, image_resized, image_resized], axis=-1)

        box_cropped[:, [0, 2]] = box_cropped[:, [0, 2]] / ih
        box_cropped[:, [1, 3]] = box_cropped[:, [1, 3]] / iw

        if flip:
            box_cropped[:, [1, 3]] = 1 - box_cropped[:, [3, 1]]

        box_cropped, label, num_match = ssd_bboxes_encode(box_cropped)
        return image_resized, box_cropped, label, num_match

    return _data_aug(input_image, box, is_training, image_size=config.img_shape)
...
def create_voc_label(is_training):
    ...
    if not os.path.isdir(voc_dir):
        raise ValueError("Cannot find {} dataset path.".format(sub_dir))
    ...
    for anno_file in os.listdir(anno_dir):
        ...
        if not anno_file.endswith('xml'):
            continue
        tree = ElementTree.parse(os.path.join(anno_dir, anno_file))
        root_node = tree.getroot()
        file_name = root_node.find('filename').text
        img_id = get_image_id_from_filename(file_name, id_iter)
        id_iter += 1
...
def create_mindrecord(
    ...
    if rank == 0 and not os.path.exists(mindrecord_file):
        print("The mindrecord dataset is starting to be created!")
