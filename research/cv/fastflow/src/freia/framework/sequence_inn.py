...
class SequenceINN(InvertibleModule):
    ...
    def __init__(self, *dims: int, force_tuple_output=False):
        super(SequenceINN, self).__init__([dims])

        self.shapes = [tuple(dims)]
        self.conditions = []
        self.module_list = nn.CellList()
        self.reversed = ops.ReverseV2(axis=[0])
    ...
    def construct(
            self, x_or_z, c: Iterable[Tensor] = None, rev: bool = False, jac: bool = True
        ) -> Tuple[Tensor, Tensor]:
        ...
        if rev:
            iterator = self.reversed(iterator)
