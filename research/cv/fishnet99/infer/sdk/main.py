...
def parse_args():
    ...
    parser.add_argument('--inputdata_dir', required=True, type=str, help="imagenet2012_val data path.",
                        default="../data/input/imagenet_val/")
    parser.add_argument('--save_path', required=True, type=str, help="save result path", default='../data/sdk_result/')
...
if __name__ == '__main__':
    ...
    with open("../data/config/fishnet99.pipeline", 'rb') as f:
        pipelineStr = f.read()
    ...
    inputdata_dir = args.inputdata_dir
    result_dir_name = args.save_path
    file_list = os.listdir(inputdata_dir)

    if not os.path.exists(result_dir_name):
        os.makedirs(result_dir_name)

    for file_name in file_list:
        file_path = os.path.join(inputdata_dir, file_name)
        ...
        if load_dict.get('MxpiClass') is None:
            with open(result_dir_name + "/" + file_name[:-5] + '.txt', 'w') as f_write:
                f_write.write("")
            continue
        res_vec = load_dict.get('MxpiClass')

        with open(result_dir_name + "/" + file_name[:-5] + '_1.txt', 'w') as f_write:
