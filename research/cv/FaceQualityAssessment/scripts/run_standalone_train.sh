...
if [ $# != 2 ] && [ $# != 3 ]
then
    echo "Usage: bash run_standalone_train.sh [TRAIN_LABEL_FILE] [USE_DEVICE_ID] [PRETRAINED_BACKBONE]"
    echo "   or: bash run_standalone_train.sh [TRAIN_LABEL_FILE] [USE_DEVICE_ID]"
    exit 1
fi
