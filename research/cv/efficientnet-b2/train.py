...
if __name__ == '__main__':
    ...
    if config.save_checkpoint and (device_num == 1 or device_id == 0):
        ...
        if args_opt.run_modelarts:
            ckpt_cb = ModelCheckpoint(f"Efficientnet_b2-rank{device_id}", directory=local_train_url, config=config_ck)
        else:
            save_ckpt_path = os.path.join(config.save_checkpoint_path, 'model_' + str(device_id) + '/')
            ckpt_cb = ModelCheckpoint(f"Efficientnet_b2-rank{device_id}", directory=save_ckpt_path, config=config_ck)
        cb += [ckpt_cb]
