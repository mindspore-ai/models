...
def create_dataset(dataset_path, do_train, batch_size=16, device_num=1, rank=0):
    ...
    if device_num == 1:
        if do_train:
            ...
            ds = de.ImageFolderDataset(dataset_path, num_parallel_workers=40, shuffle=False)
    else:
...
def shear_x(level):
    ...
    return C2.RandomChoice([C.RandomAffine(degrees=0, shear=(-v, -v)), C.RandomAffine(degrees=0, shear=(v, v))])

def shear_y(level):
    v = float_parameter(level, 0.3)
    return C2.RandomChoice([C.RandomAffine(degrees=0, shear=(0, 0, -v, -v)),
                            C.RandomAffine(degrees=0, shear=(0, 0, v, v))])

def translate_x(level):
    v = float_parameter(level, 150 / 331)
    return C2.RandomChoice([C.RandomAffine(degrees=0, translate=(-v, -v)), C.RandomAffine(degrees=0, translate=(v, v))])

def translate_y(level):
    v = float_parameter(level, 150 / 331)
    return C2.RandomChoice([C.RandomAffine(degrees=0, translate=(0, 0, -v, -v)),
...
def rotate_impl(level):
    ...
    return C2.RandomChoice([C.RandomRotation(degrees=(-v, -v)), C.RandomRotation(degrees=(v, v))])
