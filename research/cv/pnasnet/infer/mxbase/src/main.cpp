...
int main(int argc, char* argv[]) {
    ...
    initParam.modelPath = "../../convert/pnasnet.om";
    auto pnasnet = std::make_shared<PNASNET>();
    APP_ERROR ret = pnasnet->Init(initParam);
    if (ret != APP_ERR_OK) {
        LogError << "pnasnetClassify init failed, ret=" << ret << ".";
        return ret;
    }
    ...
    for (uint32_t i = 0; i <= imgFilePaths.size() - BATCH_SIZE; i += BATCH_SIZE) {
        ...
        ret = pnasnet->Process(batchImgFilePaths);
        if (ret != APP_ERR_OK) {
            LogError << "PNASNETClassify process failed, ret=" << ret << ".";
            pnasnet->DeInit();
            return ret;
        }
    ...
    pnasnet->DeInit();
    double fps = 1000.0 * inferImgsCount / pnasnet->GetInferCostMilliSec();
    LogInfo << "[Process Delay] cost: " << pnasnet->GetInferCostMilliSec() << " ms\tfps: " << fps << " imgs/sec";
    return APP_ERR_OK;
}
