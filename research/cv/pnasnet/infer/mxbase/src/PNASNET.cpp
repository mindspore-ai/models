...
namespace {
    ...
    const size_t W = 224;
    const size_t C = 3;
    const std::vector<uint32_t> Shape = {1, 224, 224, 3};
}  // namespace

APP_ERROR PNASNET::Init(const InitParam &initParam) {
...
APP_ERROR PNASNET::ReadImage(const std::string &imgPath, cv::Mat *imageMat) {
    *imageMat = cv::imread(imgPath, cv::IMREAD_COLOR);
    return APP_ERR_OK;
}

APP_ERROR PNASNET::ResizeImage(const cv::Mat &srcImageMat, cv::Mat *dstImageMat) {
...
APP_ERROR PNASNET::CVMatToTensorBase(const cv::Mat &imageMat, MxBase::TensorBase *tensorBase) {
    uint32_t dataSize = 1;
...
APP_ERROR PNASNET::Inference(const std::vector<MxBase::TensorBase> &inputs,
...
APP_ERROR PNASNET::PostProcess(const std::vector<MxBase::TensorBase> &inputs,
...
APP_ERROR PNASNET::SaveResult(const std::vector<std::string> &batchImgPaths,
...
APP_ERROR PNASNET::Process(const std::vector<std::string> &batchImgPaths) {
