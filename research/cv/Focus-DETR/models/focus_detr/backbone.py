...
def conv3x3(in_planes, out_planes, stride=1, groups=1, dilation=1):
    ...
    # return nn.Conv2d(in_planes, out_planes, kernel_size=3, stride=stride,padding=dilation, pad_mode='pad', group=groups, dilation=dilation)
    return nn.Conv2d(
...
def conv1x1(in_planes, out_planes, stride=1):
    ...
    # return nn.Conv2d(in_planes, out_planes, kernel_size=1, pad_mode="valid", stride=stride)
...
class BackboneBase(nn.Cell):
    ...
    def construct(self, x):
        ...
        for one_x in xs:
            ...
            mask = ops.ResizeNearestNeighbor(size=one_x.shape[-2:])(m[None].astype("float32"))[0]
            mask = mask.astype("bool")
            out[str(self.nums)] = {"data": one_x, "mask": mask}
        return out
...
class Backbone(nn.Cell):
    ...
    def construct(self, tensor_list):
        ...
        for x in xs.value():
            out.append(x)
            pos.append(self.position_embedding(x["mask"]).astype(x["data"].dtype))
        return out, pos
