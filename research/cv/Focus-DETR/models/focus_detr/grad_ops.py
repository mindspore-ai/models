...
def area_box_grad(box):
    """box area grad"""
    return np.stack(
        [box[:, 1] - box[:, 3], box[:, 0] - box[:, 2], box[:, 3] - box[:, 1], box[:, 2] - box[:, 0]], axis=1
    )
...
def grad_xywh_to_cxcy(arr):
    """xywh to cxcy grad"""
    return np.stack(
        [arr[:, 0] + arr[:, 2], arr[:, 1] + arr[:, 3], (-arr[:, 0] + arr[:, 2]) / 2, (-arr[:, 1] + arr[:, 3]) / 2],
        axis=1,
    )


def dres_dwh(dres_df, wh, is_maxmin, src_boxes_cxcy, target_boxes_cxcy):
    """dres/dwh"""
    d_dwh = np.stack([dres_df * wh[:, :, 1], dres_df * wh[:, :, 0]], axis=2)
    if is_maxmin:
        dwh_dmax = -d_dwh * (wh > 0.0).astype(np.int32)
        dwh_dmin = d_dwh * (wh > 0.0).astype(np.int32)
        ...
        src_grad = np.concatenate(
            [(dwh_dmax * max_grad_src).sum(axis=1), (dwh_dmin * min_grad_src).sum(axis=1)], axis=1
        )
    else:
        dwh_dmax = d_dwh * (wh > 0.0).astype(np.int32)
        dwh_dmin = -d_dwh * (wh > 0.0).astype(np.int32)
        ...
        min_grad_src = (src_boxes_cxcy[:, :2] < target_boxes_cxcy[:, :2]).astype(np.int32)
        src_grad = np.concatenate(
            [(dwh_dmin * min_grad_src).sum(axis=1), (dwh_dmax * max_grad_src).sum(axis=1)], axis=1
        )
