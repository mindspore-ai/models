...
if __name__ == '__main__':
    ...
    result_dir = args.outputs_dir
    object_imageSize = args.image_size
    rst_path = args.dataroot

    for i in range(len(os.listdir(rst_path))):
        file_name = os.path.join(rst_path, "CycleGAN_data_bs" + str(args.batch_size) + '_' + str(i) + '_0.bin')
        output = np.fromfile(file_name, np.float32).reshape(3, object_imageSize, object_imageSize)
        print(output.shape)
        save_image(output, result_dir + "/" + str(i + 1))
