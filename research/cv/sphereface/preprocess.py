...
def save_mnist_to_jpg(datatxt_path, pairtxt_path, datasrc, output_path):
    output_path = output_path + "/00_data"
    landmark = {}
    ...
    for j in range(6000):
        ...
        img1path = output_path + '/' + '{}_{}_{}.bin'.format(j, 1, sameflag)
        img2 = np.array(Image.open(name2).convert('RGB'), np.float32)
        img2 = img2[:, :, ::-1]
        img2 = alignment(img2, landmark[name2])
        img2path = output_path + '/' + '{}_{}_{}.bin'.format(j, 2, sameflag)
        imglist = [img1, cv2.flip(img1, 1), img2, cv2.flip(img2, 1)]
        for t in range(len(imglist)):
            imglist[t] = imglist[t].transpose(2, 0, 1)
            imglist[t] = imglist[t].reshape(1, 3, 112, 96)
            imglist[t] = (imglist[t] - 127.5) / 128.0
        imglist[0].tofile(img1path)
        imglist[2].tofile(img2path)
    print("=" * 20, "preprocess data finished", "=" * 20)


if __name__ == '__main__':
    save_mnist_to_jpg(config.datatxt_src, config.pairtxt_src, config.datasrc, "./preprocess_result")
