...
#    xy = np.array(xy)
    ...
    # Solve for trans1
    trans1 = findNonreflectiveSimilarity(uv, xy, options)
...
def get_similarity_transform(src_pts, dst_pts, reflective=True):
    ...
    if reflective:
        trans = findSimilarity(src_pts, dst_pts)
    else:
        trans = findNonreflectiveSimilarity(src_pts, dst_pts)
...
def get_similarity_transform_for_cv2(src_pts, dst_pts, reflective=True):
    ...
    ----------
        @cv2_trans: 2x3 np.array
            ...
            for cv2.warpAffine()
    """
    trans = get_similarity_transform(src_pts, dst_pts, reflective)
