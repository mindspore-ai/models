...
def _bn(channel, res_base=False, is_training=False):
    if res_base:
        return nn.BatchNorm2d(channel, eps=1e-5, momentum=0.1,
                              gamma_init=1, beta_init=0, use_batch_statistics=FROZEN).set_train(FROZEN)
    return nn.BatchNorm2d(channel, eps=1e-4, momentum=0.9,
                          gamma_init=1, beta_init=0, use_batch_statistics=FROZEN).set_train(FROZEN)


def _bn_last(channel, is_training=False):
    return nn.BatchNorm2d(channel, eps=1e-4, momentum=0.9,
                          gamma_init=0, beta_init=0, use_batch_statistics=FROZEN).set_train(FROZEN)
...
class ResidualBlock(nn.Cell):
    ...
    def __init__(self,
        ...
        self.bn1 = nn.BatchNorm2d(channel, use_batch_statistics=FROZEN).set_train(FROZEN)
...
class ResNet(nn.Cell):
    ...
    def __init__(self,
        ...
        self.bn1 = nn.BatchNorm2d(64, use_batch_statistics=FROZEN).set_train(FROZEN)
        self.relu = P.ReLU()
...
def resnet50(pretrained=False, preckpt_path=None, class_num=1001):
    model = ResNet(ResidualBlock, [3, 4, 6, 3], [64, 256, 512, 1024], [256, 512, 1024, 2048], \
    [1, 2, 2, 2], class_num)
    if pretrained:
        if preckpt_path is not None:
            param_dict = load_checkpoint(preckpt_path)
