...
if [[ $# -lt 3 || $# -gt 4 ]]
then
    echo "Usage: bash run_eval_retrieval_images.sh [IMAGES_PATH] [GT_PATH] [PIPELINE-PATH] 
    DEVICES is optional.
    Please write the two images' name into the list_images.txt which you want to match.
    Usage example:bash run_eval_retrieval_images.sh ../data/ox ../data/ox_gt ./delf.pipeline
    Only for extract features:python3 main.py  --use_list_txt=False     --PL_PATH=./delf.pipeline     --images_path=../data/ox --target_path='./eval_features'
    "
exit 1
fi
...
function extract_feature()
{
    ...
    python3 main.py  --use_list_txt=False \
