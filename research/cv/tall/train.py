...
if __name__ == '__main__':
    ...
    if cfg.mode == context.GRAPH_MODE:
        model.train(cfg.max_epoch, dataset, callbacks=[ckpoint, LossMonitor(), TimeMonitor()], dataset_sink_mode=True)
    else:
        model.train(cfg.max_epoch, dataset, callbacks=[ckpoint, LossMonitor(), TimeMonitor()], dataset_sink_mode=False)
    print('Done.')

    print("Save CTRL Model...")
    save_checkpoint(net, f"{cfg.log_dir}/latest.ckpt")
    print('Done.')
