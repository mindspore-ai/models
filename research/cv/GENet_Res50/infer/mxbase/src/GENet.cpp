...
APP_ERROR GENet::Init(const InitParam &initParam) {
...
void GENet::ReadImage(const std::string &imgPath, cv::Mat *imageMat) {
    *imageMat = cv::imread(imgPath, cv::IMREAD_COLOR);
}

void GENet::ResizeImage(const cv::Mat &srcImageMat, cv::Mat *dstImageMat) {
...
APP_ERROR GENet::Crop(const cv::Mat &srcImageMat, cv::Mat *dstImageMat) {
    static cv::Rect rectOfImg(16, 16, 224, 224);
    *dstImageMat = srcImageMat(rectOfImg).clone();
    return APP_ERR_OK;
}

APP_ERROR GENet::CVMatToTensorBase(const cv::Mat &imageMat,
...
APP_ERROR GENet::Inference(const std::vector<MxBase::TensorBase> &inputs,
...
APP_ERROR GENet::PostProcess(const std::vector<MxBase::TensorBase> &inputs,
...
APP_ERROR GENet::SaveResult(const std::string &imgPath,
...
APP_ERROR GENet::Process(const std::string &imgPath) {
    cv::Mat imageMat;
