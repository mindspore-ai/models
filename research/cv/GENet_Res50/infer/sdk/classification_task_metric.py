...
def cre_groundtruth_dict_fromtxt(gtfile_path):
    """load_statistical_predict_result"""
    img_gt_dict = {}
...
def load_statistical_predict_result(filepath):  # such as /data/n02423022_5982.JPEG
...
def create_visualization_statistical_result(prediction_file_path,
    ...
    for tfile_name in os.listdir(prediction_file_path):  # tfile_name : such as n02423022_5982.JPEG
        count += 1
        temp = tfile_name.split('.')[0]
        index = temp.rfind('_')
        img_name = temp[:index]                             # n02423022
