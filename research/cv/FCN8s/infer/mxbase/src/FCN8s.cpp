...
APP_ERROR FCN8s::Init(const InitParam &initParam) {
    ...
    if (ret != APP_ERR_OK) {
        LogError << "FCN8sPostProcess init failed, ret=" << ret << ".";
        return ret;
    }
    return APP_ERR_OK;
}
...
void FCN8s::ReadImage(const std::string &imgPath, cv::Mat &imageMat) {
    imageMat = cv::imread(imgPath, cv::IMREAD_COLOR);
    cv::cvtColor(imageMat, imageMat, cv::COLOR_BGR2RGB);
}

void FCN8s::ResizeImage(const cv::Mat &srcImageMat, cv::Mat &dstImageMat,
...
void FCN8s::Padding(const cv::Mat &srcImageMat, cv::Mat &dstImageMat) {
    constexpr int32_t paddingHeight = 512;
    constexpr int32_t paddingWidth = 512;
...
APP_ERROR FCN8s::CVMatToTensorBase(const cv::Mat &imageMat, MxBase::TensorBase &tensorBase) {
...
APP_ERROR FCN8s::Inference(const std::vector<MxBase::TensorBase> &inputs,
...
APP_ERROR FCN8s::Process(const std::string &imgPath) {
    cv::Mat imageMat;
...
void FCN8s::SaveResultToImage(const MxBase::SemanticSegInfo &segInfo, const std::string &filePath) {
