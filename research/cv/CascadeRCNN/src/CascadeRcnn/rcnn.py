...
class Rcnn(nn.Cell):
    ...
    def construct(self, featuremap, bbox_targets, labels, mask, stage=None):
        "RCNN"
        ...
        if self.training:
            ...
            labels_ = self.cast(self.onehot(labels, self.num_classes, self.on_value, self.off_value), self.ms_type)
            bbox_targets = self.tile(self.expandims(bbox_targets, 1), (1, self.num_classes, 1))


            loss, loss_cls, loss_reg, loss_print = self.loss(x_cls, x_reg, bbox_targets, bbox_weights, labels_, mask)
            #labels2 = self.cast(self.less(labels, 82), mstype.int32)*labels
            bbox_weights_ = self.tile(self.expandims((self.expandims(labels, 1)), 1), (1, 1, 4))
            #bbox_weights_ = self.tile(self.expandims(labels, 1), (1, 1, 4))

            #bbox_weights_ = self.cast(self.reshape(bbox_weights_, (-1, 1, 4)),mstype.int32)

            x_reg = self.reshape(x_reg, (-1, self.num_classes, 4))
            x_reg = self.gatherD(x_reg, 1, bbox_weights_)

            x_reg = self.squeeze(x_reg)

            out = (x_reg, loss, loss_cls, loss_reg, loss_print)
        else:
            x_reg = self.reg_scores(x)
            if stage != 3:
                x_reg = self.reshape(x_reg, (-1, self.num_classes, 4))
                index, _ = self.argmax(x_cls)
                bbox_weights_ = self.tile(self.expandims((self.expandims(index, 1)), 1), (1, 1, 4))
                x_reg = self.gatherD(x_reg, 1, bbox_weights_)
                x_reg = self.squeeze(x_reg)
            else:
                x_reg = x_reg
            out = (x_reg, x_cls, (x_cls / self.value), x_reg, x_cls)
    ...
    def loss(self, cls_score, bbox_pred, bbox_targets, bbox_weights, labels, weights):
        """Loss method."""
        loss_print = ()
        labels = self.cast(labels, mstype.float16)
