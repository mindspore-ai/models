...
"""train CascadeRcnn and get checkpoint files."""
...
parser = argparse.ArgumentParser(description="CascadeRcnn training")
...
if __name__ == '__main__':
    ...
    # and the file name is CascadeRcnn.mindrecord0, 1, ... file_num.
    ...
    # When create MindDataset, using the fitst mindrecord file, such as CascadeRcnn.mindrecord0.
    dataset = create_cascadercnn_dataset(mindrecord_file, batch_size=config.batch_size,
    ...
    net = Cascade_Rcnn_Resnet101(config=config)
    net = net.set_train()
    ...
    time_cb = TimeMonitor(dataset_size)
    loss_cb = LossCallBack(rank_id=rank)
    cb = [time_cb, loss_cb]
    if config.save_checkpoint:
        ckptconfig = CheckpointConfig(save_checkpoint_steps=config.save_checkpoint_epochs * dataset_size,
                                      keep_checkpoint_max=config.keep_checkpoint_max)
        save_checkpoint_path = os.path.join(config.save_checkpoint_path, "ckpt_0908_101_int32_" + str(rank) + "/")
        ckpoint_cb = ModelCheckpoint(prefix='cascade_rcnn', directory=save_checkpoint_path, config=ckptconfig)
        cb += [ckpoint_cb]
