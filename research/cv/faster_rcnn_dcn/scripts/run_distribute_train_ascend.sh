...
if [ $# -le 2 ]
then 
    echo "Usage: bash run_distribute_train_ascend.sh [RANK_TABLE_FILE] [PRETRAINED_PATH] [COCO_ROOT] [MINDRECORD_DIR](option)"
exit 1
fi
...
do
    ...
    --rank_id=$i --run_distribute=True --device_num=$DEVICE_NUM --pre_trained=$PATH2 &> log &
    cd ..
done