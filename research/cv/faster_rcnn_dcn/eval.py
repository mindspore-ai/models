...
def fasterrcnn_eval(dataset_path, ckpt_path, anno_path):
    ...
    summary_metrics = coco_eval(result_files, eval_types, dataset_coco, config, single_result=True)
    acc_file = open(config.acclog_path, "w+")
    for key in summary_metrics:
        acc_file.write("{} = {:.3f}".format(key, summary_metrics[key]))
        acc_file.write("\n")
    acc_file.close()
    print("eval end")
