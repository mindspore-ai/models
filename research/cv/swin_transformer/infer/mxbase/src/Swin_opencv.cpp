...
#include <string>
#include <vector>
#include "Swin_opencv.h"
#include "MxBase/DeviceManager/DeviceManager.h"
#include "MxBase/Log/Log.h"


APP_ERROR Swin_opencv::Init(const InitParam &initParam) {
    ...
    if (ret != APP_ERR_OK) {
        LogError << "SwinPostProcess init failed, ret=" << ret << ".";
        return ret;
    }
    return APP_ERR_OK;
}

APP_ERROR Swin_opencv::DeInit() {
    model_->DeInit();
    post_->DeInit();
...
void Swin_opencv::ReadImage(const std::string &imgPath, cv::Mat &imageMat) {
    imageMat = cv::imread(imgPath, cv::IMREAD_COLOR);
}

void Swin_opencv::ResizeImage(const cv::Mat &srcImageMat, cv::Mat &dstImageMat) {
...
APP_ERROR Swin_opencv::CVMatToTensorBase(const cv::Mat &imageMat, MxBase::TensorBase &tensorBase) {
...
APP_ERROR Swin_opencv::Inference(const std::vector<MxBase::TensorBase> &inputs,
...
APP_ERROR Swin_opencv::PostProcess(const std::vector<MxBase::TensorBase> &inputs,
...
APP_ERROR Swin_opencv::SaveResult(const std::string &imgPath,
    ...
    if (outfile.fail()) {
        LogError << "Failed to open result file: " << resFileName;
...
APP_ERROR Swin_opencv::Process(const std::string &imgPath) {
    cv::Mat imageMat;
