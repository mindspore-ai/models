...
int main(int argc, char* argv[]) {
    if (argc <= 1) {
        LogWarn << "Please input image path, such as './Swin image_dir'.";
        return APP_ERR_OK;
    }
    ...
    initParam.labelPath = "../data/config/label.txt";
    initParam.topk = 5;
    initParam.softmax = false;
    initParam.checkTensor = true;
    initParam.modelPath = "../data/model/swin_opencv.om";
    auto resnet50 = std::make_shared<Swin_opencv>();
    APP_ERROR ret = resnet50->Init(initParam);
    if (ret != APP_ERR_OK) {
        LogError << "Swin_opencv init failed, ret=" << ret << ".";
        return ret;
    }
    ...
    for (auto &imgFile : imgFilePaths) {
        ...
        if (ret != APP_ERR_OK) {
            LogError << "Swin_opencv process failed, ret=" << ret << ".";
            resnet50->DeInit();
            return ret;
        }
    }
