...
from src.models.mtcnn import ONetWithLoss, ONetTrainOneStepCell
...
def train_onet(args):
    ...
    network = ONetWithLoss()

    network.set_train(True)

    # decay lr
    if args.distribute:
        lr_scheduler = MultiEpochsDecayLR(args.lr, [12, 17, 20], steps_per_epoch)
    else:
    ...
    # train net
    train_net = ONetTrainOneStepCell(network, optimizer)
    ...
    if args.distribute and device_id == 0:
        save_checkpoint(network, os.path.join(args.ckpt_path, 'onet_distribute.ckpt'))
    elif not args.distribute:
        save_checkpoint(network, os.path.join(args.ckpt_path, 'onet_standalone.ckpt'))
