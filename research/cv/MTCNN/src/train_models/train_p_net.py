...
from src.models.mtcnn import PNetTrainOneStepCell, PNetWithLoss
...
def train_pnet(args):
    ...
    if args.distribute and device_id == 0:
        save_checkpoint(network, os.path.join(args.ckpt_path, 'pnet_distribute.ckpt'))
    elif not args.distribute:
        save_checkpoint(network, os.path.join(args.ckpt_path, 'pnet_standalone.ckpt'))
