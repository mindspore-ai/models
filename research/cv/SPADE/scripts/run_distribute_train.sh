...
then
    ...
    echo "bash ./scripts/run_distribute_train.sh [vgg_ckpt_path] [data_root] [load_epoch]"
    echo "bash ./scripts/run_distribute_train.sh [vgg_ckpt_path] [data_root] [load_epoch]"
    echo "For example: bash ./scripts/run_distribute_train.sh ./vgg/vgg19.ckpt ./ADEChallengeData2016 0"
...
python -u ./train.py --distribute True \
                      --vgg_ckpt_path $VGG_CKPT_PATH \
                      --dataroot $DATA_ROOT \
                      --now_epoch $LOAD_EPOCH > distribute_train.log 2>&1 &