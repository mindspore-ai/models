...
from src.mmpose.post_transforms import transform_preds
...
def keypoints_from_heatmaps(heatmaps,
    ...
    if use_udp:
        if target_type.lower() == 'GaussianHeatMap'.lower():
            ...
            process_combined_target_heatmaps(heatmaps, kernel)
    ...
    elif post_process == 'unbiased':   # alleviate biased coordinate
        preds, maxvals = _get_max_preds(heatmaps)
    ...
    elif (post_process is not None) & (post_process != 'unbiased'):
        preds, maxvals = _get_max_preds(heatmaps)
...
def process_combined_target_heatmaps(heatmaps, kernel):
