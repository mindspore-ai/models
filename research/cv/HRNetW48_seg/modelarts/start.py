...
def main():
    ...
    if device_id == 0:
        ...
        pd = load_checkpoint(ckpt_model)
        params_dict = {}
        for k, v in pd.items():
            params_dict["model." + k] = v
        load_param_into_net(net, params_dict, strict_load=True)
        net.set_train(False)
        file_name = os.path.join(local_train_url, f"hrnetw48seg-{args.dataset}")
        input_data = Tensor(np.zeros([1, 3, image_size[0], image_size[1]], dtype=np.float32))
