...
from src.trainer.accu_grad_trainer import TrainOneStepWithLossScaleCellAccumulation
...
def set_device(args):
    ...
    device_num = int(os.environ.get("DEVICE_NUM", args.device_num))
...
def get_train_one_step(args, net_with_loss, optimizer):
    ...
    elif args.accumulation_step > 1:
        if args.device_target == "Ascend":
            raise ValueError("It's not support to Ascend when accumulation_step is larger than 1")
        net_with_loss = TrainOneStepWithLossScaleCellAccumulation(net_with_loss, optimizer, scale_sense=scale_sense,
