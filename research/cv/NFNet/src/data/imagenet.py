...
def create_dataset_imagenet(dataset_dir, args, repeat_num=1, training=True):
    ...
    if training:
        ...
        if args.crop:
            if args.crop_pct > 1:
                test_size = int(test_input_size + args.crop_pct)
            else:
                test_size = int(test_input_size / args.crop_pct)
            transform_img = [
                vision.Decode(),
                vision.Resize(test_size, interpolation=Inter.BICUBIC),
                vision.CenterCrop(test_input_size),
            ...
            transform_img = [
                vision.Decode(),
                vision.Resize(int(test_input_size), interpolation=Inter.BICUBIC),
                vision.Normalize(mean=mean, std=std),
                vision.HWC2CHW()
            ]
