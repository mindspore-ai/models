...
DEFINE_string(dataset_path, "../MOT16/train", "dataset path");
...
int main(int argc, char **argv) {
    ...
    ascend310_info->SetBufferOptimizeMode("off_optimize");
    ...
    auto all_files = GetAllFiles("./pre", ".bin");
    if (all_files.empty()) {
    ...
    cout << "model input     :";
    cout << modelInputs[0].DataSize() << endl;
    ...
    for (size_t i = 0; i < size; ++i) {
        ...
        WriteTensorToFile(all_files[i] + "1.bin", image);
        ...
        WriteResult("./pre", all_files[i], outputs);
    }
    double average = 0.0;
    int infer_cnt = 0;
    ...
    file_stream << timeCost.str() << std::endl;
    file_stream.close();
    costTime_map.clear();
    return 0;
}
