...
from mindspore import Tensor, load_checkpoint, load_param_into_net, export, context, Model
...
set_seed(1)
parser = argparse.ArgumentParser(description='osnet', formatter_class=argparse.ArgumentDefaultsHelpFormatter)
...
parser.add_argument('--max_epoch', type=int, default=2, help='the max num of epoch')
parser.add_argument('--start_epoch', type=int, default=1, help='the start num of epoch')
...
def set_save_ckpt_dir():
    ...
    if args.enable_modelarts and config.run_distribute:
...
def modelarts_pre_process():
    ...
    if config.need_modelarts_dataset_unzip:
        zip_file_1 = os.path.join(args.data_url, config.source + ".zip")
        save_dir_1 = os.path.join(args.data_url)
...
def save_ckpt_to_air(save_ckpt_path, path):
    ...
    dataset = init_dataset(name=config.target, root=args.data_url, mode='train',
    ...
    export(net, input_arr, file_name=save_ckpt_path +'osnet', file_format="AIR")
...
def train_net():
    ...
    num_classes, dataset1 = dataset_creator(root=args.data_url, height=config.height, width=config.width,
    ...
    num_classes, dataset2 = dataset_creator(root=args.data_url, height=config.height, width=config.width,
                                            transforms=config.transforms, dataset=config.source,
                                            norm_mean=config.norm_mean, norm_std=config.norm_std,
                                            batch_size_train=config.batch_size_train, workers=config.workers,
                                            cuhk03_labeled=config.cuhk03_labeled,
    ...
    model2.train(args.max_epoch-args.start_epoch, dataset2, cb2, dataset_sink_mode=True)
    print("============== End Training ==============")
    print("train success")
    path = os.path.join(set_save_ckpt_dir(), 'osnet'+'-'+str(args.max_epoch-args.start_epoch)+'_'+str(num_batches)+\
                        '.ckpt')
    save_ckpt_to_air(set_save_ckpt_dir(), path)
    moxing.file.copy_parallel(set_save_ckpt_dir(), args.train_url)
