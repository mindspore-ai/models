...
int main(int argc, char* argv[]) {
    ...
    auto osnet = std::make_shared<OSNet>();
    APP_ERROR ret = osnet->Init(initParam);
    if (ret != APP_ERR_OK) {
        LogError << "OSNet init failed, ret=" << ret << ".";
        return ret;
    }
    ...
    for (uint32_t i = 0; i < files.size(); i++) {
        ...
        ret = osnet->Process(input_data_path, files[i]);
        if (ret != APP_ERR_OK) {
            LogError << "OSNet process failed, ret=" << ret << ".";
            osnet->DeInit();
            return ret;
        }
    }
