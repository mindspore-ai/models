...
def parse_cli_to_yaml(parser, cfg, helper=None, choices=None, \
    cfg_path="resnet50_cifar10_config.yaml"):
    """
    Parse command line arguments to the configuration according to the default \
        yaml.
    ...
    for item in cfg:
        if not isinstance(cfg[item], list) and not isinstance(cfg[item], dict):
            help_description = helper[item] if item in helper \
                else f"Please reference to {cfg_path}"
    ...
    with open(yaml_path, 'r', encoding="utf-8") as fin:
        try:
            ...
            if len(cfgs) == 1:
                ...
                raise ValueError("At most 3 docs (config description for help, choices) \
                    are supported in config yaml")
            print(cfg_helper)
        except ValueError as value_err:
            raise ValueError("Failed to parse yaml") from value_err
...
def get_config():
    ...
    pprint(default)
    args = parse_cli_to_yaml(parser=parser,
                             cfg=default,
                             helper=helper,
                             choices=choices,
