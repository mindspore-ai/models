...
if __name__ == '__main__':
    ...
    if config.save_checkpoint and (device_num == 1 or device_id == 0):
        ...
        if args_opt.run_modelarts:
            ckpt_cb = ModelCheckpoint(f"EfficientNet-rank{device_id}", directory=local_train_url, config=config_ck)
        else:
            save_ckpt_path = os.path.join(config.save_checkpoint_path, 'model_' + str(device_id) + '/')
            ckpt_cb = ModelCheckpoint(f"EfficientNet-rank{device_id}", directory=save_ckpt_path, config=config_ck)
        cb += [ckpt_cb]
    # begin train
    model.train(args_opt.epoch, dataset, callbacks=cb, dataset_sink_mode=True)
    ...
    for f in os.listdir(local_train_url):
        ...
        if f.endswith('.ckpt'):
            ...
            input_shp = [1, 3, 300, 300]
            input_array = Tensor(np.random.uniform(-1.0, 1.0, size=input_shp).astype(np.float32))
            export(net, input_array, file_name=local_train_url + '/efficientnet', file_format='AIR')
    if args_opt.run_modelarts and config.save_checkpoint and (device_num == 1 or device_id == 0):
        mox.file.copy_parallel(local_train_url, args_opt.train_url)
