...
class ANETproposal():
    ...
    def __init__(self, ground_truth_filename=None, proposal_filename=None,
        ...
        if self.verbose:
            log.info('[INIT] Loaded annotations from %s subset.', (subset))
            nr_gt = len(self.ground_truth)
            log.info('\tNumber of ground truth instances: %d', (nr_gt))
            nr_pred = len(self.proposal)
            log.info('\tNumber of proposals: %d', (nr_pred))
            log.info('\tFixed threshold for tiou score: %s', (self.tiou_thresholds.__str__()))
    ...
    def evaluate(self):
        ...
        self.auc = 100.*float(area_under_curve)/proposals_per_video[-1]
