...
int main(int argc, char* argv[]) {
    ...
    auto ras = std::make_shared<RAS>();
    APP_ERROR ret = ras->Init(initParam);
    if (ret != APP_ERR_OK) {
        LogError << "RAS init failed, ret=" << ret << ".";
        return ret;
    }
    std::string data_path = input_data_path + "images/";
    std::vector<std::string> files;
    ret = ReadFilesFromPath(data_path, &files);
    if (ret != APP_ERR_OK) {
    ...
    for (uint32_t i = 0; i < files.size(); i++) {
        ...
        ret = ras->Process(input_data_path, files[i]);
        if (ret != APP_ERR_OK) {
            LogError << "RAS process failed, ret=" << ret << ".";
            ras->DeInit();
            return ret;
        }
    }
