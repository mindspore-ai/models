...
from mindspore import save_checkpoint, export, load_checkpoint, load_param_into_net
...
def do_export(device_target1, device_id1, pretrained_model, model_ckpt, batchsize, save_path):
    ms.context.set_context(mode=ms.context.GRAPH_MODE, device_target=device_target1, device_id=device_id1)
    net = BoneModel(device_target1, pretrained_model)
    param_dict = load_checkpoint(model_ckpt)
    load_param_into_net(net, param_dict)
    input_arr = ms.Tensor(np.ones((batchsize, 3, 352, 352)).astype(np.float32))

    export(net, input_arr, file_name=save_path + "RAS", file_format='AIR')
...
class RASWhole:
    ...
    def train(self):
        ...
        do_export(par.device_target, par.device_id,
                  self.pre_trained_model_path, save_path_all, self.batchsize, checkpoint_out)
        mox.file.copy_parallel(checkpoint_out, models_path)
...
if __name__ == "__main__":
    ...
    checkpoint_out = '/cache/output/'
    local_data_path = '/cache/datasets/'
    ...
    mox.file.copy_parallel(src_url=pre_trained_true_path.rsplit("/", 1)[0], dst_url=model_path)
    ...
    pre_trained_model_path = os.path.join(model_path, pre_trained_true_path.split("/")[-1])
