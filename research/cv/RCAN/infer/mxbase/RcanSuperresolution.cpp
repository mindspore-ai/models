...
APP_ERROR RcanSuperresolution::Init(const InitParam &initParam) {
...
APP_ERROR RcanSuperresolution::DeInit() {
    model_->DeInit();
...
// padding
APP_ERROR RcanSuperresolution::PaddingImage(cv::Mat *imageSrc, cv::Mat *imageDst, const uint32_t &targetLength) {
...
APP_ERROR RcanSuperresolution::ReadImage(const std::string &imgPath, cv::Mat *imageMat) {
...
APP_ERROR RcanSuperresolution::CVMatToTensorBase(const cv::Mat &imageMat, MxBase::TensorBase *tensorBase) {
...
APP_ERROR RcanSuperresolution::Inference(std::vector<MxBase::TensorBase> *inputs,
...
APP_ERROR RcanSuperresolution::PostProcess(std::vector<MxBase::TensorBase> *inputs, cv::Mat *imageMat) {
        ...
        auto data = reinterpret_cast<float(*)[outputModelChannel][outputModelHeight]
        [outputModelWidth]>(tensor.GetBuffer());
...
APP_ERROR RcanSuperresolution::Process(const std::string &imgPath) {
    cv::Mat imageMat;
