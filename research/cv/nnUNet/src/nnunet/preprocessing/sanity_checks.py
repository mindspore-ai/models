...
import numpy as np
from batchgenerators.utilities.file_and_folder_operations import subfiles, isfile, join, isdir, load_json, os
...
def verify_dataset_integrity(folder):
    ...
    # check all cases
    if len(expected_train_identifiers) != len(np.unique(expected_train_identifiers)): raise RuntimeError(
        "found duplicate training cases in dataset.json")

    print("Verifying training set")
    geometries_OK, has_nan = Verifying_training_set(expected_train_identifiers, folder, geometries_OK, has_nan,
                                                    label_files, nii_files_in_imagesTr, nii_files_in_labelsTr,
                                                    num_modalities)

    # check for stragglers
    assert not nii_files_in_imagesTr, \
        "there are training cases in imagesTr that are not listed in dataset.json: %s" % nii_files_in_imagesTr
    assert not nii_files_in_labelsTr, \
        "there are training cases in labelsTr that are not listed in dataset.json: %s" % nii_files_in_labelsTr
    ...
    assert all(
        ...
        f'Labels must be in consecutive order (0, 1, 2, ...). The labels ' \
        f'{np.array(expected_labels)[1:][~labels_valid_consecutive]} ' \
        f'do not satisfy this restriction'
    ...
    if fail:
        raise AssertionError(
            "Found unexpected labels in the training dataset. "
            "Please correct that or adjust your dataset.json accordingly")
    if not fail:
        print("Labels OK")

    # check test set, but only if there actually is a test set
    verify_dataset_integrity_part2(expected_test_identifiers, folder, nii_files_in_imagesTr, num_modalities)

    all_same, _ = verify_all_same_orientation(join(folder, "imagesTr"))
    if not all_same:
        print(
            "WARNING: Not all images in the dataset have the same axis ordering. "
            "We very strongly recommend you correct that by reorienting the data. "
            "fslreorient2std should do the trick")
    # save unique orientations to dataset.json
    if not geometries_OK:
        raise Warning(
            "GEOMETRY MISMATCH FOUND! CHECK THE TEXT OUTPUT! This does not cause an error at this point "
            "but you should definitely check whether your geometries are alright!")
    if geometries_OK:
        print("Dataset OK")

    if has_nan:
        raise RuntimeError(
            "Some images have nan values in them. This will break the training. "
            "See text output above to see which ones")


def verify_dataset_integrity_part2(expected_test_identifiers, folder, nii_files_in_imagesTr, num_modalities):
    """part of function verify_dataset_integrity"""
    if expected_test_identifiers:
            ...
            if num_modalities > 1:
                ...
                for i, img in enumerate(images_itk[1:]):
                    assert verify_same_geometry(img, reference_img), \
                        ...
                        "registered. Please coregister your modalities." % (expected_image_files[i])
        ...
        assert not nii_files_in_imagesTs, \
            "there are training cases in imagesTs that are not listed in dataset.json: %s" % nii_files_in_imagesTr


def Verifying_training_set(expected_train_identifiers, folder, geometries_OK, has_nan, label_files,
                           nii_files_in_imagesTr, nii_files_in_labelsTr, num_modalities):
    """part of function verify_dataset_integrity"""
