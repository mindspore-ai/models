...
from batchgenerators.utilities.file_and_folder_operations import os, pickle, subfiles, maybe_mkdir_p, load_pickle, join
from scipy.ndimage.interpolation import map_coordinates
from skimage.transform import resize

from src.nnunet.configuration import default_num_threads, RESAMPLING_SEPARATE_Z_ANISO_THRESHOLD
from src.nnunet.preprocessing.cropping import get_case_identifier_from_npz, ImageCropper


def get_do_separate_z(spacing, anisotropy_threshold=RESAMPLING_SEPARATE_Z_ANISO_THRESHOLD):
    """separate z axis from data"""
    do_separate_z = (np.max(spacing) / np.min(spacing)) > anisotropy_threshold
    return do_separate_z


def get_lowres_axis(new_spacing):
    "get lowre axis according spacing"
...
def resample_data_or_seg(data, new_shape, is_seg, axis=None, order=3, do_separate_z=False, order_z=0):
    ...
    if np.any(shape != new_shape):
        ...
        if do_separate_z:
            ...
            reshaped_final_data = []
            part_of_resample_data_or_seg(axis, data, dtype_data, is_seg, kwargs, new_shape, new_shape_2d, order,
                                         order_z, reshaped_final_data, resize_fn, shape)
...
def part_of_resample_data_or_seg(axis, data, dtype_data, is_seg, kwargs, new_shape, new_shape_2d, order, order_z,
                                 reshaped_final_data, resize_fn, shape):
    """
    part of resample data or seg
    """
    for c in range(data.shape[0]):
        reshaped_data = []
        ...
        if shape[axis] != new_shape[axis]:
            ...
            if not is_seg or order_z == 0:
                ...
                for _, cl in enumerate(unique_labels):
...
class GenericPreprocessor():
    ...
    def run(self, target_spacings, input_folder_with_cropped_npz, output_folder, data_identifier,
        ...
        :param input_folder_with_cropped_npz: dim: c, x, y, z |
        npz_file['data'] np.savez_compressed(fname.npz, data=arr)
        :param output_folder:
        :param num_threads:
        ...
        for i in range(num_stages):
            ...
            for _, case in enumerate(list_of_cropped_npz_files):
                case_identifier = get_case_identifier_from_npz(case)
                args = spacing, case_identifier, output_folder_stage, \
                       input_folder_with_cropped_npz, force_separate_z, all_classes
...
class PreprocessorFor2D(GenericPreprocessor):
    ...
    def run(self, target_spacings, input_folder_with_cropped_npz, output_folder, data_identifier,
            num_threads=default_num_threads, force_separate_z=None):
        """PreprocessorFor2D run"""
        print("Initializing to run preprocessing")
        print("npz folder:", input_folder_with_cropped_npz)
        print("output_folder:", output_folder)
        list_of_cropped_npz_files = subfiles(input_folder_with_cropped_npz, True, None, ".npz", True)
        assert list_of_cropped_npz_files, "set list of files first"
        maybe_mkdir_p(output_folder)
        all_args = []
        num_stages = len(target_spacings)
    ...
    def resample_and_normalize(self, data, target_spacing, properties, seg=None, force_separate_z=None):
        """resample and normalize function"""
        original_spacing_transposed = np.array(properties["original_spacing"])[self.transpose_forward]
        before = {
            'spacing': properties["original_spacing"],
            'spacing_transposed': original_spacing_transposed,
...
class PreprocessorFor2D_noNormalization(GenericPreprocessor):
    """PreprocessorFor2D and noNormalization"""
