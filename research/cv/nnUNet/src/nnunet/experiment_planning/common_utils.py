...
def split_4d_nifti(filename, output_folder):
    ...
    if dim == 3:
        ...
        return
    if dim != 4:
        print("Unexpected dimensionality: %d of file %s, cannot split" % (dim, filename))
    else:
        img_npy = sitk.GetArrayFromImage(img_itk)
        spacing_origin = img_itk.GetSpacing()
        origin = img_itk.GetOrigin()
        direction = np.array(img_itk.GetDirection()).reshape(4, 4)
        # now modify these to remove the fourth dimension
        spacing = tuple(list(spacing_origin[:-1]))
...
def get_pool_and_conv_props(spacing, patch_size, min_feature_map_size, max_numpool):
    ...
    while True:
        ...
        part_get_pool_and_conv_props(current_size, current_spacing, num_pool_per_axis, other_axes, pool_kernel_sizes,
                                     valid_axes_for_pool)
...
def part_get_pool_and_conv_props(current_size, current_spacing, num_pool_per_axis, other_axes, pool_kernel_sizes,
                                 valid_axes_for_pool):
    """part get pool and conv props"""
...
def get_pool_and_conv_props_v2(spacing, patch_size, min_feature_map_size, max_numpool):
    ...
    while True:
        ...
        pool_kernel_sizes = [0] * dim
        part_get_pool_and_conv_propsv2(conv_kernel_sizes, current_size, current_spacing, kernel_size, num_pool_per_axis,
                                       other_axes, pool_kernel_sizes, pool_op_kernel_sizes, valid_axes_for_pool)
...
def part_get_pool_and_conv_propsv2(conv_kernel_sizes, current_size, current_spacing, kernel_size, num_pool_per_axis,
                                   other_axes, pool_kernel_sizes, pool_op_kernel_sizes, valid_axes_for_pool):
    """This function is part of get_pool_and_conv_props_v2"""
