...
from mindspore.ops import AvgPool, AvgPool3D


class DownsampleSegForDSTransform3(AbstractTransform):
    """returns one hot encodings """
    ...
    def __call__(self, **data_dict):
        data_dict[self.output_key] = downsample_seg_for_ds_transform3(data_dict[self.input_key][:, 0], self.ds_scales,
                                                                      self.classes)
        return data_dict


def downsample_seg_for_ds_transform3(seg, ds_scales=((1, 1, 1), (0.5, 0.5, 0.5), (0.25, 0.25, 0.25)), classes=None):
    """downsample seg for ds transform"""
    output = []
...
def downsample_seg_for_ds_transform2(seg, ds_scales=((1, 1, 1), (0.5, 0.5, 0.5), (0.25, 0.25, 0.25)), order=0,
                                     axes=None):
    """downsample seg for ds transform2"""
    if axes is None:
