...
def test_net():
    ...
    context.set_context(mode=context.GRAPH_MODE, enable_graph_kernel=False, device_target=config.device_target)
    if config.device_target == "Ascend":
        context.set_context(device_id=config.device_id)

    # logger
    config.outputs_dir = os.path.join(config.save_dir, datetime.datetime.now().strftime("%Y-%m-%d_time_%H_%M_%S"))
    dataset, _ = classification_dataset(config.batch_size, 1, shuffle=True, repeat_num=1, drop_remainder=True)
