...
def get_result(stream_name_, stream_manager, out_plugin_id=0):
    ...
    infer_result = stream_manager.GetProtobuf(stream_name_, out_plugin_id, key_vec)
...
class PreprocessEvalC3D:
    ...
    def __init__(self):
        ...
        """
        self.transforms = []
        self.clip_mean = np.load("../../pretrained_model/sport1m_train16_128_mean.npy")[0]
...
class Dataset:
    ...
    def getSamples(self, jsonFilePath):
        ...
        for videoInfo in jsonData:
            ...
            for clip in clips:
                self.samples.append(dict(frames=clip, base_path=videoInfo["base_path"]))

    def extractClips(self, video):
        if len(video) >= 16:
            final_video = [video[_idx] for _idx in np.linspace(0, len(video) - 1, 16, dtype="int32")]
            ...
            indices = indices[np.linspace(0, len(indices) - 1, 16, dtype="int32")]
...
def run():
    ...
    for _, (input_data, label) in enumerate(dataset):
        ...
        send_tensor_input(b"c3d", 0, input_data.tobytes(), input_data.shape, streamManagerApi)
