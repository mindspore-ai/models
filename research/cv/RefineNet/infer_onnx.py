...
def parse_args():
    ...
    parser.add_argument('--onnx_path', type=str, default='', help='onnx model to evaluate')
...
def eval_batch(args, onnx_session, img_lst, crop_size=513, flip=True):
    """eval_batch"""
    result_lst = []
    ...
    net_out = onnx_session.run(None, {get_input_name(onnx_session)[0]: batch_img})[0]
    if flip:
        batch_img = batch_img[:, :, :, ::-1]
        net_out_flip = onnx_session.run(None, {get_input_name(onnx_session)[0]: batch_img})[0]
...
def eval_batch_scales(args, onnx_session, img_lst, scales,
                      base_crop_size=513, flip=True):
    """eval_batch_scales"""
    sizes_ = [int((base_crop_size - 1) * sc) + 1 for sc in scales]
    probs_lst = eval_batch(args, onnx_session, img_lst, crop_size=sizes_[0], flip=flip)
    print(sizes_)
    for crop_size_ in sizes_[1:]:
        probs_lst_tmp = eval_batch(args, onnx_session, img_lst, crop_size=crop_size_, flip=flip)
...
def net_eval():
    ...
    onnx_session = onnxruntime.InferenceSession(args.onnx_path, providers=providers)
    # evaluate
    ...
    for i, line in enumerate(img_lst):
        ...
        if bi == args.batch_size:
            batch_res = eval_batch_scales(args, onnx_session, batch_img_lst, scales=args.scales,
