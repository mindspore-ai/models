...
APP_ERROR Refinenet::Init(const InitParam &initParam) {
...
APP_ERROR Refinenet::DeInit() {
    this->model_->DeInit();
    MxBase::DeviceManager::GetInstance()->DestroyDevices();
    return APP_ERR_OK;
}

APP_ERROR Refinenet::ReadTensorFromFile(const std::string &file, float *data) {
    if (data == NULL) {
...
APP_ERROR Refinenet::ReadInputTensor(const std::string &fileName, std::vector<MxBase::TensorBase> *inputs) {
...
APP_ERROR Refinenet::Inference(const std::vector<MxBase::TensorBase> &inputs,
...
APP_ERROR Refinenet::PostProcess(std::vector<MxBase::TensorBase> &inputs, cv::Mat &output) {
    ...
    for (int h = 0; h < outputModelHeight; h++) {
        for (int w = 0; w < outputModelWidth; w++) {
            ...
            output.at<uchar>(h, w) = (index == 0 ? 0 : 255);
        }
    }
    return APP_ERR_OK;
}

APP_ERROR Refinenet::Process(const std::string &inferPath, const std::string &fileName) {
    ...
    cv::Mat output(outputModelHeight, outputModelWidth, CV_8UC1);
