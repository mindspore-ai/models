...
int main(int argc, char *argv[]) {
    ...
    auto refinenet = std::make_shared<Refinenet>();
    APP_ERROR ret = refinenet->Init(initParam);
    if (ret != APP_ERR_OK) {
        LogError << "Refinenet init failed, ret=" << ret << ".";
        return ret;
    }
    ...
    for (uint32_t i = 0; i < files.size(); i++) {
        ...
        ret = refinenet->Process(input_data_path, files[i]);
        if (ret != APP_ERR_OK) {
            LogError << "Refinenet process failed, ret=" << ret << ".";
            refinenet->DeInit();
            return ret;
        }
    }
