...
from src.retinanet_nasfpn import retinanetWithLossCell, retinanetNASFPN, TrainingWrapper
...
def main():
    ...
    ckpt_cb = ModelCheckpoint(prefix="retinanet_nasfpn", directory=config.save_checkpoint_path, config=config_ck)
    if config.distribute:
        if rank == 0:
            cb += [ckpt_cb]
