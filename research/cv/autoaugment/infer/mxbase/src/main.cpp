...
int main(int argc, char* argv[]) {
    ...
    initParam.labelPath = "../../data/test_label.txt";
    initParam.topk = 5;
    initParam.softmax = false;
    initParam.checkTensor = true;
    initParam.modelPath = "../../convert/autoaugment.om";
    auto autoaugment = std::make_shared<AutoAugment>();
    APP_ERROR ret = autoaugment->Init(initParam);
    if (ret != APP_ERR_OK) {
        LogError << "autoaugmentClassify init failed, ret=" << ret << ".";
        return ret;
    }
    ...
    for (uint32_t i = 0; i < files.size(); i++) {
        ret = autoaugment->Process(files[i]);
        if (ret != APP_ERR_OK) {
            LogError << "autoaugmentClassify process failed, ret=" << ret << ".";
            autoaugment->DeInit();
            return ret;
        }
    }
    ...
    double fps = 1000.0 * files.size() / autoaugment->GetInferCostMilliSec();
