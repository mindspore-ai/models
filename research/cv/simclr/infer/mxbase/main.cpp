...
int main(int argc, char* argv[]) {
    if (argc <= 1) {
        LogWarn << "Please input image path, such as './simclr image_dir'.";
        return APP_ERR_OK;
    }
    ...
    initParam.labelPath = "../data/config/cifar10_clsidx_to_labels.names";
    initParam.topk = 5;
    initParam.softmax = false;
    initParam.checkTensor = true;
    initParam.modelPath = "../data/model/simclr.om";
    ...
    for (uint32_t i = 0; i < files.size(); i++) {
        ret = simclr->Process(files[i]);
        if (ret != APP_ERR_OK) {
            LogError << "simclrClassify process failed, ret=" << ret << ".";
            simclr->DeInit();
            return ret;
        }
    }
    ...
    double fps = 1000.0 * files.size() / simclr->GetInferCostMilliSec();
