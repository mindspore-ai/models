        ...
        if cur_epoch >= self.start_epoch:
            if (cur_epoch - self.start_epoch) % self.interval == 0:
                ...
                if (pixel_auc + img_auc) >= (self.best_pixel_added + self.best_image_added):
                    ...
                    save_checkpoint(self.network, "./ckpt/" + self.category + "_best_added.ckpt")
                print("\033[1;34m", flush=True)
                print(
                    "epoch: {},  pixel auc: {}, image auc: {}. "
                    "best pixel auc: {}, best image auc: {} "
                    "at {} epoch".format(
                        cur_epoch,
                        pixel_auc,
                        img_auc,
                        self.best_pixel_added,
                        self.best_image_added,
                        self.best_epoch_added,
                    ),
                    flush=True,
                )
                ...
                if cur_epoch == self.end_epoch:
                    with open(file_name, "a+") as f:
                        f.write(
                            "category: {}, best pixel auc: {}, best image auc: {} \r\n".format(
