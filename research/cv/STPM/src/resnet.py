...
def _conv3x3(in_channel, out_channel, stride=1, use_se=False, res_base=False):
    ...
    if res_base:
        return nn.Conv2d(
            in_channel, out_channel, kernel_size=3, stride=stride, padding=1, pad_mode="pad", weight_init=weight
        )
    return nn.Conv2d(
...
def _conv1x1(in_channel, out_channel, stride=1, use_se=False, res_base=False):
    ...
    if res_base:
        return nn.Conv2d(
            in_channel, out_channel, kernel_size=1, stride=stride, padding=0, pad_mode="pad", weight_init=weight
        )
    return nn.Conv2d(
...
def _conv7x7(in_channel, out_channel, stride=1, use_se=False, res_base=False):
    ...
    if res_base:
        return nn.Conv2d(
            in_channel, out_channel, kernel_size=7, stride=stride, padding=3, pad_mode="pad", weight_init=weight
        )
    return nn.Conv2d(
...
class ResidualBlockBase(nn.Cell):
    ...
    def __init__(
        self, in_channel, out_channel, stride=1, use_se=False, se_block=False, res_base=True, use_batch_statistics=None
    ):
        ...
        if self.down_sample:
            self.down_sample_layer = nn.SequentialCell(
                [
                    _conv1x1(in_channel, out_channel, stride, use_se=use_se, res_base=self.res_base),
                    _bn(out_channel, res_base, use_batch_statistics=use_batch_statistics),
                ]
            )
...
class ResNet(nn.Cell):
    ...
    def _make_layer(
        self, block, layer_num, in_channel, out_channel, stride, use_se=False, se_block=False, use_batch_statistics=None
    ):
        """
        ...
        resnet_block = block(
            in_channel, out_channel, stride=stride, use_se=use_se, use_batch_statistics=use_batch_statistics
        )
        layers.append(resnet_block)
        if se_block:
            for _ in range(1, layer_num - 1):
                resnet_block = block(
                    out_channel, out_channel, stride=1, use_se=use_se, use_batch_statistics=use_batch_statistics
                )
                layers.append(resnet_block)
            resnet_block = block(
                out_channel,
