...
from src.tracker.multitracker_onnx import JDETracker
...
def getonnxmodel():
    model = onnxruntime.InferenceSession(opts.load_model, providers=['CUDAExecutionProvider'])
    return model
...
def eval_seq(opt, net, dataloader, data_type, result_filename, save_dir=None, show_image=True, frame_rate=30):
    ...
    for _, img, img0 in dataloader:
        ...
        blob = blob.asnumpy()
        inputs = {net.get_inputs()[0].name: blob}
        id_feature, dets = net.run(None, inputs)

        online_targets = tracker.update(id_feature, dets, meta)
        online_tlwhs = []
        online_ids = []
