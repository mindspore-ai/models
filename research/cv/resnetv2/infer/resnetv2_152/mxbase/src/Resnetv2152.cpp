...
APP_ERROR resnetv2152::Init(const InitParam &initParam) {
    ...
    model_resnetv2152 = std::make_shared<MxBase::ModelInferenceProcessor>();
    ret = model_resnetv2152->Init(initParam.modelPath, modelDesc_);
    if (ret != APP_ERR_OK) {
...
APP_ERROR resnetv2152::ReadBin(const std::string &path, std::vector<std::vector<float>> &dataset) {
...
APP_ERROR resnetv2152::VectorToTensorBase(const std::vector<std::vector<float>> &input,
...
APP_ERROR resnetv2152::Inference(const std::vector<MxBase::TensorBase> &inputs,
                                      std::vector<MxBase::TensorBase> &outputs) {
    auto dtypes = model_resnetv2152->GetOutputDataType();
    ...
    APP_ERROR ret = model_resnetv2152->ModelInference(inputs, outputs, dynamicInfo);
    auto endTime = std::chrono::high_resolution_clock::now();
    double costMs = std::chrono::duration<double, std::milli>(endTime - startTime).count();
    inferCostTimeMilliSec += costMs;
    if (ret != APP_ERR_OK) {
        LogError << "ModelInference resnetv2152 failed, ret=" << ret << ".";
        return ret;
    }
    return APP_ERR_OK;
}

APP_ERROR resnetv2152::Process(const std::string &image_path, const InitParam &initParam, std::vector<int> &outputs) {
