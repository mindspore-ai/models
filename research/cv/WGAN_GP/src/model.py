...
class DcganG(nn.Cell):
    def __init__(self, DIM):
        ...
        main.append(
            nn.Conv2dTranspose(
                self.DIM * 4, self.DIM * 2, KERNEL_SIZE, stride=STRIDE, weight_init="normal", pad_mode="same"
            )
        )
        ...
        main.append(
            nn.Conv2dTranspose(
                self.DIM * 2, self.DIM, KERNEL_SIZE, stride=STRIDE, weight_init="normal", pad_mode="same"
            )
        )
        main.append(nn.BatchNorm2d(self.DIM))
        main.append(nn.ReLU())

        main.append(nn.Conv2dTranspose(self.DIM, 3, KERNEL_SIZE, stride=STRIDE, weight_init="normal", pad_mode="same"))
        main.append(nn.Tanh())
        self.main = main
