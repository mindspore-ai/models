...
if __name__ == "__main__":
    ...
    dataset = create_dataset(
        args_opt.dataroot, args_opt.batchSize, args_opt.imageSize, 1, args_opt.workers, args_opt.device_target
    )
    ...
    def init_weight(net):
        for _, cell in net.cells_and_names():
            if isinstance(cell, (nn.Conv2d, nn.Conv2dTranspose)):
                ...
                cell.gamma.set_data(
                    init.initializer(
                        Tensor(np.random.normal(1, 0.02, cell.gamma.shape), mstype.float32), cell.gamma.shape
                    )
                )
    ...
    def save_image(img, img_path, IMAGE_SIZE):
        ...
        to_image = Image.new(
            "RGB",
            (
                IMAGE_COLUMN * IMAGE_SIZE + PADDING * (IMAGE_COLUMN + 1),
                IMAGE_ROW * IMAGE_SIZE + PADDING * (IMAGE_ROW + 1),
            ),
    ...
    # # setup optimizer
    optimizerD = nn.Adam(
        netD.trainable_params(), learning_rate=args_opt.lrD, beta1=args_opt.beta1, beta2=args_opt.beta2
    )
    optimizerG = nn.Adam(
        netG.trainable_params(), learning_rate=args_opt.lrG, beta1=args_opt.beta1, beta2=args_opt.beta2
    )
    ...
    for epoch in range(args_opt.niter):
        ...
        while i < length:
            ...
            t1 = time.time()
            print(
                "[%d/%d][%d/%d][%d] Loss_D: %f Loss_G: %f"
                % (epoch, args_opt.niter, i, length, gen_iterations, loss_D.asnumpy(), loss_G.asnumpy())
            )
            ...
            if gen_iterations % args_opt.save_iterations == 0:
                ...
                save_image(real, "{0}/real_samples.png".format(args_opt.experiment), args_opt.imageSize)
                save_image(
                    fake, "{0}/fake_samples_{1}.png".format(args_opt.experiment, gen_iterations), args_opt.imageSize
                )
