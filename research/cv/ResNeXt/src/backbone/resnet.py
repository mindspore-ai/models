...
class BasicBlock(nn.Cell):
    ...
    def __init__(
        self, in_channels, out_channels, stride=1, down_sample=None, use_se=False, platform="Ascend", **kwargs
    ):
...
class ResNet(nn.Cell):
    ...
    def _make_layer(self, block, out_channels, blocks_num, stride=1, use_se=False, platform="Ascend"):
        ...
        if stride != 1 or self.in_channels != out_channels * block.expansion:
            down_sample = _DownSample(self.in_channels, out_channels * block.expansion, stride=stride)
