...
class Predictor:
    ...
    def _predict_get_result(self, stream_name, unique_id):
        ...
        if result.errorCode != 0:
            raise Exception(f"GetResultWithUniqueId error." f"errorCode={result.errorCode}, msg={result.data.decode()}")
        return result
...
def result_encode(file_name, result):
    sep = ","
    pred_class_ids = sep.join(str(i.get("classId")) for i in result.get("MxpiClass", []))
