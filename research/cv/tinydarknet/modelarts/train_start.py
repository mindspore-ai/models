        ...
        if not os.path.exists(os.path.join(save_dir, config.modelarts_dataset_unzip_name)):
            ...
            if zip_isexist:
                ...
                for file_name in fz.namelist():
                    if i % data_print == 0:
                        print("Unzip percent: {}%".format(int(i * 100 / data_num)))
                    i += 1
                    fz.extract(file_name, save_dir)
...
@moxing_wrapper(pre_process=transfer_learning_modelarts_pre_process)
def run_export():
    """export function"""
    if config.dataset_name != 'imagenet':
...
def freeze_model():
    """ freeze model """
    # Find the ckpt file
    print("ckpt_save_dir:" + config.ckpt_save_dir)
    config.output_path = "/cache/train1"
    for curDir, dirs, files in os.walk(config.ckpt_save_dir):
        print(dirs)
        for file_name in files:
            if file_name.endswith('.ckpt'):
                config.checkpoint_path = os.path.join(curDir, file_name)
                print("get_config.checkpoint_path:", config.checkpoint_path)
                run_export()


def main():
    """train the model first, then freeze the model"""
    run_train()
    freeze_model()
