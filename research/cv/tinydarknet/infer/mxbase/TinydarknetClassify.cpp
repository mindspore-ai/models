...
APP_ERROR TinydarknetClassify::Init(const InitParam &initParam) {
...
APP_ERROR TinydarknetClassify::DeInit() {
    dvppWrapper_->DeInit();
    model_->DeInit();
    post_->DeInit();
...
APP_ERROR TinydarknetClassify::ReadImage(const std::string &imgPath, cv::Mat &imageMat) {
    imageMat = cv::imread(imgPath, cv::IMREAD_COLOR);
    return APP_ERR_OK;
}

APP_ERROR TinydarknetClassify::ResizeImage(const cv::Mat &srcImageMat, cv::Mat &dstImageMat) {
    static constexpr uint32_t resizeHeight = 291;
    static constexpr uint32_t resizeWidth = 291;

    cv::resize(srcImageMat, dstImageMat, cv::Size(resizeWidth, resizeHeight));
    return APP_ERR_OK;
}

APP_ERROR TinydarknetClassify::Crop(const cv::Mat &srcImageMat, cv::Mat &dstImageMat) {
    static cv::Rect rectOfImg(33, 33, 224, 224);
    dstImageMat = srcImageMat(rectOfImg).clone();
    return APP_ERR_OK;
}

APP_ERROR TinydarknetClassify::CVMatToTensorBase(const cv::Mat &imageMat, MxBase::TensorBase &tensorBase) {
...
APP_ERROR TinydarknetClassify::Inference(const std::vector<MxBase::TensorBase> &inputs,
...
APP_ERROR TinydarknetClassify::PostProcess(const std::vector<MxBase::TensorBase> &inputs,
...
APP_ERROR TinydarknetClassify::SaveResult(const std::string &imgPath,
...
APP_ERROR TinydarknetClassify::Process(const std::string &imgPath) {
    cv::Mat imageMat;
