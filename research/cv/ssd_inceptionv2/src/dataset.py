...
def create_ssd_dataset(mindrecord_file, batch_size=32, repeat_num=10, device_num=1, rank=0,
                       is_training=True, num_parallel_workers=8, use_multiprocessing=True):
...
def convert_anno(configs, number_path):
    """convert json"""
    number_path = number_path + '/number_id_val.txt'
    f = open(number_path, "r")
    number = f.readlines()
    f.close()
    anno_train_raw = os.path.join(configs.coco_root_raw, configs.instances_set.format('train2017'))
    anno_val_raw = os.path.join(configs.coco_root_raw, configs.instances_set.format('val2017'))
    anno_train = os.path.join(configs.coco_root, configs.instances_set.format('train'))
    anno_val = os.path.join(configs.coco_root, configs.instances_set.format('val'))
    if not os.path.exists(anno_train) or os.path.exists(anno_val):
        with open(anno_train_raw, 'r') as load_f:
            load_train = json.load(load_f)
        with open(anno_val_raw, 'r') as load_f:
            load_val = json.load(load_f)
        dic_val = {}
        dic_train = {}
        os.makedirs(configs.coco_root + '/annotations')
        dic_val['info'] = load_val['info']
        dic_train['info'] = load_train['info']
        temp_img_info_val = []
        temp_img_info_train = []
        for train_info_img in load_train['images']:
            flag = 0
            for i in range(len(number)):
                if int(number[i].strip('\n')) == train_info_img['id']:
                    flag = 1
                    temp_img_info_val.append(train_info_img)
            if flag == 0:
                temp_img_info_train.append(train_info_img)
            print(f"number:{train_info_img['id']}-train image info update")
        for val_info_img in load_val['images']:
            flag = 0
            for i in range(len(number)):
                if int(number[i].strip('\n')) == val_info_img['id']:
                    flag = 1
                    temp_img_info_val.append(val_info_img)
            if flag == 0:
                temp_img_info_train.append(val_info_img)
            print(f"number:{val_info_img['id']}-val image info update")
        dic_val['images'] = temp_img_info_val
        dic_train['images'] = temp_img_info_train
        temp_anno_info_val = []
        temp_anno_info_train = []
        for train_info_anno in load_train['annotations']:
            flag = 0
            for i in range(len(number)):
                if int(number[i].strip('\n')) == train_info_anno['image_id']:
                    flag = 1
                    temp_anno_info_val.append(train_info_anno)
            if flag == 0:
                temp_anno_info_train.append(train_info_anno)
            print(f"number:{train_info_anno['image_id']}-train anno info update")
        for val_info_anno in load_val['annotations']:
            flag = 0
            for i in range(len(number)):
                if int(number[i].strip('\n')) == val_info_anno['image_id']:
                    flag = 0
                    temp_anno_info_val.append(val_info_anno)
            if flag == 0:
                temp_anno_info_train.append(val_info_anno)
            print(f"number:{val_info_anno['image_id']}-val anno info update")
        dic_val['annotations'] = temp_anno_info_val
        dic_train['annotations'] = temp_anno_info_train
        dic_val['categories'] = load_val['categories']
        dic_train['categories'] = load_train['categories']
        os.system('touch' + anno_val)
        os.system('touch' + anno_train)
        with open(anno_val, "w") as f:
            json.dump(dic_val, f)
        with open(anno_train, "w") as f:
            json.dump(dic_train, f)
