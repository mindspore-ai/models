...
from src.res2net import Res2NetFea, Residual2BlockUsing
...
class YOLOv3(nn.Cell):
    """
     ...
     Args:
         backbone_shape: List. Res2Net output channels shape.
...
class YOLOV3Res2Net(nn.Cell):
    """
    Res2Net based YOLOV3 network.

    Args:
        is_training: Bool. Whether train or not.

    Returns:
        Cell, cell instance of Res2Net based YOLOV3 neural network.
    ...
    def __init__(self, is_training, config=default_config):
        super(YOLOV3Res2Net, self).__init__()
        self.config = config
        self.keep_detect = self.config.keep_detect
        self.tenser_to_array = P.TupleToArray()

        # YOLOv3 network
        self.feature_map = YOLOv3(backbone=Res2NetFea(Residual2BlockUsing, self.config.res2net_block,
                                                      self.config.res2net_in_channels,
                                                      self.config.res2net_out_channels,
                                                      False,
                                                      detect=True),
