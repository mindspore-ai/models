...
def convert_infer(img_path, json_path, result_path):
    ...
    for _ in json_list:
        ...
        h, w = img.shape[:2]
        scale = min(768.0/h, 1280.0/w)
        data = json.load(open(json_path+_))
        det_data = data['data']
        f = open(f'{result_path}/'+_.replace('.json', '.txt'), 'w')
        for bbox in det_data:
            im_mask = np.zeros((h, w), dtype=np.uint8)
            if int(bbox['classId']) > 0 or float(bbox["confidence"]) < 0.9:
                continue
            np_bbox = np.array([
                float(bbox["x0"]),
                float(bbox["y0"]),
            ...
            mask_data = bbox["mask"]
            mask_width = int(bbox["maskWidth"])
            mask_height = int(bbox["maskHeight"])
            mask = [255 if i == '1' else 0 for i in mask_data]
            mask = np.array(mask)
            mask = mask.reshape(mask_height, mask_width)
            mask = mask.astype(np.uint8)
            max_value = [w, h, w, h]
            [x1, y1, x2, y2] = [max(min(int(np_bbox[i]/scale), max_value[i]-1), 0) for i in range(len(np_bbox)-1)]
            ...
            temp = cv2.findContours(im_mask, cv2.RETR_CCOMP, cv2.CHAIN_APPROX_NONE)
            temp = temp[-2]
