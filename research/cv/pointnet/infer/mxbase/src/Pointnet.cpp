...
const uint32_t MAX_LENGTH = 7500;
...
APP_ERROR Pointnet::Init(const InitParam &initParam) {
...
APP_ERROR Pointnet::ReadTensorFromFile(const std::string &file, uint32_t *data, uint32_t size) {
...
APP_ERROR Pointnet::ReadInputTensor(const std::string &fileName, std::vector<MxBase::TensorBase> *inputs) {
    ...
    std::vector<uint32_t> shape = {1, 3, 2500};
    inputs->push_back(MxBase::TensorBase(memoryDataDst, false, shape, MxBase::TENSOR_DTYPE_FLOAT32));
    return APP_ERR_OK;
}


APP_ERROR Pointnet::Inference(const std::vector<MxBase::TensorBase> &inputs,
...
APP_ERROR  Pointnet::WriteResult(const std::string& imageFile, std::vector<MxBase::TensorBase> &outputs) {
    ...
    for (size_t i = 0; i < outputs.size(); ++i) {
        ...
        LogInfo << out_shape[0] << out_shape[1] << out_shape[2];

        std::string outFileName = infer_result_path + "/" + imageFile;
        FILE *outputFile = fopen(outFileName.c_str(), "wb");
        fwrite(netOutput, out_shape[0]*out_shape[1]*out_shape[2], sizeof(float), outputFile);
        fclose(outputFile);
...
APP_ERROR Pointnet::Process(const std::string &inferPath, const std::string &fileName) {
