...
parser.add_argument('--enable_modelarts', default=True, type=ast.literal_eval, help="use feature transform")
...
def export_models():
    ...
    ckpt_list = glob.glob(local_train_url + "/pointnet*.ckpt")
    if not ckpt_list:
    ...
    num_classes_ = 4
    classifier_ = PointNetDenseCls(k=num_classes_, feature_transform=args.feature_transform)
    load_checkpoint(ckpt_model, net=classifier_)
    input_data = np.random.uniform(0.0, 1.0, size=[1, 3, 2500]).astype(np.float32)
    export(classifier_, Tensor(input_data), file_name=os.path.join(local_train_url, 'pointnet'), \
           file_format=args.file_format)
    air_name = "pointnet." + args.file_format.lower()
    mox.file.copy(os.path.join(local_train_url, air_name), os.path.join(args.train_url, air_name))
...
if __name__ == "__main__":
    ...
    num_classes = dataset_generator.num_seg_classes  # 4
