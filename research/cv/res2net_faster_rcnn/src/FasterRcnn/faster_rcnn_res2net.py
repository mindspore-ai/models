...
from .res2net import Res2NetFea, Residual2BlockUsing
...
class Faster_Rcnn_Res2net(nn.Cell):
    """
    FasterRcnn Network.
    ...
    Examples:
        net = Faster_Rcnn_Res2net()
    """

    def __init__(self, config):
        super(Faster_Rcnn_Res2net, self).__init__()
        ...
        # Backbone res2net
        self.backbone = Res2NetFea(
            Residual2BlockUsing,
            config.res2net_block,
            config.res2net_in_channels,
            config.res2net_out_channels,
            False,
        )

        # Fpn
        self.fpn_ncek = FeatPyramidNeck(
            config.fpn_in_channels, config.fpn_out_channels, config.fpn_num_outs
        )
        ...
        # Proposal
        self.proposal_generator = Proposal(
            config,
            self.train_batch_size,
        ...
        self.decode = P.BoundingBoxDecode(
            max_shape=(config.img_height, config.img_width),
        ...
        self.rcnn = Rcnn(
            config,
            config.rcnn_in_channels
            * config.roi_layer.out_size
            * config.roi_layer.out_size,
            self.train_batch_size,
            self.num_classes,
        )
        ...
        self.device_type = (
            "Ascend" if context.get_context("device_target") == "Ascend" else "Others"
        )
    ...
    def roi_init(self, config):
        ...
        Examples:
            self.roi_init(config)
        """
        self.roi_align = SingleRoIExtractor(
            config,
            config.roi_layer,
    ...
    def test_mode_init(self, config):
        ...
        self.bbox_mask = Tensor(
            np.concatenate(
                (self.ones_mask, self.zeros_mask, self.ones_mask, self.zeros_mask),
                axis=1,
            )
        )
        ...
        self.test_iou_thr = Tensor(
            np.ones((self.rpn_max_num, 1)).astype(self.dtype) * config.test_iou_thr
        )
    ...
    def init_tensor(self, config):
        ...
        roi_align_index = [
            np.array(
                np.ones(
                    (config.num_expected_pos_stage2 + config.num_expected_neg_stage2, 1)
                )
                * i,
                dtype=self.dtype,
            )
    ...
    def proposal_generator_mode(self, cls_score, bbox_pred):
        if self.training:
    ...
    def get_box_label_mode(
            self, gt_labels, gt_valids, gt_bboxes, proposal_mask, proposal
    ):
        """get_box_label_mode"""
    ...
    def get_rois_mode(self, bboxes_tuple):
        """get_rois_mode"""
        if self.training:
        ...
        return rois, bboxes_all

    def roi_align_mode(self, rois, x):
        """roi_align_mode"""
        if self.training:
    ...
    def construct(self, img_data, img_metas, gt_bboxes, gt_labels, gt_valids):
        ...
        (
            ...
            x, img_metas, self.anchor_list, gt_bboxes, self.gt_labels_stage1, gt_valids
        )
        ...
        #         cls_score, bbox_pred, self.anchor_list
        #     )
        proposal, proposal_mask = self.proposal_generator_mode(cls_score, bbox_pred)
        ...
        (
            ...
            bboxes_tuple,
            mask_tuple,
        ) = self.get_box_label_mode(
            gt_labels, gt_valids, gt_bboxes, proposal_mask, proposal
        )
        ...
        #         bboxes_all = self.cast(bboxes_all, mstype.float16)
        #     rois = self.concat_1((self.roi_align_index_test_tensor, bboxes_all))
        rois, bboxes_all = self.get_rois_mode(bboxes_tuple)
        ...
        #         self.cast(x[3], mstype.float32),
        #     )
        roi_feats = self.roi_align_mode(rois, x)
    ...
    def multiclass_nms(self, boxes_all, scores_all, mask_all):
        ...
        for i in range(self.test_batch_size):
            ...
            for j in range(self.num_classes - 1):
                ...
                _reg_mask = self.cast(
                    self.tile(self.cast(_mask, mstype.int32), (1, 4)), mstype.bool_
                )
