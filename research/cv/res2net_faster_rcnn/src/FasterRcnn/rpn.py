...
class RPN(nn.Cell):
    ...
    def __init__(
        ...
        self.num_expected_total = Tensor(
            np.array(cfg_rpn.num_expected_neg * self.batch_size).astype(self.dtype)
        )
        self.num_bboxes = cfg_rpn.num_bboxes
    ...
    def construct(
            self, inputs, img_metas, anchor_list, gt_bboxes, gt_labels, gt_valids
    ):
        """construct"""
        loss_print = ()
        rpn_cls_score = ()
        ...
        if self.training:
            for i in range(self.batch_size):
                ...
                bbox_target, bbox_weight, label, label_weight = self.get_targets(
                    gt_bboxes_i,
                    gt_labels_i,
                    self.cast(valid_flag_list, mstype.bool_),
                    anchor_using_list,
                    gt_valids_i,
                )
            ...
            for i in range(self.num_layers):
                ...
                loss_cls_item = (
                    self.sum_loss(loss_cls_item, (0,)) / self.num_expected_total
                )
                ...
                loss_reg_item = (
                    self.sum_loss(loss_reg_item, (0,)) / self.num_expected_total
                )

                loss_total = (
                    self.rpn_loss_cls_weight * loss_cls_item
