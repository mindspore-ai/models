...
class BboxAssignSample(nn.Cell):
    ...
    def get_assigned_gt_inds4(self, max_overlaps_w_ac, overlaps, assigned_gt_inds4):
        """get_assigned_gt_inds4"""
        for j in range(self.num_gts):
            max_overlaps_w_ac_j = max_overlaps_w_ac[j : j + 1 : 1]
            overlaps_w_gt_j = self.squeeze(overlaps[j : j + 1 : 1, ::])

            pos_mask_j = self.logicaland(
                self.greaterequal(max_overlaps_w_ac_j, self.min_pos_iou),
    ...
    def construct(self, gt_bboxes_i, gt_labels_i, valid_mask, bboxes, gt_valids):
        ...
        neg_sample_iou_mask = self.logicaland(
            self.greaterequal(max_overlaps_w_gt, self.zero_thr),
        ...
        assigned_gt_inds4 = self.get_assigned_gt_inds4(max_overlaps_w_ac, overlaps, assigned_gt_inds4)
        ...
        pos_assigned_gt_index = pos_assigned_gt_index * self.cast(
            valid_pos_index, mstype.int32
        )
