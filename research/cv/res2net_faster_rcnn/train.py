...
# if config.backbone in ("res2net_v1.5_50", "res2net_v1_101", "res2net_v1_152"):
#     from src.FasterRcnn.faster_rcnn_res2net import Faster_Rcnn_res2net
# elif config.backbone == "res2net_v1_50":
#     from src.FasterRcnn.faster_rcnn_res2net50v1 import Faster_Rcnn_res2net
#     config.epoch_size = 20
if config.backbone in ("res2net_50", "res2net_101"):
    from src.FasterRcnn.faster_rcnn_res2net import Faster_Rcnn_Res2net
    config.epoch_size = 20
...
def train_fasterrcnn():
    ...
    net = Faster_Rcnn_Res2net(config=config)
    net = net.set_train()
    ...
    if load_path != "":
        ...
        key_mapping = {
            'down_sample_layer.1.beta':
            'bn_down_sample.beta',
            'down_sample_layer.1.gamma':
            'bn_down_sample.gamma',
            'down_sample_layer.0.weight':
            'conv_down_sample.weight',
            'down_sample_layer.1.moving_mean':
            'bn_down_sample.moving_mean',
            'down_sample_layer.1.moving_variance':
            'bn_down_sample.moving_variance',
        }
