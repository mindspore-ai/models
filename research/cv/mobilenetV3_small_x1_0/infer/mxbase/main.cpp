...
int main(int argc, char* argv[]) {
    if (argc <= 1) {
        LogWarn << "Please input image path, such as './mobilenetv3 ./image_dir 50000'.";
        return APP_ERR_OK;
    }
    ...
    initParam.modelPath = "../data/model/mobilenetv3_small.om";
    auto mobilenetv3_small = std::make_shared<Mobilenetv3Classify>();
    APP_ERROR ret = mobilenetv3_small->Init(initParam);
    if (ret != APP_ERR_OK) {
        LogError << "Mobilenetv3Classify init failed, ret=" << ret << ".";
        return ret;
    }
    ...
    for (auto & entry : fs::directory_iterator(imgDir)) {
        ...
        ret = mobilenetv3_small->Process(entry.path());
        if (ret != APP_ERR_OK) {
            LogError << "mobilenetv3_smallClassify process failed, ret=" << ret << ".";
            mobilenetv3_small->DeInit();
            return ret;
        }
    }
    mobilenetv3_small->DeInit();
