...
class StateMonitor(Callback):
    ...
    def __init__(self, data_size, tot_batch_size=None, lrs=None,
                 eval_interval=None, eval_offset=None, eval_engine=None, logger=None, summary_dir=None):
    ...
    def epoch_end(self, run_context):
        ...
        # SummaryRecord
        # self.summary_record.add_value('scalar', 'loss', Tensor(self.loss))
        ...
        if (self.epoch_num + 1) % self.eval_interval == self.eval_offset:
            ...
            if output is not None:
                if isinstance(output, list):
                    ...
                    # SummaryRecord
                    # self.summary_record.add_value('scalar', 'top1 accuracy', Tensor(float(output[0])))
                    # self.summary_record.add_value('scalar', 'top5 accuracy', Tensor(float(output[1])))
                    ...
                    # SummaryRecord
                    # self.summary_record.add_value('scalar', 'accuracy', Tensor(float(output)))

                    if float(output) > self.best_acc:
                        self.best_acc = float(output)

        # SummaryRecord
        # self.summary_record.record(self.epoch_num)
