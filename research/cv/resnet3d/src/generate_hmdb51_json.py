...
# ============================================================================
"""
generate annotation file for dataset hmdb51.
"""
import argparse
import json
...
def convert_csv_to_dict(csv_dir_path, split_indexs):
    """
    ...
    for file_path in csv_dir_path.iterdir():
        ...
        if 'split{}'.format(split_indexs) not in filename:
            continue
...
def convert_hmdb51_csv_to_json(csv_dir_path, split_index_hmdb51, video_dir_path, dst_json_path_hmdb51):
    """
    Convert csv annotation file to dict for dataset hmdb51.
    """

    labels = get_labels(csv_dir_path)
    database = convert_csv_to_dict(csv_dir_path, split_index_hmdb51)
    ...
    with dst_json_path_hmdb51.open('w') as dst_file:
...
if __name__ == '__main__':
    ...
    for split_index in range(1, 4):
        dst_json_path = args.dst_dir_path / \
            'hmdb51_{}.json'.format(split_index)
