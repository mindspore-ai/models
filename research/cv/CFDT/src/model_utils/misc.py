...
def inverse_sigmoid(x, eps=0.01):
    min_value = ms.Tensor(0, ms.float32)
    max_value = ms.Tensor(0.95, ms.float32)
    x = ops.clip_by_value(x, min_value, max_value)
    min_value2 = ms.Tensor(eps, ms.float32)
    x1 = ops.clip_by_value(x, min_value2, max_value)
    x2 = ops.clip_by_value((1 - x), min_value2, max_value)
