...
from src.model_util import g_type2class, g_class2type, g_type2onehotclass
from src.model_util import g_type_mean_size
from src.model_util import NUM_HEADING_BIN
...
class FrustumDataset():
    ...
    def __init__(self,
        ...
        if overwritten_data_path is None:
            overwritten_data_path = os.path.join(
                ROOT_DIR, 'kitti/frustum_caronly_%s.pickle' % (split))
        ...
        if from_rgb_detection:
            ...
            with open(overwritten_data_path, 'rb') as fp:
                ...
                self.frustum_angle_list = pickle.load(fp,
                                                      encoding="iso-8859-1")
                # print(self.frustum_angle_list[:50]) # same
    ...
    def __getitem__(self, index):
        ...
        if self.rotate_to_center:
            point_set = self.get_center_view_point_set(index)  #(n,4)
        else:
            point_set = self.input_list[index]

        # print(point_set.shape) #(n,4)
        # Resample
        choice = np.random.choice(point_set.shape[0],
                                  self.npoints,
                                  replace=True)
        point_set = point_set[choice, :]  #(n,4)
...
def get_rustumDataset_ms(split: str,
                         npoints=1024,
                         shuffle=False,
                         num_shards=None,
                         shard_id=None) -> ds.GeneratorDataset:
    dss = FrustumDataset(npoints=npoints,
                         split=split,
    ...
    _dataset = ds.GeneratorDataset(dss,
                                   cols,
                                   shuffle=shuffle,
...
def from_prediction_to_label_format(center, angle_class, angle_res, size_class,
                                    size_res, rot_angle):
