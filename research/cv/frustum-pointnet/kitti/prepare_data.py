...
def extract_frustum_data(idx_filename,
    ...
    '''
    type_whitelist = ['Car'] if type_whitelist is None else type_whitelist
...
def get_box3d_dim_statistics(idx_filename):
    ...
    for data_idx in data_idx_list:
        print('------------- ', data_idx)
        _ = dataset.get_calibration(data_idx)  # 3 by 4 matrix
        objects = dataset.get_label_objects(data_idx)
        for obj_idx in range(len(objects)):
            obj = objects[obj_idx]
...
def extract_frustum_data_rgb_detection(det_filename,
    ...
    for det_idx in range(len(det_id_list)):
        ...
        if ymax - ymin < img_height_threshold or len(
                pc_in_box_fov) < lidar_point_threshold:
            continue
        id_list.append(data_idx)
...
def write_2d_rgb_detection(det_filename, split, result_dir):
    ...
    '''
    _ = kitti_object(os.path.join(ROOT_DIR, 'dataset/KITTI/object'), split)
    det_id_list, det_type_list, det_box2d_list, det_prob_list = read_det_file(
        det_filename)
...
if __name__ == '__main__':
    ...
    if args.car_only:
        ...
        _type_whitelist = ['Car', 'Pedestrian', 'Cyclist']
        output_prefix = 'frustum_carpedcyc_'
    if args.gen_train:
        extract_frustum_data(os.path.join(BASE_DIR, 'image_sets/train.txt'),
                             'training',
    ...
    if args.gen_val:
        extract_frustum_data(os.path.join(BASE_DIR, 'image_sets/val.txt'),
                             'training',
                             os.path.join(BASE_DIR,
