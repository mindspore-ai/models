...
from GhostSR.EDSR_mindspore.edsr import EDSR4GhostSRMs
from .dataset import create_dataset_DIV2K
from .edsr import EDSR


def plt_tensor_img(tensor):
    img = tensor.asnumpy().astype('uint8').squeeze().transpose(1, 2, 0)
    plt.imshow(img)
    plt.show()
...
def init_env(cfg):
    ...
    # context.set_context(mode=ms.PYNATIVE_MODE, device_target=cfg.device_target)
...
def init_net(cfg):
    ...
    if cfg.network == 'EDSR_mindspore':
        net = EDSR(scale=cfg.scale,
                   ...
                   rgb_std=cfg.rgb_std)
    elif cfg.network == 'EDSR4GhostSRMs':
        net = EDSR4GhostSRMs(scale=cfg.scale)
    ...
    def unzip(zip_file, save_dir):
        ...
        if zip_isexist:
            ...
            for i, _file in enumerate(fz.namelist()):
                if i % data_print == 0:
                    print(
                        "[{1:>{0}}/{2:>{0}}] {3:>2}%  const time: {4:0>2}:{5:0>2}  unzipping {6}".format(
                            len_data_num,
                            i,
                            data_num,
                            int(i / data_num * 100),
...
def do_eval(eval_network, ds_val, metrics, cur_epoch=None):
    ...
    for i, (lr, hr) in enumerate(ds_val):
        ...
        step_begin = time.time()
        print(f"[{i + 1:>{setw}}/{total_step:>{setw}}] rank = {rank_id} result = {result}",
              flush=True)
    result = {k: m.eval(sync=False) for k, m in metrics.items()}
