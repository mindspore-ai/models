...
class ResNet(nn.Cell):
    ...
    def __init__(self,
        ...
        self.mlp = mlp
        if self.mlp:
            self.fc1 = _fc(out_channels[3], 2048)
            self.fc2 = _fc(2048, low_dim)
        else:
            self.fc = _fc(out_channels[3], low_dim)
        ...
        for _, cell in self.cells_and_names():
            if isinstance(cell, nn.Conv2d):
                n = cell.weight.shape[0] * cell.weight.shape[2] * cell.weight.shape[3]
                cell.weight.set_data(init.initializer(init.Normal(sigma=math.sqrt(2. / n)),
                                                      cell.weight.shape,
                                                      cell.weight.dtype))
            elif isinstance(cell, nn.Dense):
                cell.weight.set_data(init.initializer(init.HeUniform(),
                                                      cell.weight.shape,
                                                      cell.weight.dtype))
                if cell.bias is not None:
                    fan_in = cell.weight.shape[1]
...
def resnet50(class_num=10, mlp=False, low_dim=128):
    """
    ...
    return ResNet(ResidualBlock,
                  ...
                  [256, 512, 1024, 2048],
                  [1, 2, 2, 2],
                  class_num, mlp=mlp, low_dim=low_dim)
...
class Normalize(nn.Cell):
    def __init__(self, power=2):
        ...
        self.power = power
        self.op_power = mindspore.ops.Pow()
        self.op_reducesum = mindspore.ops.ReduceSum(keep_dims=True)
        self.op_div = mindspore.ops.Div()

    def construct(self, x):
        x_power = self.op_power(x, self.power)
        x_sum = self.op_reducesum(x_power, 1)
        x_power_again = self.op_power(x_sum, (1. / self.power))
        x_div = self.op_div(x, x_power_again)
        return x_div
