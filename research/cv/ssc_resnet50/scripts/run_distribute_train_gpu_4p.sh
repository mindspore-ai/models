...
usage() {
  ...
  echo "bash run_distribute_train_ascend_4p.sh ANNOTATION EXP_DIR PRE_TRAINED"
  echo "for example: bash run_distribute_train_ascend_4p.sh /path/to/annotation.json /path/to/save/folder /path/to/pre_trained.ckpt(option) "
...
bash run_distribute_train_model_gpu.sh 4 $BASE_MODEL_OUTPUT $PRE_TRAINED $ANNOTATION


# sorted data value by trained model
echo '--------------step2: start sorting data in a json-----------------'
BASE_MODEL="$BASE_MODEL_OUTPUT/model_last.ckpt"
bash select_sample_gpu.sh 4 $EXP_DIR $ANNOTATION $BASE_MODEL
...
bash run_distribute_train_model_gpu.sh 4 $FINAL_MODEL_OUTPUT $NEW_ANNOTATION $PRE_TRAINED
