...
def cal_acc_cifar10(result_path, label_path):
    ...
    for file in files:
        ...
        if os.path.isfile(full_file_path):
            result = np.fromfile(
                full_file_path, dtype=np.float32).reshape(result_shape)
            label_file = os.path.join(
                label_path, file.split(".bin")[0][:-2] + ".bin")
    ...
    print(
        f"Total data: {img_tot}, top1 accuracy: {top1_correct / img_tot}, top5 accuracy: {top5_correct / img_tot}.")
    ...
    for file in files:
        ...
        for batch in range(batch_size):
            ...
            if labels[img_ids_name+".JPEG"] in predict[:5]:
                top5 += 1
    print(
        f"Total data: {total_data}, top1 accuracy: {top1/total_data}, top5 accuracy: {top5/total_data}.")
