...
from src.res2net import conv_variance_scaling_initializer
...
def set_graph_kernel_context(run_platform, net_name):
    if run_platform == "GPU" and net_name == "res2net101":
...
def set_parameter():
    ...
    if config.mode_name == 'GRAPH':
        if target == "Ascend":
            # rank_save_graphs_path = os.path.join(
            #     config.save_graphs_path, "soma", str(os.getenv('DEVICE_ID')))
            ...
            context.set_context(
                mode=context.GRAPH_MODE, device_target=target, save_graphs=config.save_graphs)
    ...
    if config.run_distribute:
        if target == "Ascend":
            ...
            if config.net_name == "res2net50" or config.net_name == "se-res2net50":
                if config.boost_mode not in ["O1", "O2"]:
                    context.set_auto_parallel_context(
                        all_reduce_fusion_config=config.all_reduce_fusion_config)
            elif config.net_name in ["res2net101", "res2net152"]:
                context.set_auto_parallel_context(
                    all_reduce_fusion_config=config.all_reduce_fusion_config)
            init()
        # GPU target
    ...
    if config.pre_trained:
        if os.path.isdir(config.pre_trained):
            ckpt_save_dir = os.path.join(
                config.output_path, config.checkpoint_path, "ckpt_0")
    ...
    if config.pre_trained:
        if param_dict:
            if param_dict.get("epoch_num") and param_dict.get("step_num"):
                config.has_trained_epoch = int(
                    param_dict["epoch_num"].data.asnumpy())
                config.has_trained_step = int(
            ...
            if isinstance(cell, nn.Dense):
                if config.dense_init == "TruncatedNormal":
                    ...
                    weight = Tensor(np.reshape(
                        weight, (out_channel, in_channel)), dtype=cell.weight.dtype)
                    cell.weight.set_data(weight)
...
def init_lr(step_size):
    ...
    if config.optimizer == "Thor":
        ...
        if config.net_name in ("res2net50", "res2net152", "se-res2net50"):
...
def run_eval(target, model, ckpt_save_dir, cb):
    ...
    if config.run_eval:
        if config.eval_dataset_path is None or (not os.path.isdir(config.eval_dataset_path)):
            raise ValueError("{} is not a existing path.".format(
        ...
        eval_param_dict = {"model": model,
                           "dataset": eval_dataset, "metrics_name": "acc"}
...
def train_net():
    ...
    dist_eval_network = ClassifyCorrectCell(
        net) if config.run_distribute else None
    metrics = {"acc"}
    if config.run_distribute:
        metrics = {'acc': DistAccuracy(
            batch_size=config.batch_size, device_num=config.device_num)}
    if (config.net_name not in ("res2net50", "res2net101", "res2net152", "se-res2net50")) or \
    ...
    if config.optimizer == "Thor" and config.dataset == "imagenet2012":
        ...
        damping = get_thor_damping(
            0, config.damping_init, config.damping_decay, 70, step_size)
        split_indices = [26, 53]
        ...
        model = ConvertModelUtils().convert_to_thor_model(model=model, network=net, loss_fn=loss, optimizer=opt,
                                                          loss_scale_manager=loss_scale, metrics={
                                                              'acc'},
    ...
    if config.save_checkpoint:
        ckpt_append_info = [
            {"epoch_num": config.has_trained_epoch, "step_num": config.has_trained_step}]
        config_ck = CheckpointConfig(save_checkpoint_steps=config.save_checkpoint_epochs * step_size,
                                     keep_checkpoint_max=config.keep_checkpoint_max,
                                     append_info=ckpt_append_info)
        ckpt_cb = ModelCheckpoint(
            prefix="res2net", directory=ckpt_save_dir, config=config_ck)
        cb += [ckpt_cb]
    run_eval(target, model, ckpt_save_dir, cb)
    # train model
    if config.net_name == "se-res2net50":
