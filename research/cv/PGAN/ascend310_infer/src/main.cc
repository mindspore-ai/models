...
int main(int argc, char **argv) {
  ...
  std::cout << "Start predict input files:" << input0_files[0] << std::endl;

  auto input0 = ReadFileToTensor(input0_files[1]);
  auto input1 = ReadFileToTensor(input0_files[0]);
  ...
  if (ret != kSuccess) {
    std::cout << "Predict " << input0_files[0] << " failed." << std::endl;
    return 1;
  }
  startTimeMs = (1.0 * start.tv_sec * 1000000 + start.tv_usec) / 1000;
  endTimeMs = (1.0 * end.tv_sec * 1000000 + end.tv_usec) / 1000;
  costTime_map.insert(std::pair<double, double>(startTimeMs, endTimeMs));
  WriteResult(input0_files[1], outputs);
  double average = 0.0;
  int inferCount = 0;
