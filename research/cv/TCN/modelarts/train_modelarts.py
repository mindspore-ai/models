...
def train_net():
    ...
    if config.dataset_name == 'permuted_mnist':
        train_dataset = create_dataset(config.data_url + '/train', config.batch_size)
        test_dataset = create_dataset(config.data_url + '/test', config.batch_size)
    elif config.dataset_name == 'adding_problem':
        train_dataset = create_datasetAP(config.data_url, config.N_train, config.seq_length, 'train',
                                         config.batch_train)
        test_dataset = create_datasetAP(config.data_url, config.N_test, config.seq_length, 'test',
                                        config.batch_test)
    ...
    ckpoint_cb = ModelCheckpoint(prefix="checkpoint_tcn", directory=config.train_url, config=config_ck)
    eval_per_epoch = 1
    ...
    print("============ ckpt_file: " + config.ckpt_file)
    # export network
    if config.dataset_name == 'permuted_mnist':
        inputs = Tensor(np.ones([1, config.channel_size, config.image_height * config.image_width]),
                        mindspore.float32)
    elif config.dataset_name == 'adding_problem':
        inputs = Tensor(np.ones([1, config.channel_size, config.seq_length]), mindspore.float32)
    export(net, inputs, file_name=config.train_url + '/' + config.file_name, file_format='AIR')
