...
def ckpt_to_air(save_ckpt_path, path):
    ...
    export(net, input_arr, file_name=save_ckpt_path +'ddrnet', file_format="AIR")
...
def main():
    ...
    ckpt_save_dir = os.path.join(args.train_url, 'ckpt_' + str(rank) + '')
    ...
    path = os.path.join(ckpt_save_dir, 'DDRNet230'+'-'+str(args.epochs-args.start_epoch)+'_'+str(batch_num)+'.ckpt')
    print(ckpt_save_dir, path)
    ckpt_to_air(ckpt_save_dir, path)
