...
#include "DDRNetClassify.h"

APP_ERROR DDRNetClassify::Init(const InitParam &initParam) {
...
APP_ERROR DDRNetClassify::DeInit() {
    dvppWrapper_->DeInit();
    model_->DeInit();
    post_->DeInit();
...
APP_ERROR DDRNetClassify::ReadImage(const std::string &imgPath, cv::Mat &imageMat) {
    imageMat = cv::imread(imgPath, cv::IMREAD_COLOR);
    return APP_ERR_OK;
}

APP_ERROR DDRNetClassify::ResizeImage(const cv::Mat &srcImageMat, cv::Mat &dstImageMat) {
...
APP_ERROR DDRNetClassify::CVMatToTensorBase(const cv::Mat &imageMat, MxBase::TensorBase &tensorBase) {
...
APP_ERROR DDRNetClassify::Crop(const cv::Mat &srcImageMat, cv::Mat &dstImageMat) {
    static cv::Rect rectOfImg(16, 16, 224, 224);
    dstImageMat = srcImageMat(rectOfImg).clone();
    return APP_ERR_OK;
}

APP_ERROR DDRNetClassify::Inference(const std::vector<MxBase::TensorBase> &inputs,
...
APP_ERROR DDRNetClassify::PostProcess(const std::vector<MxBase::TensorBase> &inputs,
...
APP_ERROR DDRNetClassify::SaveResult(const std::string &imgPath,
...
APP_ERROR DDRNetClassify::Process(const std::string &imgPath) {
    cv::Mat imageMat;
