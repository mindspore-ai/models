...
int main(int argc, char* argv[]) {
    if (argc <= 1) {
        LogWarn << "Please input image path, such as './DDRNet image_dir'.";
        return APP_ERR_OK;
    }
    ...
    initParam.modelPath = "../data/model/DDRNet23.om";
    auto ddrnet = std::make_shared<DDRNetClassify>();
    APP_ERROR ret = ddrnet->Init(initParam);
    if (ret != APP_ERR_OK) {
        LogError << "DDRNetClassify init failed, ret=" << ret << ".";
        return ret;
    }
    ...
    for (auto &imgFile : imgFilePaths) {
        ret = ddrnet->Process(imgFile);
        if (ret != APP_ERR_OK) {
            LogError << "DDRNetClassify process failed, ret=" << ret << ".";
            ddrnet->DeInit();
            return ret;
        }
    }
    ...
    double fps = 1000.0 * imgFilePaths.size() / ddrnet->GetInferCostMilliSec();
