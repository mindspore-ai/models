...
int main(int argc, char **argv) {
  ...
  if (input0_files.empty()) {
    std::cout << "ERROR: input0 data empty." << std::endl;
    return 1;
  }

  if (input1_files.empty()) {
    std::cout << "ERROR: input1 data empty." << std::endl;
    return 1;
  }
  ...
  for (size_t i = 0; i < size; ++i) {
    ...
    std::cout << "input0 files:" << input0_files[i] << std::endl;
    std::cout << "input1 files:" << input1_files[i] << std::endl;
