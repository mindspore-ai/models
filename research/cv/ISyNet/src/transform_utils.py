...
def _rotate_level_to_arg(*inputs):
    # range [-30, 30]
    level = inputs[0]
...
def _enhance_level_to_arg(*inputs):
    # range [0.1, 1.9]
    level = inputs[0]
    return ((level / _MAX_LEVEL) * 1.8 + 0.1,)


def _shear_level_to_arg(*inputs):
    # range [-0.3, 0.3]
    level = inputs[0]
...
def _translate_abs_level_to_arg(*inputs):
    level = inputs[0]
...
def _translate_rel_level_to_arg(*inputs):
    # range [-0.45, 0.45]
    level = inputs[0]
...
def _posterize_original_level_to_arg(*inputs):
...
def _posterize_research_level_to_arg(*inputs):
...
def _posterize_tpu_level_to_arg(*inputs):
...
def _solarize_level_to_arg(*inputs):
    # range [0, 256]
    level = inputs[0]
    return (int((level / _MAX_LEVEL) * 256),)


def _solarize_add_level_to_arg(*inputs):
    # range [0, 110]
    level = inputs[0]
    return (int((level / _MAX_LEVEL) * 110),)


def _distort_level_to_arg(*inputs):
    level = inputs[0]
    return (int((level / _MAX_LEVEL) * 10 + 10),)


def _zoom_level_to_arg(*inputs):
    level = inputs[0]
    return ((level / _MAX_LEVEL) * 0.4,)


def _blur_level_to_arg(*inputs):
    level = inputs[0]
