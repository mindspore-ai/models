...
parser.add_argument("--loss_scale", type=int, default=8192, help='loss_scale, default: 8192')
parser.add_argument("--epoch_size", type=int, default=1200, help='epoch_size, default: 1200')
...
if __name__ == '__main__':
    ...
    if not os.path.isdir(args.data_url):
        os.makedirs(args.data_url)
    ...
    if args.is_load_pre:
        ...
        args.pre_trained = pre_ckpt_dir + "/" + glob.glob(pre_ckpt_dir + '/' + '*.ckpt')[0]
    else:
        args.pre_trained = ''
    ...
    ckpts = glob.glob(args.ckpt_path + '/' + '*.ckpt')
    for ckpt_name in ckpts:
        ckpt_path = os.path.join(args.ckpt_path, ckpt_name)
        param_dict = load_checkpoint(ckpt_path)
        load_param_into_net(net, param_dict)
        export_path = os.path.join(args.ckpt_path, ckpt_name[:-5])
        export(net, input_data, file_name=export_path, file_format=args.file_format)
