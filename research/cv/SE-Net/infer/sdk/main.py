...
def run():
    ...
    with open("../data/config/senet_opencv.pipeline", 'rb') as f:
        pipelineStr = f.read()
    ...
    for file_name in file_list:
        ...
        if load_dict.get('MxpiClass') is None:
            with open(os.path.join(res_dir_name, '{}.txt'.format(file_name[:-5])), 'w') as f_write:
                f_write.write("")
            continue
