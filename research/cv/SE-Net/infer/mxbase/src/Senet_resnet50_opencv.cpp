...
APP_ERROR Senet_resnet50_opencv::Init(const InitParam &initParam) {
    ...
    if (ret != APP_ERR_OK) {
        LogError << "Senet_Resnet50PostProcess init failed, ret=" << ret << ".";
        return ret;
    }
    return APP_ERR_OK;
}

APP_ERROR Senet_resnet50_opencv::DeInit() {
    model_->DeInit();
    post_->DeInit();
...
void Senet_resnet50_opencv::ReadImage(const std::string &imgPath, cv::Mat &imageMat) {
    imageMat = cv::imread(imgPath, cv::IMREAD_COLOR);
}

void Senet_resnet50_opencv::ResizeImage(const cv::Mat &srcImageMat, cv::Mat &dstImageMat) {
...
APP_ERROR Senet_resnet50_opencv::CVMatToTensorBase(const cv::Mat &imageMat, MxBase::TensorBase &tensorBase) {
...
APP_ERROR Senet_resnet50_opencv::Inference(const std::vector<MxBase::TensorBase> &inputs,
...
APP_ERROR Senet_resnet50_opencv::PostProcess(const std::vector<MxBase::TensorBase> &inputs,
...
APP_ERROR Senet_resnet50_opencv::SaveResult(const std::string &imgPath,
...
APP_ERROR Senet_resnet50_opencv::Process(const std::string &imgPath) {
    cv::Mat imageMat;
