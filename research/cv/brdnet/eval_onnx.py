...
def test(model_path):
    session, input_name = create_session(model_path, cfg.device_target)
    ...
    for file in file_list:
        ...
        img_clean = np.float32(np.expand_dims(np.transpose(img_clean, (2, 0, 1)), 0))
        img_test = np.float32(np.expand_dims(np.transpose(img_test, (2, 0, 1)), 0))

        y_predict = session.run(None, {input_name: img_test})[0]
        # print(y_predict.shape)
        img_clean = Tensor(img_clean, mindspore.float32)
        img_test = Tensor(img_test, mindspore.float32)
        y_predict = Tensor(y_predict, mindspore.float32)
    ...
    cfg.logger.info('Before denoise: Average PSNR_b = {0:.6f}, SSIM_b = {1:.6f};After denoise: Average PSNR = {2:.6f},'
                    ' SSIM = {3:.6f}'.format(psnr_avg_b, ssim_avg_b, psnr_avg, ssim_avg))
...
if __name__ == '__main__':
    ...
    test(cfg.onnx_name)  # onnx model url
