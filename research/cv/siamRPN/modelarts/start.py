...
parser.add_argument('--is_parallel', default=False, type=bool,
                    help='whether parallel or not parallel')

parser.add_argument('--is_cloudtrain', default=False,
                    type=bool, help='whether cloud or not')
...
parser.add_argument('--unzip_mode', default=0, type=int,
                    metavar='N', help='unzip mode:0:no unzip,1:tar,2:unzip')

parser.add_argument('--device_id', default=2, type=int,
                    metavar='N', help='number of device to run')

parser.add_argument('--checkpoint_path', default=r'./ckpt', type=str,
                    metavar='N', help='path of ckpt to save')

parser.add_argument('--pretrain_model', default='mindspore_alexnet.ckpt', type=str,
                    metavar='N', help='name of pretrain_model')

parser.add_argument('--max_epoches', default=50, type=int,
                    metavar='N', help='number of total epochs to run')

parser.add_argument('--batch_size', default=32, type=int,
                    metavar='N', help='number of per step')
...
def main(args):
    """ Model"""
    net = SiameseRPN(groups=args.batch_size, is_train=True)
    criterion = MultiBoxLoss(args.batch_size)
    args.checkpoint_path = args.data_url
    if config.check:
        checkpoint_path = os.path.join(
            args.checkpoint_path, args.pretrain_model)
    ...
    if not checkpoint_path is None:
        ...
        total_epoch = args.max_epoches - cur_epoch
    #dataloader
    ...
    if args.is_parallel:
        ...
        dataset = ds.GeneratorDataset(
            data_loader, ["template", "detection", "label"], shuffle=True)
    ...
    lr = adjust_learning_rate(
        config.start_lr, config.end_lr, args.max_epoches, dataset.get_dataset_size())
    ...
    class Print_info(Callback):
        ...
        def epoch_end(self, run_context):
            ...
            print("epoch time: %s, per step time: %s" %
                  (epoch_seconds, epoch_seconds/cb_params.batch_num))
    ...
    ckpt_cfg = CheckpointConfig(
        save_checkpoint_steps=dataset.get_dataset_size(), keep_checkpoint_max=51)
    if args.is_cloudtrain:
        ckpt_cb = ModelCheckpoint(
            prefix='siamRPN', directory=config.train_path+'/ckpt', config=ckpt_cfg)
    else:
        ckpt_cb = ModelCheckpoint(
            prefix='siamRPN', directory='./ckpt', config=ckpt_cfg)
...
def export_AIR(ckpt_path):
    ...
    ckpt_list = glob.glob(ckpt_path  + "/siam*.ckpt")
    print(ckpt_path)
    print(ckpt_list)
    if not ckpt_list:
    ...
    load_checkpoint(ckpt_model, net=net)
    net.set_train(False)
    ...
    export(net, *input_data, file_name='siamrpn', file_format="AIR")
...
if __name__ == '__main__':
    ...
    if Args.is_cloudtrain:
        ...
        # device_id = int(os.getenv('DEVICE_ID') if os.getenv(
        ...
        tar_command1 = "tar -zxf " + local_data_path + \
            "/ytb_vid_filter.tar.gz -C " + local_data_path + '/train/'
        zip_command1 = "unzip -o -q " + local_data_path + \
            "/train.zip -d " + local_data_path + '/train/'
        Args.checkpoint_path = local_data_path
    ...
    main(Args)
    if Args.is_cloudtrain:
        export_AIR(local_data_path + "/ckpt")
        mox.file.copy_parallel(src_url=local_data_path +
                               '/ckpt', dst_url=Args.train_url + '/ckpt')
        files = os.listdir()
        print("===>>>current Files:", files)
        mox.file.copy(src_url='siamrpn.air', dst_url=Args.train_url+'/siamrpn.air')
