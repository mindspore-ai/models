...
int main(int argc, char* argv[]) {
    ...
    auto r2p1d = std::make_shared<R2P1D>();
    APP_ERROR ret = r2p1d->Init(initParam);
    if (ret != APP_ERR_OK) {
        LogError << "R2P1D init failed, ret=" << ret << ".";
        return ret;
    }
    ...
    for (uint32_t i = 0; i < files.size(); i++) {
        ...
        ret = r2p1d->Process(input_data_path, files[i]);
        if (ret != APP_ERR_OK) {
            LogError << "R2P1D process failed, ret=" << ret << ".";
            r2p1d->DeInit();
            return ret;
        }
    }
