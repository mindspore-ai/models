...
APP_ERROR R2P1D::Init(const InitParam &initParam) {
...
APP_ERROR R2P1D::DeInit() {
    this->model_->DeInit();
    MxBase::DeviceManager::GetInstance()->DestroyDevices();
    return APP_ERR_OK;
}

APP_ERROR R2P1D::ReadTensorFromFile(const std::string &file, float *data) {
    if (data == NULL) {
...
APP_ERROR R2P1D::ReadInputTensor(const std::string &fileName, std::vector<MxBase::TensorBase> *inputs) {
...
APP_ERROR R2P1D::Inference(const std::vector<MxBase::TensorBase> &inputs,
...
APP_ERROR  R2P1D::WriteResult(const std::string &imageFile, std::vector<MxBase::TensorBase> outputs) {
    for (size_t i = 0; i < outputs.size(); ++i) {
        ...
        for (uint32_t j = 1; j < out_shape[1]; j++) {
            if (netOutput[j] > netOutput[max_index]) {
                max_index = j;
            }
        }
        std::cout << "The predicted category of:" << imageFile << "--->" << \
                  this->ucf101_label_names[max_index] << std::endl;
...
APP_ERROR R2P1D::Process(const std::string &inferPath, const std::string &fileName) {
