...
class SRData:
    ...
    def __init__(self, args, name='', train=True, benchmark=False):
        ...
        self.idx_scale = 0
        if benchmark:
            self._set_filesystem(os.path.join(args.dir_data, 'benchmark'))
        else:
    ...
    def _scan(self):
        ...
        print(f"[srdata.py] names_hr:{len(names_hr)}, names_lr:{len(names_lr)}")
    ...
    def _check_and_load(self, ext, img, f, verbose=True):
        ...
        if not os.path.isfile(f) or ext.find('reset') >= 0:
            ...
            with open(f, 'wb') as ff:
                pickle.dump(imageio.imread(img), ff)
