...
def results2json(dataset, results, out_file):
    ...
    if isinstance(results[0], list):
        ...
        with open(result_files['bbox'], "w") as file_handle:
            json.dump(json_results, file_handle)
        ...
        with open(result_files['bbox'], "w") as file_handle:
            json.dump(json_results[0], file_handle)
        with open(result_files['segm'], "w") as file_handle:
            json.dump(json_results[1], file_handle)
    elif isinstance(results[0], np.ndarray):
        json_results = proposal2json(dataset, results)
        result_files['proposal'] = '{}.{}.json'.format(out_file, 'proposal')
        with open(result_files['proposal'], "w") as file_handle:
            json.dump(json_results, file_handle)
    else:
