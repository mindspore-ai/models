...
# ============================================================================
"""FasterRcnn Infer based on ResNet50, add scores into output"""
...
from ..FasterRcnn.resnet50 import ResNetFea, ResidualBlockUsing
from ..FasterRcnn.bbox_assign_sample_stage2 import BboxAssignSampleForRcnn
from ..FasterRcnn.fpn_neck import FeatPyramidNeck
from ..FasterRcnn.proposal_generator import Proposal
from ..FasterRcnn.roi_align import SingleRoIExtractor
from ..FasterRcnn.anchor_generator import AnchorGenerator
...
class Faster_Rcnn_Resnet50(nn.Cell):
    ...
    def construct(self, img_data, img_metas, gt_bboxes, gt_labels, gt_valids):
        ...
        if self.training:
            ...
            if self.device_type == "Ascend":
                _bboxes_all = self.cast(bboxes_all, mstype.float16)
                tmp = self.roi_align_index_test_tensor.astype('float16')
                rois = self.concat_1((tmp, _bboxes_all))
            else:
    ...
    def multiclass_nms(self, boxes_all, scores_all, mask_all):
        ...
        for i in range(self.test_batch_size):
            ...
            for j in range(self.num_classes - 1):
                ...
                pd_scores_sorted = self.gather(pd_scores, topk_inds)
                ...
                pd_scores_sorted_n = self.gather(pd_scores_sorted, _index)
                ...
                res_scores_tuple += (pd_scores_sorted_n,)
            ...
            res_scores_start = self.concat(res_scores_tuple[:self.concat_start])
            ...
            res_scores_end = self.concat(res_scores_tuple[self.concat_start:self.concat_end])
            ...
            res_socres = self.concat((res_scores_start, res_scores_end))
            ...
            res_scores = self.reshape(res_socres, (1, reshape_size, self.num_classes))
        ...
        all_scores = self.concat(all_scores)
        return all_bboxes, all_labels, all_masks, all_scores
    ...
    def get_anchors(self, featmap_sizes):
        ...
        Returns:
            tuple: anchors of each image, valflags of each image
        """
...
class FasterRcnn_Infer(nn.Cell):
    def __init__(self, config, infer_image_size=(4608, 3288)):
        super(FasterRcnn_Infer, self).__init__(auto_prefix=False)
        ...
        # create a fixed image_meta for only using one input when inference
        self.infer_image_size = infer_image_size
        ratio_h, ratio_w = self.compute_ratio(config)
        self.image_metas = Tensor([self.infer_image_size[1], self.infer_image_size[0],
                                   ratio_h, ratio_w]).astype(mstype.float32)
        print('image metas: ', self.image_metas)

    def construct(self, img_data):
        output = self.network(img_data, self.image_metas, None, None, None)
        return output

    def compute_ratio(self, config):
        from ..config import FasterRcnnConfig
        if FasterRcnnConfig.keep_ratio:
            ori_width, ori_height = self.infer_image_size
            resize_ratio = config.img_width / ori_width
            if resize_ratio > config.img_height / ori_height:
                resize_ratio = config.img_height / ori_height
            ratio_h = ratio_w = resize_ratio
        else:
            ratio_h = FasterRcnnConfig.img_height / self.infer_image_size[1]
            ratio_w = FasterRcnnConfig.img_width / self.infer_image_size[0]
        return ratio_h, ratio_w


class FasterRcnn_Infer_Online(nn.Cell):
    def __init__(self, config):
        super(FasterRcnn_Infer_Online, self).__init__(auto_prefix=False)
        self.network = Faster_Rcnn_Resnet50(config)
        self.network.set_train(False)
        device_type = "Ascend" if context.get_context(
            "device_target") == "Ascend" else "Others"
    ...
    def construct(self, img_data, image_metas):
        output = self.network(img_data, image_metas, None, None, None)
        return output
