...
def set_default():
    ...
    config.outputs_dir = config.ckpt_path
...
def run_export(ckpt_name):
    """export model to ir file"""
    context.set_context(mode=context.GRAPH_MODE, device_target=config.device_target)
    network = YOLOv3Inference(config.test_img_shape)
    print("Loading ckpt from {}".format(ckpt_name))
    param_dict = load_checkpoint(ckpt_name)
    load_param_into_net(network, param_dict)
    shape = [config.batch_size, 3] + config.test_img_shape # [1,3,640,640]
    input_data = Tensor(np.zeros(shape), mstype.float32)
    export(network, input_data, file_name='{0}/{1}'.format(config.output_path, config.file_name),
           file_format=config.file_format)
    mox.file.copy_parallel(src_url=config.output_path, dst_url=config.train_url)
...
def run_train():
    ...
    if config.rank_save_ckpt_flag:
        ...
        print('save_CKPT_path:******', save_ckpt_path)
    ...
    for i, data in enumerate(data_loader, i_start):
        ...
        if config.need_profiler and profiler is not None:
            if i == 10:
                profiler.analyse()
                break
    config.ckpt_name = ckpt_cb.latest_ckpt_file_name
    print('latest_ckpt_file_name:*************', config.ckpt_name)
...
if __name__ == "__main__":
    run_train()
    print('name&type', config.ckpt_name, type(config.ckpt_name))
    # frozen to air file
    run_export(config.ckpt_name)
