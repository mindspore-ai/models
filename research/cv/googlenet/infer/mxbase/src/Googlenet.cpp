...
APP_ERROR Googlenet::Init(const InitParam &initParam) {
    ...
    if (ret != APP_ERR_OK) {
        LogError << "GooglenetPostProcess init failed, ret=" << ret << ".";
        return ret;
    }
...
APP_ERROR Googlenet::ReadImage(const std::string &imgPath, cv::Mat &imageMat) {
    imageMat = cv::imread(imgPath, cv::IMREAD_COLOR);
    LogInfo << "data" << imageMat.size();
    return APP_ERR_OK;
}

void Googlenet::ResizeImage(const cv::Mat &srcImageMat, cv::Mat &dstImageMat) {
...
void Googlenet::CropImage(const cv::Mat &srcImageMat, cv::Mat &dstImageMat) {
    static cv::Rect centerCrop((262 - 224)/2, (262 - 224)/2, 224, 224);
    dstImageMat = srcImageMat(centerCrop).clone();
}

APP_ERROR Googlenet::CVMatToTensorBase(const cv::Mat &imageMat, MxBase::TensorBase &tensorBase) {
...
APP_ERROR Googlenet::Inference(const std::vector<MxBase::TensorBase> &inputs,
...
APP_ERROR Googlenet::PostProcess(const std::vector<MxBase::TensorBase> &inputs,
...
APP_ERROR Googlenet::SaveResult(const std::string &imgPath,
...
APP_ERROR Googlenet::Process(const std::string &imgPath) {
    cv::Mat imageMat;
