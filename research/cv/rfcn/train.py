...
"""train RFCN and get checkpoint files."""
...
from src.dataset import data_to_mindrecord_byte_image, create_rfcn_dataset
...
def train_rfcn_():
    ...
    # and the file name is Rfcn_coco.mindrecord0, 1, ... file_num.
    if config.dataset == "coco":
        prefix = "Rfcn_coco.mindrecord"
    else:
        prefix = "Rfcn_other.mindrecord"
    ...
    # When create MindDataset, using the fitst mindrecord file, such as rfcn.mindrecord0.
    dataset = create_rfcn_dataset(config, mindrecord_file, batch_size=config.batch_size,
...
def train_rfcn():
    """ train_rfcn """
    dataset_size, dataset = train_rfcn_()
    net = Rfcn_Resnet(config=config)
    net = net.set_train()
    ...
    if rank == 0 and config.save_checkpoint:
        ...
        ckpoint_cb = ModelCheckpoint(prefix='rfcn', directory=save_checkpoint_path, config=ckptconfig)
        cb += [ckpoint_cb]
