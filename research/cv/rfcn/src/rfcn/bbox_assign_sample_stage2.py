...
# ============================================================================
"""Rfcn positive and negative sample screening for Rcnn."""
...
class BboxAssignSampleForLoss(nn.Cell):
    """
    ...
    Examples:
        BboxAssignSampleForLoss(config, 2, 1024, True)
    """

    def __init__(self, config, batch_size, num_bboxes, add_gt_as_proposals):
        super(BboxAssignSampleForLoss, self).__init__()
        cfg = config
    ...
    def construct(self, gt_bboxes_i, gt_labels_i, valid_mask, bboxes, gt_valids):
        """bbox_assign_sample_stage2 construct"""

        # Convert the coordinates of gT_bboxes_i to -1
        ...
        # Replace the illegal coordinates in bboxes with -2
        ...
        # Calculate the IOU value of the rPN-generated box and the real box
        overlaps = self.iou(bboxes, gt_bboxes_i)

        # Match each RPN generated box with the label box that intersects the largest
        max_overlaps_w_gt_index, max_overlaps_w_gt = self.max_gt(overlaps)
        _, max_overlaps_w_ac = self.max_anchor(overlaps)

        # The mask of the negative sample in the box generated by RPN is calculated. Max_overlaps_w_gt greater than
        # or equal to 0 and less than 0.5 is used as the negative sample
        ...
        # Find positive sample mask IOU greater than or equal to 0.5
        ...
        for j in range(self.num_gts): # 128
        ...
        # Get the index position of the positive sample
        ...
        # Get neg index
        # 512 negative samples were randomly selected
        ...
        # compose
        # Box of positive sample is selected according to pos_index
        pos_bboxes_ = self.gatherND(bboxes, pos_index)
        # Pick the box with negative samples according to neg_index
        ...
        # Put the box with the positive sample and the box with the negative sample together
