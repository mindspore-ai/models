...
def create_coco_label(is_training, config):
    ...
    for img_id in image_ids:
        ...
        if not os.path.isfile(image_path):
            print(file_name + " is in annotations but not exist")
            continue
...
def data_to_mindrecord_byte_image(config, dataset="coco", is_training=True, prefix="rfcn.mindrecord", file_num=8):
    ...
    if dataset == "coco":
        ...
        image_files, image_anno_dict = create_train_data_from_txt(config.image_dir, config.anno_train_path)

    rfcn_json = {
        "image": {"type": "bytes"},
        "annotation": {"type": "int32", "shape": [-1, 6]},
    }
    writer.add_schema(rfcn_json, "rfcn_json")

    i = 0
    total = len(image_files)
    for image_name in image_files:
        # Here try block is to find corrupted images (Coco2014 has corrupted images)
        img = Image.open(image_name)
        try:
            img = np.asarray(img)
        except RuntimeError:
            print("image error： " + image_name)
            continue
        i = i + 1
        ...
        if i % 100 == 0:
            print(str(i) + "/" + str(total) + " write success")
    print(str(i) + "/" + str(total) + " write success")
    writer.commit()


def create_rfcn_dataset(config, mindrecord_file, batch_size=2, device_num=1, rank_id=0, is_training=True,
                        num_parallel_workers=8, python_multiprocessing=False):
    """Create Rfcn dataset with MindDataset."""
    cv2.setNumThreads(0)
