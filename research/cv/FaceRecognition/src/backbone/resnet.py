...
class SEBlock(Cell):
    ...
    def __init__(self, channel, reduction=16, act_type="relu"):
        ...
        self.act_layer = nn.PReLU(channel // reduction) if act_type == "prelu" else P.ReLU()
...
class IRBlock(Cell):
    ...
    def __init__(
        self, inplanes, planes, stride=1, downsample=None, use_se=1, pre_bn=1, use_inference=0, act_type="relu"
    ):
...
class MakeLayer(Cell):
    ...
    def __init__(self, block, inplanes, planes, blocks, args, stride=1):
        ...
        if stride != 1 or self.inplanes != planes * block.expansion:
            self.downsample = DownSample(self.inplanes, planes, block.expansion, stride, use_inference=args.inference)
...
class FaceResNet(Cell):
    ...
    def __init__(self, block, layers, args):
        ...
        self.layer1 = MakeLayer(block, planes=64, inplanes=self.inplanes, blocks=layers[0], stride=2, args=args)
        self.inplanes = 64
        self.layer2 = MakeLayer(block, planes=128, inplanes=self.inplanes, blocks=layers[1], stride=2, args=args)
        self.inplanes = 128
        self.layer3 = MakeLayer(block, planes=256, inplanes=self.inplanes, blocks=layers[2], stride=2, args=args)
        self.inplanes = 256
        self.layer4 = MakeLayer(block, planes=512, inplanes=self.inplanes, blocks=layers[3], stride=2, args=args)
        ...
        for _, cell in self.cells_and_names():
            if isinstance(cell, nn.Conv2d):
                cell.weight.set_data(
                    initializer(me_init.ReidKaimingUniform(a=math.sqrt(5), mode="fan_out"), cell.weight.shape)
                )
                if cell.bias is not None:
                    cell.bias.set_data(initializer("zeros", cell.bias.shape))
            elif isinstance(cell, nn.Dense):
                cell.weight.set_data(
                    initializer(me_init.ReidKaimingNormal(a=math.sqrt(5), mode="fan_out"), cell.weight.shape)
                )
                if cell.bias is not None:
                    cell.bias.set_data(initializer("zeros", cell.bias.shape))
