...
def parse_arguments():
    """parse_arguments"""
    global args
    parser = argparse.ArgumentParser(description="MindSpore CCT Training")
    ...
    parser.add_argument("--cct_config", help="Config file to use (see configs dir)", default=None, required=True)
...
def get_config():
    ...
    print(f"=> Reading YAML config from {args.cct_config}")
    # load yaml file
    if args.run_modelarts:
        import moxing as mox
        if not args.cct_config.startswith("obs:/"):
            args.cct_config = "obs:/" + args.cct_config
        with mox.file.File(args.cct_config, 'r') as f:
            yaml_txt = f.read()
    else:
        yaml_txt = open(args.cct_config).read()
