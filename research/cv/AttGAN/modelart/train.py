...
def parse(arg=None):
    ...
    parser.add_argument('--data_path', dest='data_path', type=str, default='/cache/data/img_align_celeba')  # 3.8
    parser.add_argument('--attr_path', dest='attr_path', type=str, default='/cache/data/list_attr_celeba.txt')
    ...
    parser.add_argument('--save_interval', dest='save_interval', type=int, default=1)
    ...
    parser.add_argument('--train_url', type=str, help='the path model saved')  # 3.8
    parser.add_argument('--data_url', type=str, help='the training data')  # 3.8
...
os.makedirs(join('/cache/train/', args.experiment_name), exist_ok=True)
os.makedirs(join('/cache/train/', args.experiment_name, 'checkpoint'), exist_ok=True)
with open(join('/cache/train/', args.experiment_name, 'setting.txt'), 'w') as f:
    f.write(json.dumps(vars(args), indent=4, separators=(',', ':')))

if __name__ == '__main__':

    # Define dataloader
    moxing.file.copy_parallel(src_url=args.data_url, dst_url='/cache/data')
    ...
    if rank == 0:
        local_train_url = os.path.join('/cache/train/', args.experiment_name)
    ...
    for epoch in range(args.epochs):
        ...
        for data in progressbar(train_loader, train_iter):
            ...
            if epoch >= 0 and (it + 1) % args.save_interval == 0 and rank == 0:
        ...
        export(gen, *(input_array, input_label), file_name='/cache/train/' + 'generator', file_format="AIR")
    moxing.file.copy_parallel(src_url='/cache/train/', dst_url=args.train_url)  # 3.8
