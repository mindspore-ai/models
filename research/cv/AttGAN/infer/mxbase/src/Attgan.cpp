...
namespace {
    ...
    const uint32_t LAST_WIDTH = 1792;
...
APP_ERROR Attgan::Init(const InitParam &initParam) {
...
APP_ERROR Attgan::DeInit() {
    model_->DeInit();
    MxBase::DeviceManager::GetInstance()->DestroyDevices();
    return APP_ERR_OK;
}

APP_ERROR Attgan::ReadImage(const std::string &imgPath, cv::Mat *imageMat) {
    *imageMat = cv::imread(imgPath, cv::IMREAD_COLOR);
    return APP_ERR_OK;
}

APP_ERROR Attgan::ResizeImage(const cv::Mat &srcImageMat, cv::Mat *dstImageMat) {
...
APP_ERROR Attgan::CVMatToTensorBase(const cv::Mat& imageMat, MxBase::TensorBase *tensorBase) {
    uint32_t dataSize = 1;
...
APP_ERROR Attgan::Inference(const std::vector<MxBase::TensorBase> &inputs,
...
APP_ERROR Attgan::PostProcess(std::vector<MxBase::TensorBase> outputs, cv::Mat *lastImg, int i) {
    ...
    for (size_t c = 0; c < C; c++) {
        for (size_t h = 0; h < H; h++) {
            for (size_t w = 0; w < W; w++) {
                float tmpNum = *(outputPtr + c * (H * W) + h * W + w) * NORMALIZE_STD + NORMALIZE_MEAN;
                lastImg->at<cv::Vec3b>(h, w + text_i)[c] = static_cast<int>(tmpNum);
            }
        }
    }
...
APP_ERROR Attgan::SaveResult(const cv::Mat &lastImg, const std::string &imgName) {
    ...
    cv::Mat lastImg_rgb(OUTPUT_HEIGHT, LAST_WIDTH, CV_8UC3);
    cv::cvtColor(lastImg, lastImg_rgb, cv::COLOR_BGR2RGB);
    cv::imwrite(savePath_ + "/" + imgName, lastImg_rgb);
    return APP_ERR_OK;
}

APP_ERROR Attgan::GetImageLabel(std::vector<float> label, MxBase::TensorBase *imgLabels) {
...
APP_ERROR Attgan::Process(const std::string &imgPath, const std::string &imgName,
                           const std::vector<float> &label, int i) {
    cv::Mat imageMat;
    ...
    static cv::Mat lastImg(OUTPUT_HEIGHT, LAST_WIDTH, CV_8UC3);
    ret = PostProcess(outputs, &lastImg, i);
    if (ret != APP_ERR_OK) {
        LogError << "PostProcess failed, ret=" << ret << ".";
        return ret;
    }
    if (i == 13) {
    ret = SaveResult(lastImg, imgName);
    }
