...
def extract_feature(model, input_name, dataset):
    ...
    for idx, data in enumerate(dataset.create_dict_iterator(output_numpy=False)):
        ...
        for i in range(2):
            ...
            outputs = model.run(None, dict(zip(input_name, input_data)))
...
if __name__ == '__main__':
    ...
    session1, input_names1 = create_session(args_opt.ckpt_url, 'CPU')
    print("load onnx success")

    test_feature, test_label, test_camera = extract_feature(session1, input_names1, test_dataset)
    query_feature, query_label, query_camera = extract_feature(session1, input_names1, query_dataset)
