...
# dataset
parser.add_argument("--train_LR_path", type=str, default='/DIV2K/DIV2K_train_LR_bicubic/X4_sub')
parser.add_argument("--train_GT_path", type=str, default='/DIV2K/DIV2K_train_HR_sub')
parser.add_argument("--val_PSNR_LR_path", type=str, default='/Set5/LRbicx4')
parser.add_argument("--val_PSNR_GT_path", type=str, default='/Set5/GTmod12')
parser.add_argument("--val_GAN_LR_path", type=str, default='/Set14/LRbicx4')
parser.add_argument("--val_GAN_GT_path", type=str, default='/Set14/GTmod12')
...
def export_AIR(args_opt, ckpt_path):
    """start modelarts export"""
    ckpt_model = os.path.join(ckpt_path, "gan_generator_best.ckpt")
    context.set_context(mode=context.GRAPH_MODE, device_target="Ascend")
    print(args_opt.device_id)
    #context.set_context(device_id=args_opt.device_id)

    net_ = get_generator(3, 3)
    param_dict_ = load_checkpoint(ckpt_model)
    load_param_into_net(net_, param_dict_)
    net_.set_train(False)
    input_shp = [1, 3, 200, 200]
    input_array = Tensor(np.random.uniform(-1.0, 1.0, size=input_shp).astype(np.float32))
    G_file = "{}_model".format(args_opt.file_name)
    path = os.path.join(ckpt_path, G_file)
    print(path)
    export(generator, input_array, file_name=path, file_format=args_opt.file_format)
...
if __name__ == '__main__':
    ...
    if args.modelArts:
        ...
        train_LR_path = os.path.join(device_data_url, args.train_LR_path)
        train_GT_path = os.path.join(device_data_url, args.train_GT_path)
        val_PSNR_LR_path = os.path.join(device_data_url, args.val_PSNR_LR_path)
        val_PSNR_GT_path = os.path.join(device_data_url, args.val_PSNR_GT_path)
        val_GAN_LR_path = os.path.join(device_data_url, args.val_GAN_LR_path)
        val_GAN_GT_path = os.path.join(device_data_url, args.val_GAN_GT_path)
        vgg_ckpt = os.path.join(device_data_url, args.vgg_ckpt)
        ...
        val_PSNR_LR_path = args.val_PSNR_LR_path
        val_PSNR_GT_path = args.val_PSNR_GT_path
        val_GAN_LR_path = args.val_GAN_LR_path
        val_GAN_GT_path = args.val_GAN_GT_path
        vgg_ckpt = args.vgg_ckpt
    ...
    train_ds = create_traindataset(args.train_batch_size, train_LR_path, train_GT_path, args.upscale_factor,
                                   args.image_size, num_shards, shard_id)
    test_psnr_ds = create_testdataset(args.val_batch_size, val_PSNR_LR_path, val_PSNR_GT_path)
    ...
    # for esrgan
    test_gan_ds = create_testdataset(args.val_batch_size, val_GAN_LR_path, val_GAN_GT_path)
    ...
    generator_loss = GeneratorLoss(discriminator, generator, vgg_ckpt, args)
    ...
    export_AIR(args, local_train_ckpt_path)
