...
if __name__ == '__main__':
    ...
    def create_session(checkpoint_path, target_device):
        ...
        sessions = ort.InferenceSession(checkpoint_path, providers=providers)
        input_names = sessions.get_inputs()[0].name
        return sessions, input_names

    session, input_name = create_session('./posenet.onnx', 'GPU')
    ...
    for step, item in enumerate(dataset.create_dict_iterator(output_numpy=True)):
        ...
        p1_x, p1_q, p2_x, p2_q, p3_x, p3_q = session.run(None, {input_name: Tensor(image).asnumpy()})
