...
from src.eval_utils import coco_metrics as metrics

batch_size = 1
parser = argparse.ArgumentParser(description="refinedet acc calculation")
...
config = get_config("refinedet_vgg16_320", "coco")
...
def get_result(result_path, img_id_file_path):
    ...
    print(len(pred_data))
    mAP = metrics(pred_data, anno_json, config)
    print(f" mAP:{mAP}")
