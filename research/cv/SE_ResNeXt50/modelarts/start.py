...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_path', type=str, default="/cache/dataset", help='Data path')
    parser.add_argument("--train_path", type=str, default="/cache/result")
    ...
    if args_opt.dataset_name == "imagenet":
        data_url = args_opt.data_path
        dataset = create_dataset_imagenet(data_url, 1)
    else:
        raise ValueError("Unsupported dataset.")
    ...
    ckpt_save_dir = args_opt.train_path
    ...
    if device_id == 0:
        ...
        local_train_url = "./checkpoint"
        input_array = Tensor(np.zeros([cfg.batch_size, 3, 224, 224], np.float32))
        file_name = os.path.join(local_train_url, "SE_ResNeXt")
        export(net, input_array, file_name=file_name, file_format="AIR")

    if device_id == 0:
        mox.file.copy_parallel(local_train_url, args_opt.train_path)
