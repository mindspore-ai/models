...
int main(int argc, char* argv[]) {
    if (argc <= 1) {
        LogWarn << "Please input image path, such as './SEResNeXt image_dir'.";
        return APP_ERR_OK;
    }
    ...
    initParam.modelPath = "../data/model/seresnext.om";
    auto SEResNeXt = std::make_shared<SEResNeXtClassifyOpencv>();
    APP_ERROR ret = SEResNeXt->Init(initParam);
    if (ret != APP_ERR_OK) {
        LogError << "SEResNeXtClassify init failed, ret=" << ret << ".";
        return ret;
    }
    ...
    for (auto &imgFile : imgFilePaths) {
        ret = SEResNeXt->Process(imgFile);
        if (ret != APP_ERR_OK) {
            LogError << "SEResNeXtClassify process failed, ret=" << ret << ".";
            SEResNeXt->DeInit();
            return ret;
        }
    }
    ...
    double fps = 1000.0 * imgFilePaths.size() / SEResNeXt->GetInferCostMilliSec();
