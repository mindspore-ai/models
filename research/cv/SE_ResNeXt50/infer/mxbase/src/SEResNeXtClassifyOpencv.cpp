...
#include "SEResNeXtClassifyOpencv.h"

namespace localParameter {
    const uint32_t VECTOR_FIRST_INDEX = 0;
    const uint32_t VECTOR_SECOND_INDEX = 0;
    const uint32_t VECTOR_THIRD_INDEX = 0;
    const uint32_t VECTOR_FOURTH_INDEX = 0;
    const uint32_t VECTOR_FIFTH_INDEX = 0;
}

APP_ERROR SEResNeXtClassifyOpencv::Init(const InitParam &initParam) {
    ...
    if (ret != APP_ERR_OK) {
        LogError << "SEResNeXtPostProcess init failed, ret=" << ret << ".";
        return ret;
    }
    return APP_ERR_OK;
}

APP_ERROR SEResNeXtClassifyOpencv::DeInit() {
    dvppWrapper_->DeInit();
    model_->DeInit();
    post_->DeInit();
...
APP_ERROR SEResNeXtClassifyOpencv::ReadImage(const std::string &imgPath, cv::Mat &imageMat) {
    imageMat = cv::imread(imgPath, cv::IMREAD_COLOR);
    return APP_ERR_OK;
}

APP_ERROR SEResNeXtClassifyOpencv::ResizeImage(const cv::Mat &srcImageMat, cv::Mat &dstImageMat) {
...
APP_ERROR SEResNeXtClassifyOpencv::CVMatToTensorBase(const cv::Mat &imageMat, MxBase::TensorBase &tensorBase) {
...
APP_ERROR SEResNeXtClassifyOpencv::Crop(const cv::Mat &srcImageMat, cv::Mat &dstImageMat) {
    static cv::Rect rectOfImg(16, 16, 224, 224);
    dstImageMat = srcImageMat(rectOfImg).clone();
    return APP_ERR_OK;
}

APP_ERROR SEResNeXtClassifyOpencv::Inference(const std::vector<MxBase::TensorBase> &inputs,
...
APP_ERROR SEResNeXtClassifyOpencv::PostProcess(const std::vector<MxBase::TensorBase> &inputs,
...
APP_ERROR SEResNeXtClassifyOpencv::SaveResult(const std::string &imgPath,
...
APP_ERROR SEResNeXtClassifyOpencv::Process(const std::string &imgPath) {
    cv::Mat imageMat;
