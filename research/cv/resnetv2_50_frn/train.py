...
from src.resnetv2_50_frn import preact_resnet50_frn as resnetv2_50_frn
...
def train():
    ...
    # network
    net = resnetv2_50_frn(num_classes=config.num_classes, method=config.method)
    if resume != '':
    ...
    lr = warmup_cosine_annealing_lr(lr=config.lr_init, max_epoch=epoch_size,
    ...
    ckpoint_cb = ModelCheckpoint(prefix=f"resnetv2-50-frn-rank{config.rank}",
