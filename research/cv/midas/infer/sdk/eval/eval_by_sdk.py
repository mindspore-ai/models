...
import numpy as np
from eval.util import depth_read_kitti, depth_read_sintel, BadPixelMetric
...
def get_eval_result(img_path, result_path, data_name):
    ...
    if data_name == "Kitti":
        ...
        print("result_file path is ", id_feature_result_file)
    elif data_name == "Sintel":
        seq = all_path[-2]
        id_feature_result_file = os.path.join(result_path, seq, filename + "_0.bin")
    elif data_name == "TUM":
        id_feature_result_file = os.path.join(result_path, "TUM", "TUM", filename + "_0.bin")
    else:
        print("no data_name")
...
def eval_kitti(data_path, result_path, visualization):
    """
    eval Kitti dataset
    """
    ...
    for ind, file_name in enumerate(image_path):
        ...
        if visualization:
            seq = all_path[-3]
            result_file = os.path.join(result_path, seq, depth_path_name)
            write_depth(result_file, prediction, bits=2)
...
def eval_tum(data_path, result_path, visualization):
    """
    eval TUM dataset
    """
    ...
    for line in open(file_path):
        ...
        prediction = cv2.resize(prediction, (mask.shape[2], mask.shape[1]))
        if visualization:
            all_path1 = image_path.split('/')
            tempfilename = all_path1[-1]
            filename, _ = os.path.splitext(tempfilename)
            result_file = os.path.join(result_path, 'TUM', 'TUM', filename)
            write_depth(result_file, prediction, bits=2)
...
def eval_sintel(data_path, result_path, visualization):
    """
    eval Sintel dataset
    """
