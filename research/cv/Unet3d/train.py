...
if config.device_target == "Ascend":
    ...
    context.set_context(
        mode=context.GRAPH_MODE, device_target=config.device_target, save_graphs=False, device_id=device_id
    )
else:
...
@moxing_wrapper()
def train_net(data_path, run_distribute):
    ...
    train_dataset = create_dataset(
        data_path=data_dir, seg_path=seg_dir, rank_size=rank_size, rank_id=rank_id, is_training=True
    )
    ...
    ckpt_config = CheckpointConfig(
        save_checkpoint_steps=train_data_size, keep_checkpoint_max=config.keep_checkpoint_max
    )
    ckpt_save_dir = os.path.join(config.output_path, config.checkpoint_path)
    ckpoint_cb = ModelCheckpoint(
        prefix="Unet3d", directory=ckpt_save_dir + "./ckpt_{}/".format(rank_id), config=ckpt_config
    )
...
if __name__ == "__main__":
    train_net(data_path=config.data_path, run_distribute=config.run_distribute)
