...
class UNet3d_(nn.Cell):
    ...
    def __init__(self):
        ...
        self.down5 = Down(in_channel=128, out_channel=256, stride=1, kernel_size=(1, 1, 1), dtype=mstype.float32)

        # up
        self.up1 = Up(in_channel=256, down_in_channel=128, out_channel=64, dtype=mstype.float32)
        self.up2 = Up(in_channel=64, down_in_channel=64, out_channel=32, dtype=mstype.float32)
        self.up3 = Up(in_channel=32, down_in_channel=32, out_channel=16, dtype=mstype.float32)
        self.up4 = Up(
            in_channel=16, down_in_channel=16, out_channel=self.n_classes, dtype=mstype.float32, is_output=True
        )
...
class UNet3d(nn.Cell):
    def __init__(self):
        ...
        self.down5 = Down(
            in_channel=128, out_channel=256, stride=1, kernel_size=(1, 1, 1), dtype=mstype.float16
        ).to_float(mstype.float16)

        # up
        self.up1 = Up(in_channel=256, down_in_channel=128, out_channel=64, dtype=mstype.float16).to_float(
            mstype.float16
        )
        self.up2 = Up(in_channel=64, down_in_channel=64, out_channel=32, dtype=mstype.float16).to_float(mstype.float16)
        self.up3 = Up(in_channel=32, down_in_channel=32, out_channel=16, dtype=mstype.float16).to_float(mstype.float16)
        self.up4 = Up(
            in_channel=16, down_in_channel=16, out_channel=self.n_classes, dtype=mstype.float16, is_output=True
        ).to_float(mstype.float16)
