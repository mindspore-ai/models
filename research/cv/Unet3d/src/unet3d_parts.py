...
class ResidualUnit(nn.Cell):
    def __init__(self, in_channel, out_channel, stride=2, kernel_size=(3, 3, 3), down=True, is_output=False):
        ...
        self.down_conv_1 = nn.Conv3d(
            in_channel, out_channel, kernel_size=(3, 3, 3), pad_mode="pad", stride=self.stride, padding=1
        )
        ...
        if self.down:
            self.down_conv_2 = nn.Conv3d(
                out_channel, out_channel, kernel_size=(3, 3, 3), pad_mode="pad", stride=1, padding=1
            )
            self.relu2 = nn.PReLU()
            if kernel_size[0] == 1:
                self.residual = nn.Conv3d(
                    in_channel, out_channel, kernel_size=(1, 1, 1), pad_mode="valid", stride=self.stride
                )
            else:
                self.residual = nn.Conv3d(
                    in_channel, out_channel, kernel_size=(3, 3, 3), pad_mode="pad", stride=self.stride, padding=1
...
class Up(nn.Cell):
    def __init__(self, in_channel, down_in_channel, out_channel, stride=2, is_output=False, dtype=mstype.float16):
        ...
        self.conv3d_transpose = nn.Conv3dTranspose(
            in_channels=self.in_channel + self.down_in_channel,
        ...
        self.conv = ResidualUnit(
            self.out_channel, self.out_channel, stride=1, down=False, is_output=is_output
        ).to_float(dtype)
        self.batchNormal1 = BatchNorm3d(num_features=self.out_channel)
        self.relu = nn.PReLU()
