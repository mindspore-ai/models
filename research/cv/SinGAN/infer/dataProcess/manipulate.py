...
def SinGAN_generate(Gs, Zs, reals, NoiseAmp, opt, n=0):
    ...
    for G, Z_opt, noise_amp in zip(Gs, Zs, NoiseAmp):
        ...
        for i in range(1):
            ...
            if n == len(Gs)-2:
                I_curr_light = numpyProcess(I_curr)
                dir2save = '%s/RandomSamples/%s' % (opt.out, opt.input_name[:-4])
                try:
                    os.makedirs(dir2save)
                except OSError:
                    pass
                plt.imsave('%s/%d.png' % (dir2save, i), functions.convert_image_np(I_curr_light), vmin=0, vmax=1)
...
def numpyProcess(I_curr):
    _, c, x, y = I_curr.shape
    count = 0
    l = []
    for I_curri in I_curr[0]:
        for I_currj in I_curri:
            count = count + 1
            print('Converting mindspore Tensor to float by %d/%d\r'%(count * y, c * x * y), end='')
            for k in I_currj:
                l.append(float((k + 0.0).asnumpy()))
    I_curr_light = np.asarray(l).reshape(I_curr.shape)
    np.save("I_curr6", I_curr_light, allow_pickle=True)
    return I_curr_light
