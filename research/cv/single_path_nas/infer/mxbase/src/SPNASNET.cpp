...
APP_ERROR SPNASNET::Init(const InitParam &initParam) {
...
APP_ERROR SPNASNET::ReadImage(const std::string &imgPath, cv::Mat *imageMat) {
    *imageMat = cv::imread(imgPath, cv::IMREAD_COLOR);
    return APP_ERR_OK;
}

APP_ERROR SPNASNET::ResizeImage(const cv::Mat &srcImageMat, cv::Mat *dstImageMat) {
    // first resize image to 299 * 299
    ...
    cv::Rect myROI(37.5, 37.5, 224, 224);
    *dstImageMat = scaledImage(myROI);
    return APP_ERR_OK;
}

APP_ERROR SPNASNET::CVMatToTensorBase(const cv::Mat &imageMat, MxBase::TensorBase *tensorBase) {
    uint32_t dataSize = 1;
    ...
    std::vector<uint32_t> shape = {1, 224, 224, 3};
    *tensorBase = TensorBase(memoryDataDst, false, shape, TENSOR_DTYPE_UINT8);
    return APP_ERR_OK;
}

APP_ERROR SPNASNET::Inference(const std::vector<MxBase::TensorBase> &inputs,
...
APP_ERROR SPNASNET::PostProcess(const std::vector<MxBase::TensorBase> &inputs,
...
APP_ERROR SPNASNET::SaveResult(const std::vector<std::string> &batchImgPaths,
...
APP_ERROR SPNASNET::Process(const std::vector<std::string> &batchImgPaths) {
