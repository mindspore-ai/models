...
int main(int argc, char **argv) {
    ...
    if (argc != 4) {
        std::cout << "example: ./main mindir_path input_path device_id " << std::endl;
        return -1;
    }
    std::cout << "mindir_path:" << argv[1] << std::endl;
    std::cout << "input_path:" << argv[2] << std::endl;
    std::cout << "device_id:" << argv[3] << std::endl;
    int device_id = argv[3][0] - '0';
    if (RealPath(argv[1]).empty()) {
    ...
    Serialization::Load(argv[1], ModelType::kMindIR, &graph);
    ...
    auto input0_files = GetAllFiles(argv[2]);
