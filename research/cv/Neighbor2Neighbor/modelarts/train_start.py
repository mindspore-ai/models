...
parser = argparse.ArgumentParser(description='Neighbor2Neighbor train.')
...
parser.add_argument('--eval_while_train', type=int, default=0, help='eval_while_train.')
parser.add_argument('--epoch', type=int, default=1, help='Train epoch.')
parser.add_argument('--file_format', type=str, default='AIR', help='Export file format.')

args_opt = parser.parse_args()

CACHE_TRAINING_URL1 = '/cache/n2n/'
if not os.path.isdir(CACHE_TRAINING_URL1):
    os.makedirs(CACHE_TRAINING_URL1)
CACHE_TRAINING_URL2 = 'cache/n2n/dataset'
if not os.path.isdir(CACHE_TRAINING_URL2):
    os.makedirs(CACHE_TRAINING_URL2)
CACHE_TRAINING_URL3 = 'cache/n2n/output'
if not os.path.isdir(CACHE_TRAINING_URL3):
    os.makedirs(CACHE_TRAINING_URL3)
...
def copy_data_from_obs():
    ...
    if args_opt.is_modelarts:
        ...
        mox.file.copy_parallel(args_opt.data_url, 'cache/n2n/dataset')
        cfg.logger.info("copying traindata finished....")
        if args_opt.eval_while_train:
            cfg.logger.info("copying test data from obs to cache....")
            mox.file.copy_parallel(args_opt.test_dir, 'cache/test')
            cfg.logger.info("copying test data finished....")
            args_opt.test_dir = 'cache/test/'
        if cfg.resume_path:
...
def copy_data_to_obs():
    if args_opt.is_modelarts:
        ...
        mox.file.copy_parallel('/cache', args_opt.train_url)
        cfg.logger.info("copying finished....")

def main():
    '''main'''
    dataset, cfg.steps_per_epoch = create_Dataset(args_opt.data_url, cfg.patchsize, \
    ...
    for k in range(args_opt.epoch):
        ...
        if cfg.rank_save_ckpt_flag:
            # checkpoint save
            save_checkpoint(model, os.path.join('/cache/', str(cfg.rank)+"_last_map.ckpt"))
    ...
    filename = '0_last_map.ckpt'
    param_dict = load_checkpoint(os.path.join('/cache/', filename))
    load_param_into_net(net, param_dict)
    if cfg.dataset == 'Kodak':
        input_arr = Tensor(np.zeros([cfg.export_batch_size, 3, \
                                     768, 768]), mindspore.float32)
    else:
        input_arr = Tensor(np.zeros([cfg.export_batch_size, 3, \
                                     512, 512]), mindspore.float32)
    filename = cfg.file_name + cfg.dataset
    export(net, input_arr, file_name=filename, file_format=args_opt.file_format)
    shutil.copy('neighbor2neighborKodak.air', '/cache')
...
def test(model):
    ...
    best_value = 0.
    for filename in os.listdir(args_opt.train_url):
        tem_path = os.path.join(args_opt.train_url, filename)
...
if __name__ == '__main__':
    ...
    if not args_opt.is_modelarts and not os.path.exists(cfg.save_dir):
