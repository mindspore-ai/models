...
APP_ERROR Neighbor2Neighbor::Init(const InitParam &initParam) {
...
APP_ERROR Neighbor2Neighbor::DeInit() {
    model_->DeInit();
    MxBase::DeviceManager::GetInstance()->DestroyDevices();
    return APP_ERR_OK;
}

APP_ERROR Neighbor2Neighbor::Inference(const std::vector<MxBase::TensorBase> &inputs,
...
APP_ERROR Neighbor2Neighbor::Process(const std::string &imgPath) {
    std::string data_set;
    data_set = imgPath;
    std::vector<std::string> images;
    images = GetAllFiles(data_set);
    for (const auto& image_file : images) {
        float fnum[DIM * DIM * CHANNEL]={0};
        std::ifstream in(image_file, std::ios::in | std::ios::binary);
        LogInfo <<imgPath;
        in.read(reinterpret_cast<char*>(&fnum), sizeof (fnum));
        MxBase::TensorBase tensorBase;
        const uint32_t dataSize = DIM * DIM * CHANNEL * ENLARGE_NUM;
        ...
        std::vector<uint32_t> shape = {static_cast<uint32_t>(1), static_cast<uint32_t>(DIM),
                                       static_cast<uint32_t>(DIM), static_cast<uint32_t>(CHANNEL)};
...
APP_ERROR  Neighbor2Neighbor::WriteResult(const std::string& imageFile, std::vector<MxBase::TensorBase> &outputs) {
    std::string homePath;
    homePath = "./result_Files/";
