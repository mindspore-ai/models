...
def frozen_to_air(modelnet, modelargs):
    param_dict_ = load_checkpoint(modelargs.get("ckpt_file"))
    load_param_into_net(modelnet, param_dict_)
    input_arr = Tensor(
        np.zeros([1, 3, modelargs.get("height"), modelargs.get("width")], np.float32))
    export(
        modelnet,
        input_arr,
        file_name=modelargs.get("file_name"),
        file_format=modelargs.get("file_format"))

def ckpt_save():
    ckpt_path = args_opt.logs_dir
    ckpt_list = glob.glob(ckpt_path + "*.ckpt")
    ckpt_list.sort(key=os.path.getmtime)
    ckpt_model = ckpt_list[-1]
    mox.file.copy(src_url=ckpt_model, dst_url=args_opt.output_url+'/vit_base.ckpt')
    net_ = VisionTransformer(CONFIGS[args_opt.sub_type], num_classes=cfg.num_classes)
    frozen_to_air_args = {'ckpt_file': ckpt_model,
                          'height': 224,
                          'width': 224,
                          'file_name': '/cache/train_output/vit_base',
                          'file_format': 'AIR'}
    frozen_to_air(net_, frozen_to_air_args)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--modelarts', type=str, default='False', help='use ModelArts or not.(True or False)')
    parser.add_argument('--logs_dir', type=str, default='./ckpt/', help='dir to save logs and ckpt.')
    parser.add_argument('--do_val', type=str, default='False', help='do evaluation.(True or False) (Default: False)')
    parser.add_argument('--num_classes', type=int, default=10, help='dataset classification classes')
    parser.add_argument('--output_url', type=str, default=None, help='location of output')
    parser.add_argument('--batch_size', type=int, default=32, help='train batch_size')
    parser.add_argument('--epoch_size', type=int, default=60, help='epoch_size')
    args_opt = parser.parse_args()

    if ast.literal_eval(args_opt.modelarts):
    ...
    if cfg.pre_trained:
        ...
        print(f"Load pre_trained ckpt: {cfg.checkpoint_path}")
    ...
    if ast.literal_eval(args_opt.modelarts):
        cbs = [time_cb,
               ModelCheckpoint(prefix="train_vit_" + args_opt.dataset_name, directory=ckpt_save_dir, config=config_ck),
               loss_cb]
    else:
        ...
        if ast.literal_eval(args_opt.do_val):
    ...
    ckpt_save()
    print(f"air save path is {args_opt.output_url}")
    mox.file.copy(src_url='/cache/train_output/vit_base.air', dst_url=args_opt.output_url+'/vit_base.air')
