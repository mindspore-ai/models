...
APP_ERROR VIT_BASE::Init(const InitParam &initParam) {
  ...
  LogError << "CLASS_NUM: " << initParam.classNum;
...
APP_ERROR VIT_BASE::ConvertImageToTensorBase(const std::string &imgPath,
                                             TensorBase &tensorBase) {
  cv::Mat imageMat = cv::imread(imgPath, cv::IMREAD_COLOR);
  cv::Mat resize_imageMat(IMG_H, IMG_W, CV_8UC3);
  cv::resize(imageMat, resize_imageMat, cv::Size(IMG_H, IMG_W));

  const uint32_t dataSize =
      resize_imageMat.cols * resize_imageMat.rows * MxBase::YUV444_RGB_WIDTH_NU;

  MemoryData memoryDataDst(dataSize, MemoryData::MEMORY_DEVICE, deviceId_);
  MemoryData memoryDataSrc(resize_imageMat.data, dataSize,
  ...
  std::vector<uint32_t> shape = {
      resize_imageMat.rows * MxBase::YUV444_RGB_WIDTH_NU,
      static_cast<uint32_t>(resize_imageMat.cols)};
  tensorBase =
      TensorBase(memoryDataDst, false, shape, MxBase::TENSOR_DTYPE_UINT8);
  return APP_ERR_OK;
}

APP_ERROR VIT_BASE::Inference(std::vector<TensorBase> &inputs,
...
APP_ERROR VIT_BASE::PostProcess(std::vector<TensorBase> &inputs,
...
APP_ERROR VIT_BASE::Process(const std::string &imgPath) {
  TensorBase tensorBase;
