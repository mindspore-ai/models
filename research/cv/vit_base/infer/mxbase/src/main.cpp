...
int main(int argc, char *argv[]) {
  ...
  initParam.modelPath = "../../convert/vit_base.om";
  auto vit_base = std::make_shared<VIT_BASE>();
  APP_ERROR ret = vit_base->Init(initParam);
  if (ret != APP_ERR_OK) {
    LogError << "vit_base Classify init failed, ret=" << ret << ".";
    return ret;
  }
  ...
  for (uint32_t i = 0; i < files.size(); i++) {
    ret = vit_base->Process(files[i]);
    if (ret != APP_ERR_OK) {
      LogError << "vit_base Classify process failed, ret=" << ret << ".";
      vit_base->DeInit();
      return ret;
    }
  }
  ...
  double fps = 1000.0 * files.size() / vit_base->GetInferCostMilliSec();
