...
cv::Mat hwc2chw(cv::Mat dst, size_t resize_detection) {
...
void pretreatment(const cv::Mat& src, cv::Mat* target, const param& config,
                  const int& size, double s_x) {
...
void getExemplar(const std::string& temp_video,
  ...
  std::cout << "temp groundtruth_rect: " << config->dataset_path_txt
            << std::endl;
  ...
  std::cout << "read imread success" << std::endl;
  cv::Mat exemplar;
  pretreatment(src, &exemplar, *config, 127, config->s_z);
  std::cout << "box :" << config->box[0] << " " << config->box[1] << " "
...
void preInstance(std::vector<MSTensor>* input_exemplar,
                 ...
                 const std::vector<MSTensor>& model_inputs_instance,
                 Model* model2, const MSTensor& instance) {
...
void getRetInstance(int instance_num,
                    const std::vector<MSTensor>& outputs_exemplar,
                    const cv::Mat& cos_window, param* config, Model* model2) {
  ...
  std::cout << " target_position[0]: " << config->target_position[0]
            << " target_position[1]:" << config->target_position[1]
            << std::endl;
  config->s_x = (0.41 + 0.59 * scale) * config->s_x;
  config->s_x = cv::max(config->min_s_x, cv::min(config->max_s_x, config->s_x));
...
void CreateHanningWindowWithCV_32F(cv::Mat* dst, const int& rows,
                                   const int& cols) {
  if (rows == 1 && cols == 1) {
    dst->at<float>(0, 0) = 1;
...
void CreateHanningWindowWithCV_64F(cv::Mat* dst, const int& rows,
                                   const int& cols) {
  if (rows == 1 && cols == 1) {
    dst->at<double>(0, 0) = 1;
  } else if (rows == 1 && cols > 1) {
...
void myCreateHanningWindow(cv::OutputArray _dst, cv::Size winSize, int type) {
...
int Build(Model* model1, Model* model2) {
  ...
  if (ret_graph2 != kSuccess) {
    std::cout << " load graph2 failed" << std::endl;
    return 1;
  } else {
    std::cout << " load graph2 Success" << std::endl;
  }

  Status ret_build2 = model2->Build(GraphCell(graph2), context);
  if (ret_build2 != kSuccess) {
    std::cout << " build graph2 failed" << std::endl;
    return 1;
  } else {
    std::cout << " build graph2 Success" << std::endl;
  }
...
int main(int argc, char** argv) {
  ...
  for (size_t i = 0; i < size_v; ++i) {
    ...
    std::cout << "record:" << config.record_name << "  " << config.record_times
              << std::endl;
