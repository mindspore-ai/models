...
from src.eval_callbacks import EvalCallBack
from src import SiamFCTracker
sys.path.append(os.getcwd())



def train(data_dir, device_target, eval_data):
    """set train """
    # loading meta data
    ...
    if device_target == "GPU":
        evalnet = SiameseAlexNet(train=False)
        tracker = SiamFCTracker(network=evalnet)
        root_dir = os.path.abspath(eval_data)
        evalcb = EvalCallBack(tracker, root_dir)
        model.train(epoch=config.epoch,
                    train_dataset=dataset,
                    callbacks=[time_cb_train, ckpoint_cb_train, loss_cb_train, evalcb],
...
if __name__ == '__main__':
    ...
    ARGPARSER.add_argument('--eval_data',
                           type=str, default='/home/data/otb2013',
    ...
    if DEVICETARGET == "Ascend":
        ...
        context.set_context(
            ...
            device_target="GPU")
    # train
    train(ARGS.data_path, DEVICETARGET, ARGS.eval_data)
