...
def test(data_set, session, input_name, batch_size, nfolds=10):
    '''
    test
    '''
    ...
    for data in data_list:
        ...
        while ba < data.shape[0]:
            ...
            net_out = session.run(None, {input_name: np.array(img, np.float32)})[0]
            _embeddings = net_out
...
def main():
    ...
    # general
    parser.add_argument('--eval_url', default='/data/arcface/', help='')
    parser.add_argument('--target',
                        default='lfw,cfp_fp,agedb_30',
                        help='test targets.')
    parser.add_argument('--nfolds', default=10, type=int, help='')
    parser.add_argument('--device_target', default="GPU", type=str, help='device target')
    parser.add_argument('--onnx_url', default='', type=str, help='onnx path')
    parser.add_argument('--batch_size', default=64, type=int, help='batch size')
    args = parser.parse_args()

    # Create inference session
    session, input_name = create_session(args.onnx_url, args.device_target)
    ...
    for i in range(length):
        acc1, std1, acc2, std2, xnorm, _ = test(
            ver_list[i], session, input_name, args.batch_size, args.nfolds)
