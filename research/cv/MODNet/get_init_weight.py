...
def pytorch2mindspore(ckpt_path='./init_weight.pth', save_ckpt='./init_weight.ckpt'):
    ...
    par_dict = torch.load(ckpt_path)

    nums = 0
    new_params_list = []
    dict_bn = {'0': ['weight', 'gamma'],
               '1': ['bias', 'beta'],
    ...
    for idx, name in enumerate(list_dict):
        ...
        if idx + 4 < n and name.endswith('.weight') and \
                list_dict[idx + 1].endswith('.bias') and \
                list_dict[idx + 2].endswith('.running_mean') and \
                list_dict[idx + 3].endswith('.running_var') and \
                list_dict[idx + 4].endswith('.num_batches_tracked'):
...
if __name__ == '__main__':
    ...
    load_ckpt_file = './pretrained/mobilenetv2_human_seg.ckpt'
    save_ckpt_file = './pretrained/mindspore_mobilenetv2_human_seg.ckpt'

    load_file = os.path.join(file_path, load_ckpt_file)
    save_file = os.path.join(file_path, save_ckpt_file)
    pytorch2mindspore(ckpt_path=load_file, save_ckpt=save_file)
