...
then
    ...
    echo "bash scripts/run_tacred_standalone_train_gpu.sh [DATA] [MODEL_FILE] [TRAIN_BATCH_SIZE]"
    exit 1
fi
...
rm -rf logs
mkdir ./logs
python run_tacred_train.py --data="$DATA"  --model_file="$MODEL_FILE" --train_batch_size="$TRAIN_BATCH_SIZE" --num_train_epochs=5 \
         --learning_rate=0.00001 > ./logs/standalone_train.log 2>&1 &
