...
then
    ...
    echo "bash scripts/run_tacred_distribute_train_gpu.sh [DATA] [MODEL_FILE] [TRAIN_BATCH_SIZE]"
    exit 1
fi
...
rm -rf logs
mkdir ./logs
mpirun -n 8 --allow-run-as-root python run_tacred_train.py --data="$DATA"  --model_file="$MODEL_FILE" \
   --train_batch_size="$TRAIN_BATCH_SIZE" --num_train_epochs=5 --learning_rate=0.000005 --distribute=True \
