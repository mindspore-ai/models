...
#ifndef MXBASE_TEXTCNNBASE_H
#define MXBASE_TEXTCNNBASE_H
...
class TextCnnBase {
 ...
 protected:
    APP_ERROR read_tensor_from_file(const std::string& file, uint32_t* data,
                                    uint32_t size);
    APP_ERROR read_input_tensor(const std::string& fileName, uint32_t index,
                                std::vector<MxBase::TensorBase>* inputs);
    APP_ERROR load_labels(const std::string& labelPath,
                          std::vector<std::string>* labelMap);
    APP_ERROR read_input_tensor(const std::string& fileName,
                                const std::vector<uint32_t>& argmax);
    APP_ERROR write_result(const std::string& fileName,
                           const std::vector<uint32_t>& argmax);
    APP_ERROR count_predict_result(const std::string& labelFile,
                                   const std::vector<uint32_t>& argmax);
    void get_cluner_label(
      const std::vector<uint32_t>& argmax,
