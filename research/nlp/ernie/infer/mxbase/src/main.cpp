...
void InitErnieParam(InitParam* initParam) {
    initParam->deviceId = 0;
    initParam->modelPath = "../data/model/chnsenticorp.om";
}
...
int main(int argc, char* argv[]) {
    ...
    InitParam initParam;
    InitErnieParam(&initParam);
    auto ernieClassifier = std::make_shared<ErnieClassifier>();
    APP_ERROR ret = ernieClassifier->Init(initParam);
    if (ret != APP_ERR_OK) {
        LogError << "ErnieClassifier init failed, ret=" << ret << ".";
        return ret;
    }
    ...
    for (uint32_t i = 0; i < files.size(); i++) {
        ...
        ret = ernieClassifier->Process(inferPath, files[i], eval);
        if (ret != APP_ERR_OK) {
            LogError << "ErnieClassifier process failed, ret=" << ret << ".";
            ernieClassifier->DeInit();
            return ret;
        }
    }
