...
def do_eval(dataset=None, network=None, number_labels=2, load_checkpoint_path="",
            eval_net_cfg=None, assessment_method='accuracy'):
    """ do eval """
    if load_checkpoint_path == "":
        raise ValueError("Finetune model missed, evaluation task must load finetune model!")
    net_for_pretraining = network(eval_net_cfg, False, number_labels)
...
def parse_args():
    ...
    parser.add_argument('--train_url', type=str, default="", help='Train output path')
...
def frozen_to_air(net, args_opt):
    """frozen model to air"""
    load_checkpoint(args_opt.get("ckpt_file"), net=net)
    net.set_train(False)
    ...
    export(net, *input_data, file_name=args_opt.get("file_name"), file_format=args_opt.get("file_format"))
...
def run_classifier():
    ...
    if args_opt.modelarts.lower() == 'true':
        ...
        local_pretrain_checkpoint_path = '/cache/pretrain/ernie.ckpt'
        mox.file.copy_parallel(args_opt.load_pretrain_checkpoint_path, local_pretrain_checkpoint_path)
        load_pretrain_checkpoint_path = local_pretrain_checkpoint_path
    ...
    if args_opt.do_train.lower() == "true":
        ...
        frozen_to_air_args = {'ckpt_file': ckpt_model,
                              'batch_size': 1,
                              'file_name': CACHE_TRAINING_URL + args_opt.task_type + '.air',
                              'file_format': 'AIR'}
        if args_opt.task_type == 'msra_ner':
            net = ErnieNERModel(ernie_net_cfg, False, num_labels=args_opt.number_labels)
        elif args_opt.task_type == "chnsenticorp":
            net = ErnieCLSModel(ernie_net_cfg, False, num_labels=args_opt.number_labels)
        elif args_opt.task_type in ["xnli", "dbqa"]:
            ernie_net_cfg.seq_length = 512
            net = ErnieCLSModel(ernie_net_cfg, False, num_labels=args_opt.number_labels)
