...
class CrossEntropyCalculation(nn.Cell):
    ...
    def construct(self, logits, label_ids, num_labels):
        if self.is_training:
            ...
            one_hot_labels = self.onehot(label_ids, num_labels, self.on_value, self.off_value)  # [32, 512]
...
class AlbertLearningRate(LearningRateSchedule):
    """
    Warmup-decay learning rate for Albert network.
    """
    def __init__(self, learning_rate, end_learning_rate, warmup_steps, decay_steps, power):
        super(AlbertLearningRate, self).__init__()
...
def get_albert_thor_lr(lr_max=0.0034, lr_min=3.244e-05, lr_power=1.0, lr_total_steps=30000):
...
def get_albert_thor_damping(damping_max=5e-2, damping_min=1e-6, damping_power=1.0, damping_total_steps=30000):
...
class AlbertMetric(Metric):
    ...
    def __init__(self, batch_size):
        super(AlbertMetric, self).__init__()
        self.clear()
