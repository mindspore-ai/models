...
def convert_examples_to_features(examples, tokenizer, max_seq_length,
    ...
    for (example_index, example) in enumerate(examples):
        ...
        max_dist = abs(n - m) + 5
        for _ in range(2):
            f, g = _lcs_match(max_dist, n, m, f, g, do_lower_case, paragraph_text, tok_cat_text)
...
class FeatureWriter:
    """Writes InputFeature to Mind example file."""

    def __init__(self, filename, is_training, shard_num):
        self.filename = filename
        self.is_training = is_training
        self.num_features = 0
        self._writer = FileWriter(file_name=filename, shard_num=shard_num)
        self.nlp_schema = {
            "unique_ids": {"type": "int64", "shape": [-1]},
        ...
        if self.is_training:
            self.nlp_schema["start_positions"] = {"type": "int64", "shape": [-1]}
            self.nlp_schema["end_positions"] = {"type": "int64", "shape": [-1]}
            self.nlp_schema["is_impossible"] = {"type": "int64", "shape": [-1]}

        self._writer.add_schema(self.nlp_schema, "proprocessed squad dataset")

        self.data = []

    def process_feature(self, feature):
        """Write a InputFeature to the MindRecordWriter."""
        self.num_features += 1

        sample = {
            "unique_ids": np.array(feature.unique_id, dtype=np.int64),
            "input_ids": np.array(feature.input_ids, dtype=np.int64),
            "input_mask": np.array(feature.input_mask, dtype=np.int64),
            "segment_ids": np.array([feature.segment_ids], dtype=np.int64),
            "p_mask": np.array([feature.p_mask], dtype=np.int64),
        }

        if self.is_training:
            sample["start_positions"] = np.array(feature.start_position, dtype=np.int64)
            sample["end_positions"] = np.array(feature.end_position, dtype=np.int64)
            impossible = 0
            if feature.is_impossible:
                impossible = 1
            sample["is_impossible"] = np.array(impossible, dtype=np.int64)
        self.data.append(sample)

    def write(self):
        self._writer.write_raw_data(self.data)
...
def gen_data():
    ...
    parser.add_argument("--train_file", type=str, default="", help="raw data file")
    parser.add_argument("--train_feature_file", type=str, default="", help="raw data file")
    ...
    if args_opt.do_train:
        print("do_train")
        print("train_feature_file: ", args_opt.train_feature_file)
        if not os.path.exists(args_opt.train_feature_file):
            train_examples = read_squad_examples(
                input_file=args_opt.train_file, is_training=True)
            train_writer = FeatureWriter(
                filename=args_opt.train_feature_file, is_training=True, shard_num=args_opt.shard_num)

            convert_examples_to_features(
                examples=train_examples,
                tokenizer=tokenizer,
                max_seq_length=args_opt.max_seq_length,
