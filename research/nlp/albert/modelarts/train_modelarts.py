...
def do_export(args, load_finetune_checkpoint_path):
    '''export function'''
    ...
    print("load_finetune_checkpoint_path ", load_finetune_checkpoint_path)
    print("export_file_name ", args.export_file_name)
    print("os.path.realpath(file_name) ",
          os.path.realpath(args.export_file_name))
    load_checkpoint(load_finetune_checkpoint_path, net=net)
    net.set_train(False)

    input_ids = Tensor(
        np.zeros([args.export_batch_size, albert_net_cfg.seq_length]), mstype.int32)
    input_mask = Tensor(
        np.zeros([args.export_batch_size, albert_net_cfg.seq_length]), mstype.int32)
    ...
    print("remove ", os.path.realpath(args.export_file_name))
    print("remove ", args.export_file_name + "." + args.file_format)
    print("remove ", f"{args.export_file_name}.{args.file_format.lower()}")
    if os.path.exists(f"{args.export_file_name}.{args.file_format.lower()}"):
        os.remove(f"{args.export_file_name}.{args.file_format.lower()}")

    export(net, *input_data, file_name=args.export_file_name,
...
def do_train(dataset=None, network=None, load_checkpoint_path="", save_checkpoint_path="", epoch_num=1, args=None):
    ...
    if optimizer_cfg.optimizer == 'AdamWeightDecay':
        ...
        other_params = list(
            filter(lambda x: not optimizer_cfg.AdamWeightDecay.decay_filter(x), params))
        group_params = [{'params': decay_params, 'weight_decay': optimizer_cfg.AdamWeightDecay.weight_decay},
                        {'params': other_params, 'weight_decay': 0.0}]

        optimizer = AdamWeightDecay(
            group_params, lr_schedule, eps=optimizer_cfg.AdamWeightDecay.eps)
    elif optimizer_cfg.optimizer == 'Lamb':
        lr_schedule = AlbertLearningRate(learning_rate=optimizer_cfg.Lamb.learning_rate,
                                         end_learning_rate=optimizer_cfg.Lamb.end_learning_rate,
                                         warmup_steps=int(
                                             steps_per_epoch * epoch_num * 0.1),
                                         decay_steps=steps_per_epoch * epoch_num,
    ...
    ckpt_config = CheckpointConfig(
        save_checkpoint_steps=steps_per_epoch, keep_checkpoint_max=1)
    ...
    update_cell = DynamicLossScaleUpdateCell(
        loss_scale_value=2 ** 32, scale_factor=2, scale_window=1000)
    netwithgrads = AlbertSquadCell(
        network, optimizer=optimizer, scale_update_cell=update_cell)
    model = Model(netwithgrads)
    eval_callback = albert_callback(
        netwithgrads, args, steps_per_epoch, save_checkpoint_path)
    dataset_size = dataset.get_dataset_size()
    cb = [TimeMonitor(dataset_size), eval_callback, LossCallBack(dataset_size), ckpoint_cb]
    model.train(epoch_num, dataset, callbacks=cb, dataset_sink_mode=True)
...
def modelarts_pre_process():
    ...
    args_opt.load_pretrain_checkpoint_path = os.path.join(
        args_opt.data_path, args_opt.load_pretrain_checkpoint_path)
    args_opt.load_finetune_checkpoint_path = os.path.join(
        args_opt.output_path, args_opt.load_finetune_checkpoint_path)
    args_opt.save_finetune_checkpoint_path = os.path.join(
        args_opt.output_path, args_opt.save_finetune_checkpoint_path)
    args_opt.export_file_name = os.path.join(
        args_opt.output_path, args_opt.export_file_name)
    if args_opt.schema_file_path:
        args_opt.schema_file_path = os.path.join(
            args_opt.data_path, args_opt.schema_file_path)
    args_opt.train_data_file_path = os.path.join(
        args_opt.data_path, args_opt.train_data_file_path)
    args_opt.eval_json_path = os.path.join(
        args_opt.data_path, args_opt.eval_json_path)
    args_opt.vocab_file_path = os.path.join(
        args_opt.data_path, args_opt.vocab_file_path)
    args_opt.spm_model_file = os.path.join(
        args_opt.data_path, args_opt.spm_model_file)
    if os.path.exists(args_opt.predict_feature_left_file):
        args_opt.predict_feature_left_file = os.path.join(
            args_opt.data_path, args_opt.predict_feature_left_file)
...
def run_squad():
    ...
    if args_opt.device_target == "Ascend":
        ...
        if args_opt.distribute == 'true':
            ...
            save_ckpt_path = os.path.join(
                args_opt.save_finetune_checkpoint_path, 'ckpt_' + str(get_rank()) + '/')
        else:
            rank = 0
            device_num = 1
            save_ckpt_path = os.path.join(
                args_opt.save_finetune_checkpoint_path, 'ckpt_0/')
    ...
    do_export(args_opt, load_finetune_checkpoint_path)


if __name__ == "__main__":
    args_opt.enable_modelarts = True
    args_opt.do_train = "true"
    run_squad()
