...
from src import AlbertNetworkWithLoss, AlbertNetworkMatchBucket, \
    AlbertTrainOneStepCell, \
    AlbertTrainOneStepWithLossScaleCell, \
    ...
    AlbertPretrainEval, \
    AdamWeightDecayForAlbert, AdamWeightDecayOp
from src.dataset import create_albert_dataset, create_eval_dataset
from src.utils import LossCallBack, AlbertLearningRate, EvalCallBack, AlbertMetric
from src.model_utils.config import config as cfg, albert_net_cfg
...
def _set_albert_all_reduce_split():
    """set albert all_reduce fusion split, support num_hidden_layers is 12 and 24."""
...
def _get_optimizer(args_opt, network):
    """get albert optimizer, support Lamb, Momentum, AdamWeightDecay."""
    if cfg.optimizer == 'Lamb':
        lr_schedule = AlbertLearningRate(learning_rate=cfg.Lamb.learning_rate,
