...
int main(int argc, char **argv) {
  ...
  std::string ascend = "ascend";
  std::string gpu = "gpu";
  std::string fnd = "00_data";
  std::string old_path = "preprocess_Result";
  std::string new_path = "result_Files";
  ...
  for (size_t i = 0; i < size; ++i) {
    ...
    if (FLAGS_mindir_path.find(gpu) != std::string::npos) {
        input0_files[i] = input0_files[i].replace(input0_files[i].find(fnd), fnd.length(), gpu);
    } else {
        input0_files[i] = input0_files[i].replace(input0_files[i].find(fnd), fnd.length(), ascend);
    }
    input0_files[i] = input0_files[i].replace(input0_files[i].find(old_path), old_path.length(), new_path);
  ...
  std::string fileName;
  if (FLAGS_mindir_path.find(gpu) != std::string::npos) {
    fileName = "./time_Result/gpu" + std::string("/test_perform_static.txt");
  } else {
    fileName = "./time_Result/ascend" + std::string("/test_perform_static.txt");
  }
