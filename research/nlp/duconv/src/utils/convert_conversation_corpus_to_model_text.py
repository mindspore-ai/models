...
def preprocessing_for_one_conversation(text, candidate_num=10, use_knowledge=True,
                                       topic_generalization=False, for_predict=True):
    '''
    preprocess one conversation
    '''
    ...
    if topic_generalization:
        ...
        history = [topic_generalization_for_text(utterance, topic_list) for utterance in history]
    ...
    if not history:
        ...
        history = ' [INNER_SEP] '.join([parser_char_for_text(utterance) for utterance in history])
