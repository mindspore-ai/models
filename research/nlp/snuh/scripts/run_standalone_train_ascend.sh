...
if [ $# != 2 ]
then
    echo "Usage: bash run_standalone_train_ascend.sh [DATASET] [DEVICE_ID]"
exit 1
fi
...
then
    ...
    --num_trees 13 --temperature 0.7 --alpha 0.1 --beta 0.09 --device Ascend > logs/train_$DATASET.log 2>&1 &
...
then
    ...
    --num_trees 10 --temperature 0.5 --alpha 0.3 --beta 0.03 --device Ascend > logs/train_$DATASET.log 2>&1 &
...
then
    ...
    --num_trees 19 --temperature 0.2 --alpha 0.2 --beta 0.05 --device Ascend > logs/train_$DATASET.log 2>&1 &
