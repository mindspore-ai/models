...
if [ $# != 2 ]; then
  echo "Usage: bash run_squad_eval_gpu.sh [DATA_PATH] [LOAD_CHECKPOINT_PATH]"
  exit 1
fi
...
echo "start evaluation on GPU device 0"
...
python3 ../run_KTNET_squad_eval.py \
  ...
  --data_url $DATA \
  --checkpoints output/ 1>$PWD_DIR/eval_squad.log 2>&1 &
