...
class H5Dataset():
    ...
    def __init__(self, data_path, train_mode=True,
        ...
        self.data_size = self.bin_count(self._hdf_data_dir, self._file_prefix, self._num_of_parts)
        print("data_size: {}".format(self.data_size))

    @classmethod
    def bin_count(cls, hdf_data_dir, file_prefix, num_of_parts):
        """bin_count method"""
        size = 0
    ...
    @classmethod
    def auc_metric(cls, x, y, batch_size, shuffle=True):
        """
        auc_metric method
        :param x:
        :param y:
    ...
    def iterate_hdf_files(self, num_of_parts=None,
                          shuffle_block=False
                          ):
        """
    ...
    def batch_generator(self, batch_size=1000,
        ...
        for hdf_in, hdf_out, _ in self.iterate_hdf_files(self._num_of_parts,
                                                         shuffle_block
                                                         ):
            start = stop = None
            x_all = pd.read_hdf(hdf_in, start=start, stop=stop).values
            y_all = pd.read_hdf(hdf_out, start=start, stop=stop).values
            data_gen = self.auc_metric(x_all, y_all, batch_size,
...
def _get_tf_dataset(directory, file_prefixt_name="train", epochs=1, batch_size=1000,
                    line_per_sample=1000, rank_size=None, rank_id=None
                    ):
    """
    ...
    """
    dataset_files = []
    shuffle = (file_prefixt_name == "train")
    directory = os.path.join(directory, file_prefixt_name)
    ...
    print(f"dataset_files len: {len(dataset_files)}")
    schema = ds.Schema()
    schema.add_column('feat_ids', de_type=mstype.int32)
    if config.get("dense_field_size", 0) > 0:
        schema.add_column('dense_fea', de_type=mstype.float32)
...
def create_dataset(directory, train_mode: [bool, str] = True, epochs=1, batch_size=1000,
    ...
    if data_type == DataType.TFRECORD:
        if isinstance(train_mode, bool):
            train_mode = 'train' if train_mode else 'valid'
