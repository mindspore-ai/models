...
from model import DatasetGenerator, JM, focal_loss, cycle, MinNormSolver
...
def train(args):
    ...
    model = JM(n_mels, max_midi - min_midi + 1, sr, hop_length, mel_fmin, mel_fmax, model_complexity)
    model.set_train()
    ...
    class WithLossCell(nn.Cell):
        ...
        def construct(self, inputs, onsets, offsets, frames, task_weights):
    ...
    class TrainOneStepCell(nn.Cell):
        ...
        def construct(self, inputs, onsets, offsets, frames, task_weights, back=True):
            loss = self.net(inputs, onsets, offsets, frames, task_weights)
            grads = self.grad_op(self.net, self.weights)(inputs, onsets, offsets, frames, task_weights)
            if back:
    ...
    for i, batch in zip(loop, cycle(loader.create_dict_iterator())):
        ...
        grads = [[], [], []]
        grad_onset = train_net(audio_label, onset_label, offset_label, frame_label, [1, 0, 0], False)
        for each_grad in grad_onset:
            grads[0].append(each_grad.copy().asnumpy().flatten())
        grad_offset = train_net(audio_label, onset_label, offset_label, frame_label, [0, 1, 0], False)
        for each_grad in grad_offset:
            grads[1].append(each_grad.copy().asnumpy().flatten())
        grad_frame = train_net(audio_label, onset_label, offset_label, frame_label, [0, 0, 1], False)
        ...
        weight_vec = (weight_vec * normalize_coeff).tolist()

        loss = train_net(audio_label, onset_label, offset_label, frame_label, weight_vec, True)
