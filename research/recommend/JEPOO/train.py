...
from model import DatasetGenerator, JM, focal_loss, cycle
...
def train(args):
    ...
    class WithLossCell(nn.Cell):
        ...
        def construct(self, inputs, onsets, offsets, frames):
            onset_pred, offset_pred, frame_pred = self.net(inputs)
            loss = self.loss(onset_pred, onsets, alpha_loss, gamma_loss, weight) + \
                   self.loss(offset_pred, offsets, alpha_loss, gamma_loss, weight) + \
                   self.loss(frame_pred, frames, alpha_loss, gamma_loss, weight)
            return loss
    ...
    class TrainOneStepCell(nn.Cell):
        ...
        def construct(self, inputs, onsets, offsets, frames):
            loss = self.net(inputs, onsets, offsets, frames)
            grads = self.grad_op(self.net, self.weights)(inputs, onsets, offsets, frames)
    ...
    for i, batch in zip(loop, cycle(loader.create_dict_iterator())):
        ...
        if onset_label.shape[0] != batch_size:
            continue
        frame_label = batch['frame']
        offset_label = batch['offset']
        loss = train_net(audio_label, onset_label, offset_label, frame_label)
