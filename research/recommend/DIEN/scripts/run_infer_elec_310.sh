...
if [[ $# -lt 2 || $# -gt 3 ]]; then
    echo "Usage: bash run_infer_elec_310.sh [MINDIR_PATH] [INPUT_PATH] [DEVICE_ID]"
exit 1
fi
...
function infer()
{
   cd - || exit
    ...
    mkdir result_Files
    mkdir time_Result
    ../ascend310_infer/out/main --mindir_path=$model --input0_path=$input_path/mid_mask --input1_path=$input_path/uids --input2_path=$input_path/mids --input3_path=$input_path/cats --input4_path=$input_path/mid_his --input5_path=$input_path/cat_his --input6_path=$input_path/noclk_mids --input7_path=$input_path/noclk_cats --device_id=$device_id &> infer_elec.log
}
