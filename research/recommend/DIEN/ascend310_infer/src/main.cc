...
DEFINE_string(input7_path, ".", "input7 path");
...
int main(int argc, char **argv) {
  ...
  auto input7_files = GetAllFiles(FLAGS_input7_path);

  if (input0_files.empty() || input1_files.empty() || input2_files.empty()||
    input3_files.empty()|| input4_files.empty()|| input5_files.empty()|| input6_files.empty()|| input7_files.empty()) {
  ...
  for (size_t i = 0; i < size; ++i) {
    ...
    auto input7 = ReadFileToTensor(input7_files[i]);
