...
import os
import numpy as np
from mindspore import context, Model, nn, Tensor, export
...
def modelarts_pre_process():
    config.train_path = config.data_url + "/" + config.local_train_path
    config.test_path = config.data_url + "/" + config.local_test_path
...
def run_train():
    ...
    ckpt_file = config.save_dir + sorted(os.listdir(config.save_dir))[-2].__str__()
    param_dict = load_checkpoint(ckpt_file)
    net_for_export = CTCModel(input_size=config.feature_dim, batch_size=config.test_batch_size,
                              hidden_size=config.hidden_size, num_class=config.n_class, num_layers=config.n_layer)
    load_param_into_net(net_for_export, param_dict)
    net_for_export.set_train(False)
    ...
    export(net_for_export, feature, masks, file_name=config.save_dir + config.file_name, file_format="AIR")
