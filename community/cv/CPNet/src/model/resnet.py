...
class BasicBlock(nn.Cell):
    ...
    def __init__(self, inplanes, planes, stride=1, down_sample_layer=None, BatchNorm_layer=nn.BatchNorm2d):
        ...
        self.conv1 = nn.Conv2d(inplanes, planes, kernel_size=3, stride=stride, dilation=1, pad_mode="pad",
                               padding=1, has_bias=False)
        self.bn1 = self.BatchNorm_layer(planes)
        self.relu = nn.ReLU()
        self.conv2 = nn.Conv2d(planes, planes, kernel_size=3, stride=stride, dilation=1, pad_mode="pad",
                               padding=1, has_bias=False)
...
class ResNet(nn.Cell):
    ...
    def __init__(self, block, layers, deep_base=False, BatchNorm_layer=nn.BatchNorm2d):
        ...
        if not self.deep_base:
            ...
            self.inplanes = 128
            self.conv1 = nn.Conv2d(3, 64, kernel_size=3, stride=2, dilation=1, pad_mode="pad",
                                   padding=1, has_bias=False)
            self.bn1 = self.BatchNorm_layer(64)
            self.conv2 = nn.Conv2d(64, 64, kernel_size=3, stride=stride, dilation=1, pad_mode="pad",
                                   padding=1, has_bias=False)
            self.bn2 = self.BatchNorm_layer(64)
            self.conv3 = nn.Conv2d(64, 128, kernel_size=3, stride=stride, dilation=1, pad_mode="pad",
                                   padding=1, has_bias=False)
