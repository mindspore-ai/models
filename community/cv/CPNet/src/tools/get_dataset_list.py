...
parser.add_argument("--data_dir", type=str, default='/root/data/VOCdevkit', help='where dataset stored.')
...
SBD_ANNO_DIR = os.path.join(data_dir, 'benchmark_RELEASE/benchmark_RELEASE/dataset/cls')
SBD_IMG_DIR = os.path.join(data_dir, 'benchmark_RELEASE/benchmark_RELEASE/dataset/img')
...
SBD_TRAIN_TXT = os.path.join(data_dir, 'benchmark_RELEASE/benchmark_RELEASE/dataset/train.txt')
SBD_VAL_TXT = os.path.join(data_dir, 'benchmark_RELEASE/benchmark_RELEASE/dataset/val.txt')
...
def create_voc_train_aug_lst_txt():
    ...
    with open(VOC_AUG_TRAIN_LST_TXT, mode='w') as f:
        ...
        for id_ in voc_train_data_lst:
            id_ = id_.strip()
            img_ = os.path.join(VOC_IMG_DIR, id_ + '.jpg').replace('./', '')
            anno_ = os.path.join(VOC_ANNO_DIR, id_ + '.png').replace('./', '')
            f.write(img_ + ' ' + anno_ + '\n')
