...
def rescale_column(image_id, img, img_shape, gt_bboxes, gt_label, gt_num, gt_mask):
    ...
    return (image_id, pad_img_data, img_shape, gt_bboxes, gt_label, gt_num, pad_mask)


def rescale_column_test(image_id, img, img_shape, gt_bboxes, gt_label, gt_num, gt_mask):
    ...
    return (image_id, pad_img_data, img_shape, gt_bboxes, gt_label, gt_num, gt_mask)


def resize_column(image_id, img, img_shape, gt_bboxes, gt_label, gt_num, gt_mask):
    ...
    return (image_id, img_data, img_shape, gt_bboxes, gt_label, gt_num, gt_mask_data)


def resize_column_test(image_id, img, img_shape, gt_bboxes, gt_label, gt_num, gt_mask):
    ...
    return (image_id, img_data, img_shape, gt_bboxes, gt_label, gt_num, gt_mask)
...
def imnormalize_column(image_id, img, img_shape, gt_bboxes, gt_label, gt_num, gt_mask):
...
def flip_column(image_id, img, img_shape, gt_bboxes, gt_label, gt_num, gt_mask):
    ...
    return (image_id, img_data, img_shape, flipped, gt_label, gt_num, gt_mask_data)


def transpose_column(image_id, img, img_shape, gt_bboxes, gt_label, gt_num, gt_mask):
...
def expand_column(image_id, img, img_shape, gt_bboxes, gt_label, gt_num, gt_mask):
    """expand operation for image"""
    expand = Expand()
    img, gt_bboxes, gt_label, gt_mask = expand(img, gt_bboxes, gt_label, gt_mask)

    return (image_id, img, img_shape, gt_bboxes, gt_label, gt_num, gt_mask)


def pad_to_max(image_id, img, img_shape, gt_bboxes, gt_label, gt_num, gt_mask, instance_count):
    '''pad to max'''
    ...
    return image_id, img, img_shape, gt_box_new, gt_label_new, gt_iscrowd_new_revert, gt_mask


def preprocess_fn(image_id, image, box, mask, mask_shape, is_training):
    """Preprocess function for dataset."""

    def _infer_data(image_id, image_bgr, image_shape, gt_box_new, gt_label_new, gt_iscrowd_new_revert,
                    gt_mask_new, instance_count):
        image_shape = image_shape[:2]
        input_data = image_id, image_bgr, image_shape, gt_box_new, gt_label_new, gt_iscrowd_new_revert, gt_mask_new
    ...
    def _data_aug(image_id, image, box, mask, mask_shape, is_training):
        ...
        if not is_training:
            return _infer_data(image_id, image_bgr, image_shape, gt_box, gt_label, gt_iscrowd, gt_mask, instance_count)

        flip = (np.random.rand() < config.flip_ratio)
        expand = (np.random.rand() < config.expand_ratio)

        input_data = image_id, image_bgr, image_shape, gt_box, gt_label, gt_iscrowd, gt_mask
        ...
        input_data = pad_to_max(*input_data, instance_count)
        image_id, img_data, img_shape, gt_bboxes, gt_label, gt_num, gt_mask_data = transpose_column(*input_data)
        return (img_data, img_shape, gt_bboxes, gt_label, gt_num, gt_mask_data)

    return _data_aug(image_id, image, box, mask, mask_shape, is_training)
...
def create_coco_label(is_training):
    ...
    return images_ids, image_files, image_anno_dict, masks, masks_shape
...
def data_to_mindrecord_byte_image(dataset="coco", is_training=True, prefix="maskrcnn.mindrecord", file_num=8):
    ...
    if dataset == "coco":
        images_ids, image_files, image_anno_dict, masks, masks_shape = create_coco_label(is_training)
    else:
    ...
    for ind, (image_name, image_ids) in enumerate(zip(image_files, images_ids)):
        ...
        if is_training:
            ...
            row = {"image_id": image_ids, "image": img, "annotation": annos, "mask": mask, "mask_shape": mask_shape}
        if (ind + 1) % 10 == 0:
...
def create_maskrcnn_dataset(mindrecord_file, batch_size=2, device_num=1, rank_id=0,
    ...
    if is_training:
        ...
        ds = de.MindDataset(mindrecord_file, columns_list=["image_id", "image", "annotation", "mask", "mask_shape"],
                            num_shards=device_num, shard_id=rank_id,
                            num_parallel_workers=1, shuffle=True)
    ...
    if is_training:
        compose_map_func = (lambda image, annotation, mask, mask_shape:
                            preprocess_fn(0, image, annotation, mask, mask_shape, is_training))
        ...
        compose_map_func = (lambda image_id, image, annotation, mask, mask_shape:
                            preprocess_fn(image_id, image, annotation, mask, mask_shape, is_training))
        ds = ds.map(operations=compose_map_func,
                    input_columns=["image_id", "image", "annotation", "mask", "mask_shape"],
                    output_columns=["image_id", "image", "image_shape", "box", "label", "valid_num", "mask"],
                    column_order=["image_id", "image", "image_shape", "box", "label", "valid_num", "mask"],
                    num_parallel_workers=num_parallel_workers)
