...
def output2json(results, out_file, dataset_id):
    '''output2json'''
    contiguous_id_to_thing_dataset_id = {}
    for i, i_d in enumerate(dataset_id):
        contiguous_id_to_thing_dataset_id[i] = i_d
    ...
    for i in range(iter_num):
        result = results[i]
        batch_num = len(result)
        for n in range(batch_num):
            segm_json_result, bbox_json_result = result[n]
            for item in segm_json_result:
                category_id = item["category_id"]
                item["category_id"] = contiguous_id_to_thing_dataset_id[category_id]
                segm_json_results.append(item)
            for item2 in bbox_json_result:
                category_id_2 = item2["category_id"]
                item2["category_id"] = contiguous_id_to_thing_dataset_id[category_id_2]
                bbox_json_results.append(item2)

    result_files['bbox'] = '{}.{}.json'.format(out_file, 'bbox')
    result_files['segm'] = '{}.{}.json'.format(out_file, 'segm')
    with open(result_files['bbox'], 'w') as fp:
        json.dump(bbox_json_results, fp)
    with open(result_files['segm'], 'w') as fp:
        json.dump(segm_json_results, fp)
    return result_files
