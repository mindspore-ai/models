...
class StopAtEpoch(Callback):
    '''StopAtEpoch'''
    def __init__(self, start_epoch=1, stop_epoch=12):
        super(StopAtEpoch, self).__init__()
        self.start_epoch = start_epoch
        self.stop_epoch = stop_epoch
        self.profiler = Profiler(start_profile=False, output_path='./summary_dir/test11/profiler_data')
...
class MaskRcnn_Mobilenetv1_Infer(nn.Cell):
    def __init__(self, config):
        super(MaskRcnn_Mobilenetv1_Infer, self).__init__()
        self.network = Mask_Rcnn_Mobilenetv1(config)
