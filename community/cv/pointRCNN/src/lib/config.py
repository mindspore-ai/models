...
__C.PC_AREA_SCOPE = np.array([[-40, 40], [-1, 3],
                              [0,
...
__C.RCNN.SA_CONFIG.MLPS = [[128, 128, 128], [128, 128, 256], [256, 256, 512]]
...
def save_config_to_file(cfg_, pre='cfg', logger=None):
    """save config to file"""
    for key, val in cfg_.items():
        if isinstance(cfg_[key], edict):
            if logger is not None:
                logger.info('\n%s.%s = edict()' % (pre, key))
            else:
                print('\n%s.%s = edict()' % (pre, key))
            save_config_to_file(cfg_[key], pre=pre + '.' + key, logger=logger)
            continue
