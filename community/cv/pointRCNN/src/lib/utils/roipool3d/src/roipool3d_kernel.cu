...
void roipool3dLauncher_slow(int batch_size, int pts_num, int boxes_num,
                            ...
                            int* pooled_empty_flag) {
  roipool3d_forward<<<DIVUP(boxes_num, THREADS_PER_BLOCK), THREADS_PER_BLOCK>>>(
...
void roipool3dLauncher(int batch_size, int pts_num, int boxes_num,
  ...
  cudaMemset(
      pooled_features, 0,
      batch_size * boxes_num * 512 * (3 + feature_in_len) * sizeof(float));
  ...
  assign_pts_to_box3d<<<blocks, threads>>>(batch_size, pts_num, boxes_num, xyz,
                                           boxes3d, pts_assign);
  ...
  roipool3d_forward<<<blocks_pool, threads>>>(
      batch_size, pts_num, boxes_num, feature_in_len, sampled_pts_num, xyz,
      pts_idx, pts_feature, pooled_features, pooled_empty_flag);
