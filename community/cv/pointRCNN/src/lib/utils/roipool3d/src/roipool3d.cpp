...
void roipool3dLauncher_slow(int batch_size, int pts_num, int boxes_num,
...
void roipool3dLauncher(int batch_size, int pts_num, int boxes_num,
                       int feature_in_len, int sampled_pts_num,
                       const float* xyz, const float* boxes3d,
                       const float* pts_feature, float* pooled_features,
                       int* pooled_empty_flag);

int roipool3d_gpu_slow(at::Tensor xyz, at::Tensor boxes3d,
                       at::Tensor pts_feature, at::Tensor pooled_features,
  ...
  roipool3dLauncher_slow(batch_size, pts_num, boxes_num, feature_in_len,
                         sampled_pts_num, xyz_data, boxes3d_data,
                         pts_feature_data, pooled_features_data,
...
extern "C" int ms_roipool3d_gpu(int nparam, void** params, int* ndims,
...
int pts_in_boxes3d_cpu(at::Tensor pts_flag, at::Tensor pts,
                       at::Tensor boxes3d) {
  ...
  int64_t boxes_num = boxes3d.size(0);
  int64_t pts_num = pts.size(0);

  int64_t* pts_flag_flat = pts_flag.data<int64_t>();
  float* pts_flat = pts.data<float>();
  float* boxes3d_flat = boxes3d.data<float>();

  memset(pts_flag_flat, 0, boxes_num * pts_num * sizeof(int64_t));
  ...
  for (i = 0; i < boxes_num; i++) {
    for (j = 0; j < pts_num; j++) {
      cur_in_flag = pt_in_box3d_cpu(
          ...
          boxes3d_flat[i * 7], boxes3d_flat[i * 7 + 1], boxes3d_flat[i * 7 + 2],
          boxes3d_flat[i * 7 + 3], boxes3d_flat[i * 7 + 4],
          boxes3d_flat[i * 7 + 5], boxes3d_flat[i * 7 + 6]);
...
extern "C" int ms_pts_in_boxes3d_cpu(int nparam, void** params, int* ndims,
                                     int64_t** shapes, const char** dtypes,
                                     void* stream, void* extra) {
  auto tensors =
      get_torch_tensors(nparam, params, ndims, shapes, dtypes, c10::kCPU);
  pts_in_boxes3d_cpu(tensors[0], tensors[1], tensors[2]);
  tensors[3].copy_(tensors[0]);
  return 0;
}

int roipool3d_cpu(at::Tensor pts, at::Tensor boxes3d, at::Tensor pts_feature,
                  at::Tensor pooled_pts, at::Tensor pooled_features,
                  at::Tensor pooled_empty_flag) {
  // param pts: (N, 3) [x, y, z]
  // param boxes3d: (M, 7) [x, y, z, h, w, l, ry]
  ...
  int64_t boxes_num = boxes3d.size(0);
  int64_t pts_num = pts.size(0);
  int64_t feature_len = pts_feature.size(1);
  int64_t sampled_pts_num = pooled_pts.size(1);
  ...
  memset(pooled_empty_flag_flat, 0, boxes_num * sizeof(int64_t));

  int i, j, k;
  int temp_idx;
  int duplicate_idx, cur_in_flag;
  for (i = 0; i < boxes_num; i++) {
    int cnt = 0;
    for (j = 0; j < pts_num; j++) {
      cur_in_flag = pt_in_box3d_cpu(
    ...
    if (cnt == 0) {
      ...
      for (j = cnt; j < sampled_pts_num; j++) {
        ...
        for (k = 0; k < 3; k++)
          pooled_pts_flat[temp_idx + k] = pooled_pts_flat[duplicate_idx + k];
        temp_idx = i * sampled_pts_num * feature_len + j * feature_len;
        duplicate_idx =
            i * sampled_pts_num * feature_len + (j % cnt) * feature_len;
        for (k = 0; k < feature_len; k++) {
          pooled_features_flat[temp_idx + k] =
...
extern "C" int ms_roipool3d_cpu(int nparam, void** params, int* ndims,
                                int64_t** shapes, const char** dtypes,
                                void* stream, void* extra) {
  auto tensors =
      get_torch_tensors(nparam, params, ndims, shapes, dtypes, c10::kCPU);
  roipool3d_cpu(tensors[0], tensors[1], tensors[2], tensors[3], tensors[4],
                tensors[5]);
  return 0;
}
