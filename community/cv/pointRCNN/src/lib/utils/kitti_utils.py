...
import scipy
import src.lib.utils.object3d as object3d
...
def rotate_pc_along_y_torch(pc, rot_angle):
    ...
    R: Tensor = ops.concat(
        (raw_1.expand_dims(axis=1), raw_2.expand_dims(axis=1)),
        axis=1)  # (N, 2, 2)
...
def boxes3d_to_corners3d_torch(boxes3d, flip=False):
    ...
    """
    boxes_num = boxes3d.shape[0]
    h, w, l, ry = boxes3d[:, 3:4], boxes3d[:, 4:5], boxes3d[:,
                                                            5:6], boxes3d[:,
                                                                          6:7]
    if flip:
        ry = ry + np.pi
    ...
    x_corners: Tensor = ops.concat(
        [l / 2., l / 2., -l / 2., -l / 2., l / 2., l / 2., -l / 2., -l / 2.],
        axis=1)  # (N, 8)
    y_corners: Tensor = ops.concat(
        [zeros, zeros, zeros, zeros, -h, -h, -h, -h], axis=1)  # (N, 8)
    z_corners: Tensor = ops.concat(
        [w / 2., -w / 2., -w / 2., w / 2., w / 2., -w / 2., -w / 2., w / 2.],
        axis=1)  # (N, 8)
    corners = ops.concat(
        (x_corners.expand_dims(axis=1), y_corners.expand_dims(axis=1),
         z_corners.expand_dims(axis=1)),
        axis=1)  # (N, 3, 8)
    ...
    R = ops.concat((raw_1.expand_dims(axis=1), raw_2.expand_dims(axis=1),
                    raw_3.expand_dims(axis=1)),
                   axis=1)  # (N, 3, 3)
...
def get_iou3d(corners3d, query_corners3d, need_bev=False):
    ...
    for i in range(N):
        for j in range(M):
            ...
            bottom_a, bottom_b = Polygon(A[i, 0:4,
                                           [0, 2]].T), Polygon(B[j, 0:4,
                                                                 [0, 2]].T)
            ...
            union3d = bottom_a.area * (max_h_a[i] -
                                       min_h_a[i]) + bottom_b.area * (
                                           max_h_b[j] - min_h_b[j]) - overlap3d
            iou3d[i][j] = overlap3d / union3d
            iou_bev[i][j] = bottom_overlap / (bottom_a.area + bottom_b.area -
                                              bottom_overlap)
