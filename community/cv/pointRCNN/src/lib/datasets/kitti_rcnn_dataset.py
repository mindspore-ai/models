...
class KittiRCNNDataset(KittiDataset):
    ...
    def __init__(self,
        ...
        if classes == 'Car':
            ...
            aug_scene_root_dir = os.path.join(root_dir, 'KITTI',
                                              'aug_scene_ped')
        ...
        if split == 'train_aug':
            self.aug_label_dir = os.path.join(aug_scene_root_dir, 'training',
                                              'aug_label')
            self.aug_pts_dir = os.path.join(aug_scene_root_dir, 'training',
                                            'rectified_data')
        else:
            self.aug_label_dir = os.path.join(aug_scene_root_dir, 'training',
                                              'aug_label')
        ...
        if cfg.RPN.ENABLED:
            if gt_database_dir is not None:
                ...
                if cfg.GT_AUG_HARD_RATIO > 0:
                    ...
                    logger.info(
                        'Loading gt_database(easy(pt_num>100): %d, hard(pt_num<=100): %d) from %s'
                        % (len(easy_list), len(hard_list), gt_database_dir))
                else:
                    logger.info('Loading gt_database(%d) from %s' %
                                (len(self.gt_database), gt_database_dir))
            ...
            if mode == 'TRAIN':
                ...
                self.logger.info('Load testing samples from %s' %
                                 self.imageset_dir)
                self.logger.info('Done: total test samples %d' %
                                 len(self.sample_id_list))
            ...
            print('Done: filter %s results for rcnn training: %d / %d\n' %
                  (self.mode, len(self.sample_id_list), len(
                      self.image_idx_list)))
    ...
    def preprocess_rpn_training_data(self):
        ...
        """
        self.logger.info('Loading %s samples from %s ...' %
                         (self.mode, self.label_dir))
        for idx in range(0, self.num_sample):
            sample_id = int(self.image_idx_list[idx])
            obj_list = self.filtrate_objects(self.get_label(sample_id))
            if not obj_list:
    ...
    def get_rpn_features(rpn_feature_dir, idx):
        ...
        rpn_intensity_file = os.path.join(rpn_feature_dir,
                                          '%06d_intensity.npy' % idx)
        if cfg.RCNN.USE_SEG_SCORE:
            rpn_seg_file = os.path.join(rpn_feature_dir,
                                        '%06d_rawscore.npy' % idx)
            rpn_seg_score = np.load(rpn_seg_file).reshape(-1)
            rpn_seg_score = ms.ops.Sigmoid()(
                ms.Tensor.from_numpy(rpn_seg_score)).asnumpy()
        else:
            rpn_seg_file = os.path.join(rpn_feature_dir, '%06d_seg.npy' % idx)
            rpn_seg_score = np.load(rpn_seg_file).reshape(-1)
        return np.load(rpn_xyz_file), np.load(rpn_feature_file), np.load(
            rpn_intensity_file).reshape(-1), rpn_seg_score
    ...
    def filtrate_objects(self, obj_list):
        ...
        for obj in obj_list:
            if obj.cls_type not in type_whitelist:  # rm Van, 20180928
                continue
            if self.mode == 'TRAIN' and cfg.PC_REDUCE_BY_RANGE and (
                    self.check_pc_range(obj.pos) is False):
                continue
    ...
    def get_valid_flag(pts_rect, pts_img, pts_rect_depth, img_shape):
        ...
        if cfg.PC_REDUCE_BY_RANGE:
            x_range, y_range, z_range = cfg.PC_AREA_SCOPE
            pts_x, pts_y, pts_z = pts_rect[:, 0], pts_rect[:, 1], pts_rect[:,
                                                                           2]
    ...
    def get_rpn_sample(self, index):
        ...
        if cfg.GT_AUG_ENABLED and self.mode == 'TRAIN':  #True
            ...
            all_gt_obj_list = self.filtrate_dc_objects(
                self.get_label(sample_id))
            all_gt_boxes3d = kitti_utils.objs_to_boxes3d(all_gt_obj_list)

            if np.random.rand() < cfg.GT_AUG_APPLY_PROB:
                # augment one scene
                gt_aug_flag, pts_rect, pts_intensity, _, extra_gt_obj_list = \
        ...
        if self.mode == 'TRAIN' or self.random_select:
            if self.npoints < len(pts_rect):
                ...
                choice = np.concatenate((near_idxs_choice, far_idxs_choice), axis=0) \
                    if far_idxs_choice.size > 0 else near_idxs_choice
        ...
        ret_pts_features = np.concatenate(
            pts_features, axis=1) if len(pts_features) > 1 else pts_features[0]

        sample_info = self.get_sample_info(sample_id, ret_pts_rect,
                                           ret_pts_features, gt_aug_flag,
                                           extra_gt_obj_list)
        return sample_info

    def get_sample_info(self, sample_id, ret_pts_rect, ret_pts_features,
                        gt_aug_flag, extra_gt_obj_list):
        ...
        if cfg.AUG_DATA and self.mode == 'TRAIN':
            aug_pts_rect, aug_gt_boxes3d, _ = self.data_augmentation(
                aug_pts_rect, aug_gt_boxes3d, gt_alpha, sample_id)

        # prepare input
        if cfg.RPN.USE_INTENSITY:
            pts_input = np.concatenate((aug_pts_rect, ret_pts_features),
                                       axis=1)  # (N, C)
        else:
    ...
    def apply_gt_aug_to_one_scene(self, sample_id, pts_rect, pts_intensity,
                                  all_gt_boxes3d):
        """
        :param pts_rect: (N, 3)
        ...
        cur_gt_boxes3d[:,
                       5] += 0.5  # enlarge new added box to avoid too nearby boxes
        ...
        while try_times > 0:
            ...
            if cfg.GT_AUG_HARD_RATIO > 0:
                ...
                rand_idx = np.random.randint(0, len(self.gt_database))
            ...
            if cfg.PC_REDUCE_BY_RANGE and (self.check_pc_range(center) is
                                           False):
                continue

            if len(new_gt_points) < 5:  # too few points
                continue
            ...
            new_corners = kitti_utils.boxes3d_to_corners3d(
                new_enlarged_box3d.reshape(1, 7))
            ...
            enlarged_box3d[
                3] += 2  # remove the points above and below the object
            boxes_pts_mask_list = roipool3d_utils.pts_in_boxes3d_cpu(
                Tensor.from_numpy(pts_rect),
                Tensor.from_numpy(enlarged_box3d.reshape(1, 7)))
            ...
            cur_gt_boxes3d = np.concatenate(
                (cur_gt_boxes3d, new_enlarged_box3d.reshape(1, 7)), axis=0)
    ...
    def data_augmentation(self,
        ...
        if 'rotation' in aug_list and aug_enable[0] < cfg.AUG_METHOD_PROB[0]:
            angle = np.random.uniform(-np.pi / cfg.AUG_ROT_RANGE,
                                      np.pi / cfg.AUG_ROT_RANGE)
            aug_pts_rect = kitti_utils.rotate_pc_along_y(aug_pts_rect,
                                                         rot_angle=angle)
            if stage == 1:
                # xyz change, hwl unchange
                aug_gt_boxes3d = kitti_utils.rotate_pc_along_y(aug_gt_boxes3d,
                                                               rot_angle=angle)
                ...
                aug_gt_boxes3d[0] = self.rotate_box3d_along_y(
                    aug_gt_boxes3d[0], angle)
                aug_gt_boxes3d[1] = self.rotate_box3d_along_y(
                    aug_gt_boxes3d[1], angle)
            else:
                raise NotImplementedError
        ...
        if 'flip' in aug_list and aug_enable[2] < cfg.AUG_METHOD_PROB[2]:
            ...
            if stage == 1:
                aug_gt_boxes3d[:, 6] = np.sign(
                    aug_gt_boxes3d[:, 6]) * np.pi - aug_gt_boxes3d[:, 6]
            elif stage == 2:
                assert aug_gt_boxes3d.shape[0] == 2
                aug_gt_boxes3d[0, 6] = np.sign(
                    aug_gt_boxes3d[0, 6]) * np.pi - aug_gt_boxes3d[0, 6]
                aug_gt_boxes3d[1, 6] = np.sign(
                    aug_gt_boxes3d[1, 6]) * np.pi - aug_gt_boxes3d[1, 6]
            else:
                raise NotImplementedError
    ...
    def get_rcnn_sample_info(self, roi_info):
        """get rcnn sample info"""
        sample_id, gt_box3d = roi_info['sample_id'], roi_info['gt_box3d']
        rpn_xyz, rpn_features, rpn_intensity, seg_mask = self.rpn_feature_list[
            sample_id]
        ...
        pooled_boxes3d = kitti_utils.enlarge_box3d(roi_box3d.reshape(1, 7),
                                                   cfg.RCNN.POOL_EXTRA_WIDTH)

        boxes_pts_mask_list = roipool3d_utils.pts_in_boxes3d_cpu(
            Tensor.from_numpy(rpn_xyz), Tensor.from_numpy(pooled_boxes3d))
        pt_mask_flag = (boxes_pts_mask_list[0].asnumpy() == 1)
        ...
        if cfg.AUG_DATA and self.mode == 'TRAIN':
            ...
            temp_boxes3d = np.concatenate(
                [aug_roi_box3d.reshape(1, 7),
                 aug_gt_box3d.reshape(1, 7)],
                axis=0)
            temp_x, temp_z, temp_ry = temp_boxes3d[:,
                                                   0], temp_boxes3d[:,
                                                                    2], temp_boxes3d[:,
                                                                                     6]
            ...
            # data augmentation
            aug_pts, aug_boxes3d, _ = self.data_augmentation(aug_pts,
                                                             temp_boxes3d,
                                                             temp_alpha,
                                                             mustaug=True,
                                                             stage=2)
        ...
        if aug_pts.shape[0] == 0:
            ...
            input_channel = 3 + int(cfg.RCNN.USE_INTENSITY) + int(
            ...
            if cfg.RCNN.USE_MASK:
                if cfg.RCNN.MASK_TYPE == 'seg':
                    ...
                    pts_mask = roipool3d_utils.pts_in_boxes3d_cpu(
                        Tensor.from_numpy(aug_pts),
        ...
        aug_gt_corners = kitti_utils.boxes3d_to_corners3d(
            aug_gt_box3d.reshape(-1, 7))
        aug_roi_corners = kitti_utils.boxes3d_to_corners3d(
            aug_roi_box3d.reshape(-1, 7))
        ...
        pts_input_ct, gt_box3d_ct = self.canonical_transform(
            pts_input, aug_roi_box3d, aug_gt_box3d)
        pts_input_ct, pts_features = self.rcnn_input_sample(
            pts_input_ct, pts_features)
    ...
    def canonical_transform(pts_input, roi_box3d, gt_box3d):
        ...
        gt_box3d_ct = kitti_utils.rotate_pc_along_y(gt_box3d_ct.reshape(1, 7),
                                                    roi_ry).reshape(7)
    ...
    def canonical_transform_batch(pts_input, roi_boxes3d, gt_boxes3d):
        ...
        # shift to center
        pts_input[:, :,
                  [0, 1, 2]] = pts_input[:, :, [0, 1, 2]] - roi_center.reshape(
                      -1, 1, 3)
        gt_boxes3d_ct = np.copy(gt_boxes3d)
        gt_boxes3d_ct[:, 0:3] = gt_boxes3d_ct[:, 0:3] - roi_center
        # rotate to the direction of head
        gt_boxes3d_ct = kitti_utils.rotate_pc_along_y_torch(
            Tensor.from_numpy(gt_boxes3d_ct.reshape(-1, 1, 7)),
            Tensor.from_numpy(roi_ry)).asnumpy().reshape(-1, 7)
        gt_boxes3d_ct[:, 6] = gt_boxes3d_ct[:, 6] - roi_ry
        pts_input = kitti_utils.rotate_pc_along_y_torch(
            Tensor.from_numpy(pts_input), Tensor.from_numpy(roi_ry)).asnumpy()
    ...
    def rcnn_input_sample(pts_input, pts_features):
        """rcnn_input_sample"""
        choice = np.random.choice(pts_input.shape[0],
                                  cfg.RCNN.NUM_POINTS,
                                  replace=True)
    ...
    def aug_roi_by_noise(self, roi_info):
        ...
        while temp_iou < pos_thresh and cnt < 10:
            ...
            aug_corners = kitti_utils.boxes3d_to_corners3d(
    ...
    def random_aug_box3d(box3d):
        ...
        if cfg.RCNN.REG_AUG_METHOD == 'single':
            ...
            angle_rot = (np.random.rand(1) - 0.5) / (0.5 / (np.pi / 12)
                                                     )  # [-pi/12 ~ pi/12]
        ...
        if cfg.RCNN.REG_AUG_METHOD == 'multiple':
            ...
            return aug_box3d
        if cfg.RCNN.REG_AUG_METHOD == 'normal':
    ...
    def get_proposal_from_file(self, index):
        """get proposal from file"""
        sample_id = int(self.image_idx_list[index])
        proposal_file = os.path.join(self.rcnn_eval_roi_dir,
                                     '%06d.txt' % sample_id)
        roi_obj_list = kitti_utils.get_objects_from_label(proposal_file)

        rpn_xyz, rpn_features, rpn_intensity, seg_mask = self.get_rpn_features(
            self.rcnn_eval_feature_dir, sample_id)
        ...
        for obj in roi_obj_list:
            box3d = np.array([
                obj.pos[0], obj.pos[1], obj.pos[2], obj.h, obj.w, obj.l, obj.ry
            ],
                             dtype=np.float32)
        ...
        if cfg.RCNN.USE_INTENSITY:
            pts_extra_input_list = [
                pts_intensity.reshape(-1, 1),
                seg_mask.reshape(-1, 1)
            ]
        else:
        ...
        pts_input, pts_features = roipool3d_utils.roipool3d_cpu(
            roi_boxes3d,
            pts_rect,
            pts_rpn_features,
            pts_extra_input,
        ...
        gt_obj_list = self.filtrate_objects(self.get_label(sample_id))
        gt_boxes3d = np.zeros((len(gt_obj_list), 7), dtype=np.float32)
    ...
    def get_rcnn_training_sample_batch(self, index):
        """get_rcnn_training_sample_batch"""
        sample_id = int(self.sample_id_list[index])
        ...
        iou3d = kitti_utils.get_iou3d(
            kitti_utils.boxes3d_to_corners3d(roi_boxes3d),
        ...
        if fg_num_rois > 0 and bg_num_rois > 0:
            ...
            rand_num = ms.Tensor.from_numpy(rand_num).type_as(
                gt_boxes3d).astype(ms.int32)
        ...
        if fg_rois_per_this_image > 0:
            ...
            fg_rois, fg_iou3d = self.aug_roi_by_noise_batch(fg_rois_src,
                                                            gt_of_fg_rois,
                                                            aug_times=10)
        ...
        if bg_rois_per_this_image > 0:
            ...
            bg_rois, bg_iou3d = self.aug_roi_by_noise_batch(bg_rois_src,
                                                            gt_of_bg_rois,
                                                            aug_times=1)
        ...
        sample_info = self.data_aug(rois, pts_input, gt_of_rois,
                                    pts_empty_flag, iou_of_rois, sample_id,
                                    pts_features)
        return sample_info

    def data_aug(self, rois, pts_input, gt_of_rois, pts_empty_flag,
                 iou_of_rois, sample_id, pts_features):
        """data augmentation"""
        # data augmentation
        if cfg.AUG_DATA and self.mode == 'TRAIN':
            for k in range(len(rois)):
                ...
                temp_beta = np.arctan2(temp_z, temp_x).astype(np.float64)
                temp_alpha = -np.sign(
                    temp_beta) * np.pi / 2 + temp_beta + temp_ry
        ...
        # regression valid mask
        reg_valid_mask = (iou_of_rois > cfg.RCNN.REG_FG_THRESH).astype(
            np.int32) & valid_mask

        # classification label
        cls_label = (iou_of_rois > cfg.RCNN.CLS_FG_THRESH).astype(np.int32)
        invalid_mask = (iou_of_rois > cfg.RCNN.CLS_BG_THRESH) & (
            iou_of_rois < cfg.RCNN.CLS_FG_THRESH)
        cls_label[invalid_mask] = -1
        cls_label[valid_mask == 0] = -1

        # canonical transform and sampling
        pts_input_ct, gt_boxes3d_ct = self.canonical_transform_batch(
            pts_input, rois, gt_of_rois)
    ...
    def sample_bg_inds(self, hard_bg_inds, easy_bg_inds,
        ...
        if hard_bg_inds.size > 0 and easy_bg_inds.size > 0:
            ...
            # sampling hard bg
            rand_num = np.floor(
                np.random.rand(hard_bg_rois_num) * hard_bg_inds.size).astype(
                    np.int32)
            hard_bg_inds = hard_bg_inds[rand_num]
            # sampling easy bg
            rand_num = np.floor(
                np.random.rand(easy_bg_rois_num) * easy_bg_inds.size).astype(
                    np.int32)
        ...
        for key in batch[0].keys():
            ...
            if isinstance(batch[0][key], np.ndarray):
                if batch_size == 1:
                    ...
                    ans_dict[key] = np.concatenate([
                        batch[k][key][np.newaxis, ...]
                        for k in range(batch_size)
                    ],
                                                   axis=0)
