...
def index_points(points, idx):
    ...
    index = P.Concat(-1)((batch_indices.reshape(idx.shape + (1,)),
                          idx.reshape(idx.shape + (1,))))
...
def query_ball_point(radius, nsample, xyz, new_xyz):
    ...
    index = group_idx != -1
    group_first = P.Select()(index, -1 * P.OnesLike()(group_first),
                             group_first)
...
def sample_and_group_all(xyz, points):
    ...
    new_points = P.Concat(-1)((grouped_xyz, P.Reshape()(points,
                                                        (B, 1, N, -1))))
    return new_points
...
def sample_and_group(npoint, radius, nsample, xyz, points):
    ...
    if points is not None:
        ...
        new_points = P.Concat(-1)(
            (grouped_xyz_norm, grouped_points))  # [B, S, nsample, C+D]
    else:
...
class PointNetSetAbstraction(nn.Cell):
    ...
    def construct(self, xyz, points):
        ...
        new_points = self.transpose(new_points.reshape((d1, d2 * d3, d4)),
                                    (0, 2, 1))
        new_points = self.transpose(new_points.reshape((d1 * d4, d2, d3)),
                                    (0, 2, 1)).reshape((d1, d4, d3, d2))
