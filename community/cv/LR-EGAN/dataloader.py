...
def LoadMatData(params):
    '''Load tabular data'''
    filepath = f'{params.data_path}{params.data_name}.mat'
    ...
    info += f'Original shape: data={data.shape} \n'
    ...
    info += f'Data shape: data={data.shape}\n'
...
def LoadTabularData(params):
    ...
    cols['num'] = list(OrderedSet(list(data.columns)
                                  ).difference(OrderedSet(cols['cat'])))
...
class LoadDocumentData():
    ...
    def __init__(self, params):
        ...
        self.class_num = len(list(OrderedSet(self.y_origin)))
