...
def vocab_to_dict_key_token_json(vocab_file):
    """ Loads a vocab json file into a dict, key is token.  """
    vocab = collections.OrderedDict()
    with open(vocab_file, 'r') as reader:
...
def vocab_to_dict_key_id_json(vocab_file):
...
class FullTokenizer():
    ...
    def __init__(self, vocab_file, do_lower_case=True):
        self.vocab_dict = vocab_to_dict_key_token_json(vocab_file)
        ...
        self.special_tokens = ['<s>', '</s>', '<unk>', '<pad>', '<mask>']
        self.special_tokens_ids = self.convert_tokens_to_ids(self.special_tokens)
    ...
    def convert_ids_to_tokens(self, ids, skip_special_tokens=False):
        ...
        output = []
        for _id in ids:
            if skip_special_tokens and _id in self.special_tokens_ids:
                continue
            output.append(self.inv_vocab_dict[_id])
        return output
