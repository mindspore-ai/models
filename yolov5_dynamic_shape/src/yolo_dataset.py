...
from src.transforms import reshape_fn, MultiScaleTrans, PreprocessTrueBox, PadAfterBatch
...
def create_yolo_dataset(image_dir, anno_path, batch_size, device_num, rank,
    ...
    # image: origin image; or mosaic image (random 0.5)
    # annotation: (N, 5) , N: num of annos, not fixed; 5:[x_min y_min x_max y_max, label]
    # input_size: [640, 640]
    # mosaic_flag: 0: origin; 1: mosaic
    # -----mosaic:将四张图片进行随机裁剪，再拼接到一张图上作为训练数据
    ...
    if is_training:
        ...
        map2_out_column_names = ["bbox1", "bbox2", "bbox3",
        ...
        # image: (640, 640)
        # annotation；(N, 5) N: num of annos, not fixed; 5:[x_min y_min x_max y_max, label] 经过筛选
        # size: [640, 640]
        ...
        dataset = dataset.project(["image", "bbox1", "bbox2", "bbox3", "gt_box1", "gt_box2", "gt_box3"])
