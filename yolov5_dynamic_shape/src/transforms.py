...
class PreprocessTrueBox:
    ...
    def __call__(self, anno, input_shape):
        ...
        return np.array(bbox_true_1), np.array(bbox_true_2), np.array(bbox_true_3), \
...
def _correct_bbox_by_candidates(candidates, input_w, input_h, image_w,
                                image_h, flip, box, allow_outside_center, max_boxes):
    """Calculate correct boxes."""
    # candidates: (n_c, 4), 4: (dx, dy, nw, nh)
    # box: (N, 5) , N: num of annos, not fixed; 5:[x_min y_min x_max y_max, label]
    while candidates:
        if len(candidates) > 1:
        ...
        if t_box.shape[0] > 0:
            ...
            return t_box, candidate
    print("the t_box is none.")
    return np.zeros(shape=[1, 5], dtype=np.float32), (0, 0, nw, nh)
...
def _data_aug(image, box, jitter, hue, sat, val, image_input_size, max_boxes,
    ...
    box_data, candidate = _correct_bbox_by_candidates(candidates=candidates, input_w=input_w, input_h=input_h,
                                                      ...
                                                      allow_outside_center=True, max_boxes=max_boxes)
    dx, dy, nw, nh = candidate
...
class PadAfterBatch:
    ...
    def __call__(self, img, yt0, yt1, yt2, gt1, gt2, gt3, batchInfo):

        gt1_len = max(max([len(i) for i in gt1]), 1)
        gt2_len = max(max([len(i) for i in gt2]), 1)
        gt3_len = max(max([len(i) for i in gt3]), 1)
        batch_size = len(gt1)
        pad_gt_box1 = np.zeros(shape=[batch_size, gt1_len, 4], dtype=np.float32)
        pad_gt_box2 = np.zeros(shape=[batch_size, gt2_len, 4], dtype=np.float32)
        pad_gt_box3 = np.zeros(shape=[batch_size, gt3_len, 4], dtype=np.float32)

        for batch_id in range(batch_size):
            pad_gt_box1[batch_id, :gt1[batch_id].shape[0], :] = gt1[batch_id]
            pad_gt_box2[batch_id, :gt2[batch_id].shape[0], :] = gt2[batch_id]
            pad_gt_box3[batch_id, :gt3[batch_id].shape[0], :] = gt3[batch_id]

        return np.array(img), np.array(yt0), np.array(yt1), np.array(yt2), \
               np.array(pad_gt_box1), np.array(pad_gt_box2), np.array(pad_gt_box3)
