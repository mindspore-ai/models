...
class Iou(nn.Cell):
    ...
    def construct(self, box1, box2):
        """
        box1: pred_box [batch, gx, gy, anchors, 1,      4:[x_center, y_center, w, h] ]  (N, 40, 40, 3, 1, 4)
        box2: gt_box   [batch, 1,  1,  1,       None, 4]
        ...
        intersect_maxs = self.min(box1_maxs, box2_maxs) #[batch, gx, gy, anchors, None, 2]
...
class YoloLossBlock(nn.Cell):
    ...
    def construct(self, prediction, pred_xy, pred_wh, y_true, gt_box, input_shape):
        """
        prediction : origin output from yolo                      [n, grid_size[0], grid_size[1], 3,
                                                                   num_attrib(85: xywh:4, confidence:1, probs:80)]
        pred_xy: (sigmoid(xy)+grid)/grid_size                     [n, grid_size[0], grid_size[1], 3, 2]
        pred_wh: (exp(wh)*anchors)/input_shape                    [n, grid_size[0], grid_size[1], 3, 2]
        y_true : after normalize                                  [config.per_batch_size, 40, 40, 3, 85]
        gt_box: [batch, maxboxes, xyhw] after normalize           [config.per_batch_size, None, 4]
        input_shape                                               [640, 640]
        """

        #"image": [config.per_batch_size, 12, 320, 320],

        object_mask = y_true[:, :, :, :, 4:5]  # is_choose
        class_probs = y_true[:, :, :, :, 5:]   # which class, 这个是不是也可以变成一个数值而不是mask??不行，网络输出就是各个class的概率
        true_boxes = y_true[:, :, :, :, :4]    # (xywh)
        ...
        gt_box = self.reshape(gt_box, (gt_shape[0], 1, 1, 1, gt_shape[1], gt_shape[2])) # (N, 1,1,1,None, 4)

        # add one more dimension for broadcast
        iou = self.iou(self.expand_dims(pred_boxes, -2), gt_box) #(N, 40, 40, 3, 1, 4)
